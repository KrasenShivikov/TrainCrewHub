...
Option 2: Install and provide the "ws" package:
  ...
  const client = new RealtimeClient(url, {
    ...options,
    transport: ws
  })`):new Error(`WebSocket not available: ${r}`)}this._setupConnectionHandlers()}}endpointURL(){return this._appendParams(this.endPoint,Object.assign({},this.params,{vsn:this.vsn}))}disconnect(t,r){if(!this.isDisconnecting())if(this._setConnectionState("disconnecting",!0),this.conn){const n=setTimeout(()=>{this._setConnectionState("disconnected")},100);this.conn.onclose=()=>{clearTimeout(n),this._setConnectionState("disconnected")},typeof this.conn.close=="function"&&(t?this.conn.close(t,r??""):this.conn.close()),this._teardownConnection()}else this._setConnectionState("disconnected")}getChannels(){return this.channels}async removeChannel(t){const r=await t.unsubscribe();return r==="ok"&&this._remove(t),this.channels.length===0&&this.disconnect(),r}async removeAllChannels(){const t=await Promise.all(this.channels.map(r=>r.unsubscribe()));return this.channels=[],this.disconnect(),t}log(t,r,n){this.logger(t,r,n)}connectionState(){switch(this.conn&&this.conn.readyState){case lt.connecting:return $t.Connecting;case lt.open:return $t.Open;case lt.closing:return $t.Closing;default:return $t.Closed}}isConnected(){return this.connectionState()===$t.Open}isConnecting(){return this._connectionState==="connecting"}isDisconnecting(){return this._connectionState==="disconnecting"}channel(t,r={config:{}}){const n=`realtime:${t}`,s=this.getChannels().find(a=>a.topic===n);if(s)return s;{const a=new sr(`realtime:${t}`,r,this);return this.channels.push(a),a}}push(t){const{topic:r,event:n,payload:s,ref:a}=t,i=()=>{this.encode(t,o=>{var l;(l=this.conn)===null||l===void 0||l.send(o)})};this.log("push",`${r} ${n} (${a})`,s),this.isConnected()?i():this.sendBuffer.push(i)}async setAuth(t=null){this._authPromise=this._performAuth(t);try{await this._authPromise}finally{this._authPromise=null}}_isManualToken(){return this._manuallySetToken}async sendHeartbeat(){var t;if(!this.isConnected()){try{this.heartbeatCallback("disconnected")}catch(r){this.log("error","error in heartbeat callback",r)}return}if(this.pendingHeartbeatRef){this.pendingHeartbeatRef=null,this._heartbeatSentAt=null,this.log("transport","heartbeat timeout. Attempting to re-establish connection");try{this.heartbeatCallback("timeout")}catch(r){this.log("error","error in heartbeat callback",r)}this._wasManualDisconnect=!1,(t=this.conn)===null||t===void 0||t.close(Ql,"heartbeat timeout"),setTimeout(()=>{var r;this.isConnected()||(r=this.reconnectTimer)===null||r===void 0||r.scheduleTimeout()},_n.HEARTBEAT_TIMEOUT_FALLBACK);return}this._heartbeatSentAt=Date.now(),this.pendingHeartbeatRef=this._makeRef(),this.push({topic:"phoenix",event:"heartbeat",payload:{},ref:this.pendingHeartbeatRef});try{this.heartbeatCallback("sent")}catch(r){this.log("error","error in heartbeat callback",r)}this._setAuthSafely("heartbeat")}onHeartbeat(t){this.heartbeatCallback=t}flushSendBuffer(){this.isConnected()&&this.sendBuffer.length>0&&(this.sendBuffer.forEach(t=>t()),this.sendBuffer=[])}_makeRef(){let t=this.ref+1;return t===this.ref?this.ref=0:this.ref=t,this.ref.toString()}_leaveOpenTopic(t){let r=this.channels.find(n=>n.topic===t&&(n._isJoined()||n._isJoining()));r&&(this.log("transport",`leaving duplicate topic "${t}"`),r.unsubscribe())}_remove(t){this.channels=this.channels.filter(r=>r.topic!==t.topic)}_onConnMessage(t){this.decode(t.data,r=>{if(r.topic==="phoenix"&&r.event==="phx_reply"&&r.ref&&r.ref===this.pendingHeartbeatRef){const d=this._heartbeatSentAt?Date.now()-this._heartbeatSentAt:void 0;try{this.heartbeatCallback(r.payload.status==="ok"?"ok":"error",d)}catch(c){this.log("error","error in heartbeat callback",c)}this._heartbeatSentAt=null,this.pendingHeartbeatRef=null}const{topic:n,event:s,payload:a,ref:i}=r,o=i?`(${i})`:"",l=a.status||"";this.log("receive",`${l} ${n} ${s} ${o}`.trim(),a),this.channels.filter(d=>d._isMember(n)).forEach(d=>d._trigger(s,a,i)),this._triggerStateCallbacks("message",r)})}_clearTimer(t){var r;t==="heartbeat"&&this.heartbeatTimer?(clearInterval(this.heartbeatTimer),this.heartbeatTimer=void 0):t==="reconnect"&&((r=this.reconnectTimer)===null||r===void 0||r.reset())}_clearAllTimers(){this._clearTimer("heartbeat"),this._clearTimer("reconnect")}_setupConnectionHandlers(){this.conn&&("binaryType"in this.conn&&(this.conn.binaryType="arraybuffer"),this.conn.onopen=()=>this._onConnOpen(),this.conn.onerror=t=>this._onConnError(t),this.conn.onmessage=t=>this._onConnMessage(t),this.conn.onclose=t=>this._onConnClose(t),this.conn.readyState===lt.open&&this._onConnOpen())}_teardownConnection(){if(this.conn){if(this.conn.readyState===lt.open||this.conn.readyState===lt.connecting)try{this.conn.close()}catch(t){this.log("error","Error closing connection",t)}this.conn.onopen=null,this.conn.onerror=null,this.conn.onmessage=null,this.conn.onclose=null,this.conn=null}this._clearAllTimers(),this._terminateWorker(),this.channels.forEach(t=>t.teardown())}_onConnOpen(){this._setConnectionState("connected"),this.log("transport",`connected to ${this.endpointURL()}`),(this._authPromise||(this.accessToken&&!this.accessTokenValue?this.setAuth():Promise.resolve())).then(()=>{this.flushSendBuffer()}).catch(r=>{this.log("error","error waiting for auth on connect",r),this.flushSendBuffer()}),this._clearTimer("reconnect"),this.worker?this.workerRef||this._startWorkerHeartbeat():this._startHeartbeat(),this._triggerStateCallbacks("open")}_startHeartbeat(){this.heartbeatTimer&&clearInterval(this.heartbeatTimer),this.heartbeatTimer=setInterval(()=>this.sendHeartbeat(),this.heartbeatIntervalMs)}_startWorkerHeartbeat(){this.workerUrl?this.log("worker",`starting worker for from ${this.workerUrl}`):this.log("worker","starting default worker");const t=this._workerObjectUrl(this.workerUrl);this.workerRef=new Worker(t),this.workerRef.onerror=r=>{this.log("worker","worker error",r.message),this._terminateWorker()},this.workerRef.onmessage=r=>{r.data.event==="keepAlive"&&this.sendHeartbeat()},this.workerRef.postMessage({event:"start",interval:this.heartbeatIntervalMs})}_terminateWorker(){this.workerRef&&(this.log("worker","terminating worker"),this.workerRef.terminate(),this.workerRef=void 0)}_onConnClose(t){var r;this._setConnectionState("disconnected"),this.log("transport","close",t),this._triggerChanError(),this._clearTimer("heartbeat"),this._wasManualDisconnect||(r=this.reconnectTimer)===null||r===void 0||r.scheduleTimeout(),this._triggerStateCallbacks("close",t)}_onConnError(t){this._setConnectionState("disconnected"),this.log("transport",`${t}`),this._triggerChanError(),this._triggerStateCallbacks("error",t);try{this.heartbeatCallback("error")}catch(r){this.log("error","error in heartbeat callback",r)}}_triggerChanError(){this.channels.forEach(t=>t._trigger(We.error))}_appendParams(t,r){if(Object.keys(r).length===0)return t;const n=t.match(/\?/)?"&":"?",s=new URLSearchParams(r);return`${t}${n}${s}`}_workerObjectUrl(t){let r;if(t)r=t;else{const n=new Blob([od],{type:"application/javascript"});r=URL.createObjectURL(n)}return r}_setConnectionState(t,r=!1){this._connectionState=t,t==="connecting"?this._wasManualDisconnect=!1:t==="disconnecting"&&(this._wasManualDisconnect=r)}async _performAuth(t=null){let r,n=!1;if(t)r=t,n=!0;else if(this.accessToken)try{r=await this.accessToken()}catch(s){this.log("error","Error fetching access token from callback",s),r=this.accessTokenValue}else r=this.accessTokenValue;n?this._manuallySetToken=!0:this.accessToken&&(this._manuallySetToken=!1),this.accessTokenValue!=r&&(this.accessTokenValue=r,this.channels.forEach(s=>{const a={access_token:r,version:Gl};r&&s.updateJoinPayload(a),s.joinedOnce&&s._isJoined()&&s._push(We.access_token,{access_token:r})}))}async _waitForAuthIfNeeded(){this._authPromise&&await this._authPromise}_setAuthSafely(t="general"){this._isManualToken()||this.setAuth().catch(r=>{this.log("error",`Error setting auth in ${t}`,r)})}_triggerStateCallbacks(t,r){try{this.stateChangeCallbacks[t].forEach(n=>{try{n(r)}catch(s){this.log("error",`error in ${t} callback`,s)}})}catch(n){this.log("error",`error triggering ${t} callbacks`,n)}}_setupReconnectionTimer(){this.reconnectTimer=new Eo(async()=>{setTimeout(async()=>{await this._waitForAuthIfNeeded(),this.isConnected()||this.connect()},_n.RECONNECT_DELAY)},this.reconnectAfterMs)}_initializeOptions(t){var r,n,s,a,i,o,l,d,c,u,p,m;switch(this.transport=(r=t==null?void 0:t.transport)!==null&&r!==void 0?r:null,this.timeout=(n=t==null?void 0:t.timeout)!==null&&n!==void 0?n:Os,this.heartbeatIntervalMs=(s=t==null?void 0:t.heartbeatIntervalMs)!==null&&s!==void 0?s:_n.HEARTBEAT_INTERVAL,this.worker=(a=t==null?void 0:t.worker)!==null&&a!==void 0?a:!1,this.accessToken=(i=t==null?void 0:t.accessToken)!==null&&i!==void 0?i:null,this.heartbeatCallback=(o=t==null?void 0:t.heartbeatCallback)!==null&&o!==void 0?o:fs,this.vsn=(l=t==null?void 0:t.vsn)!==null&&l!==void 0?l:Za,t!=null&&t.params&&(this.params=t.params),t!=null&&t.logger&&(this.logger=t.logger),(t!=null&&t.logLevel||t!=null&&t.log_level)&&(this.logLevel=t.logLevel||t.log_level,this.params=Object.assign(Object.assign({},this.params),{log_level:this.logLevel})),this.reconnectAfterMs=(d=t==null?void 0:t.reconnectAfterMs)!==null&&d!==void 0?d:h=>ad[h-1]||id,this.vsn){case Jl:this.encode=(c=t==null?void 0:t.encode)!==null&&c!==void 0?c:(h,f)=>f(JSON.stringify(h)),this.decode=(u=t==null?void 0:t.decode)!==null&&u!==void 0?u:(h,f)=>f(JSON.parse(h));break;case To:this.encode=(p=t==null?void 0:t.encode)!==null&&p!==void 0?p:this.serializer.encode.bind(this.serializer),this.decode=(m=t==null?void 0:t.decode)!==null&&m!==void 0?m:this.serializer.decode.bind(this.serializer);break;default:throw new Error(`Unsupported serializer version: ${this.vsn}`)}if(this.worker){if(typeof window<"u"&&!window.Worker)throw new Error("Web Worker is not supported");this.workerUrl=t==null?void 0:t.workerUrl}}}var Vr=class extends Error{constructor(e,t){var r;super(e),this.name="IcebergError",this.status=t.status,this.icebergType=t.icebergType,this.icebergCode=t.icebergCode,this.details=t.details,this.isCommitStateUnknown=t.icebergType==="CommitStateUnknownException"||[500,502,504].includes(t.status)&&((r=t.icebergType)==null?void 0:r.includes("CommitState"))===!0}isNotFound(){return this.status===404}isConflict(){return this.status===409}isAuthenticationTimeout(){return this.status===419}};function dd(e,t,r){const n=new URL(t,e);if(r)for(const[s,a]of Object.entries(r))a!==void 0&&n.searchParams.set(s,a);return n.toString()}async function cd(e){return!e||e.type==="none"?{}:e.type==="bearer"?{Authorization:`Bearer ${e.token}`}:e.type==="header"?{[e.name]:e.value}:e.type==="custom"?await e.getHeaders():{}}function ud(e){const t=e.fetchImpl??globalThis.fetch;return{async request({method:r,path:n,query:s,body:a,headers:i}){const o=dd(e.baseUrl,n,s),l=await cd(e.auth),d=await t(o,{method:r,headers:{...a?{"Content-Type":"application/json"}:{},...l,...i},body:a?JSON.stringify(a):void 0}),c=await d.text(),u=(d.headers.get("content-type")||"").includes("application/json"),p=u&&c?JSON.parse(c):c;if(!d.ok){const m=u?p:void 0,h=m==null?void 0:m.error;throw new Vr((h==null?void 0:h.message)??`Request failed with status ${d.status}`,{status:d.status,icebergType:h==null?void 0:h.type,icebergCode:h==null?void 0:h.code,details:m})}return{status:d.status,headers:d.headers,data:p}}}}function wn(e){return e.join("")}var pd=class{constructor(e,t=""){this.client=e,this.prefix=t}async listNamespaces(e){const t=e?{parent:wn(e.namespace)}:void 0;return(await this.client.request({method:"GET",path:`${this.prefix}/namespaces`,query:t})).data.namespaces.map(n=>({namespace:n}))}async createNamespace(e,t){const r={namespace:e.namespace,properties:t==null?void 0:t.properties};return(await this.client.request({method:"POST",path:`${this.prefix}/namespaces`,body:r})).data}async dropNamespace(e){await this.client.request({method:"DELETE",path:`${this.prefix}/namespaces/${wn(e.namespace)}`})}async loadNamespaceMetadata(e){return{properties:(await this.client.request({method:"GET",path:`${this.prefix}/namespaces/${wn(e.namespace)}`})).data.properties}}async namespaceExists(e){try{return await this.client.request({method:"HEAD",path:`${this.prefix}/namespaces/${wn(e.namespace)}`}),!0}catch(t){if(t instanceof Vr&&t.status===404)return!1;throw t}}async createNamespaceIfNotExists(e,t){try{return await this.createNamespace(e,t)}catch(r){if(r instanceof Vr&&r.status===409)return;throw r}}};function Kt(e){return e.join("")}var hd=class{constructor(e,t="",r){this.client=e,this.prefix=t,this.accessDelegation=r}async listTables(e){return(await this.client.request({method:"GET",path:`${this.prefix}/namespaces/${Kt(e.namespace)}/tables`})).data.identifiers}async createTable(e,t){const r={};return this.accessDelegation&&(r["X-Iceberg-Access-Delegation"]=this.accessDelegation),(await this.client.request({method:"POST",path:`${this.prefix}/namespaces/${Kt(e.namespace)}/tables`,body:t,headers:r})).data.metadata}async updateTable(e,t){const r=await this.client.request({method:"POST",path:`${this.prefix}/namespaces/${Kt(e.namespace)}/tables/${e.name}`,body:t});return{"metadata-location":r.data["metadata-location"],metadata:r.data.metadata}}async dropTable(e,t){await this.client.request({method:"DELETE",path:`${this.prefix}/namespaces/${Kt(e.namespace)}/tables/${e.name}`,query:{purgeRequested:String((t==null?void 0:t.purge)??!1)}})}async loadTable(e){const t={};return this.accessDelegation&&(t["X-Iceberg-Access-Delegation"]=this.accessDelegation),(await this.client.request({method:"GET",path:`${this.prefix}/namespaces/${Kt(e.namespace)}/tables/${e.name}`,headers:t})).data.metadata}async tableExists(e){const t={};this.accessDelegation&&(t["X-Iceberg-Access-Delegation"]=this.accessDelegation);try{return await this.client.request({method:"HEAD",path:`${this.prefix}/namespaces/${Kt(e.namespace)}/tables/${e.name}`,headers:t}),!0}catch(r){if(r instanceof Vr&&r.status===404)return!1;throw r}}async createTableIfNotExists(e,t){try{return await this.createTable(e,t)}catch(r){if(r instanceof Vr&&r.status===409)return await this.loadTable({namespace:e.namespace,name:t.name});throw r}}},md=class{constructor(e){var n;let t="v1";e.catalogName&&(t+=`/${e.catalogName}`);const r=e.baseUrl.endsWith("/")?e.baseUrl:`${e.baseUrl}/`;this.client=ud({baseUrl:r,auth:e.auth,fetchImpl:e.fetch}),this.accessDelegation=(n=e.accessDelegation)==null?void 0:n.join(","),this.namespaceOps=new pd(this.client,t),this.tableOps=new hd(this.client,t,this.accessDelegation)}async listNamespaces(e){return this.namespaceOps.listNamespaces(e)}async createNamespace(e,t){return this.namespaceOps.createNamespace(e,t)}async dropNamespace(e){await this.namespaceOps.dropNamespace(e)}async loadNamespaceMetadata(e){return this.namespaceOps.loadNamespaceMetadata(e)}async listTables(e){return this.tableOps.listTables(e)}async createTable(e,t){return this.tableOps.createTable(e,t)}async updateTable(e,t){return this.tableOps.updateTable(e,t)}async dropTable(e,t){await this.tableOps.dropTable(e,t)}async loadTable(e){return this.tableOps.loadTable(e)}async namespaceExists(e){return this.namespaceOps.namespaceExists(e)}async tableExists(e){return this.tableOps.tableExists(e)}async createNamespaceIfNotExists(e,t){return this.namespaceOps.createNamespaceIfNotExists(e,t)}async createTableIfNotExists(e,t){return this.tableOps.createTableIfNotExists(e,t)}},ns=class extends Error{constructor(e,t="storage",r,n){super(e),this.__isStorageError=!0,this.namespace=t,this.name=t==="vectors"?"StorageVectorsError":"StorageError",this.status=r,this.statusCode=n}};function ss(e){return typeof e=="object"&&e!==null&&"__isStorageError"in e}var Sn=class extends ns{constructor(e,t,r,n="storage"){super(e,n,t,r),this.name=n==="vectors"?"StorageVectorsApiError":"StorageApiError",this.status=t,this.statusCode=r}toJSON(){return{name:this.name,message:this.message,status:this.status,statusCode:this.statusCode}}},Co=class extends ns{constructor(e,t,r="storage"){super(e,r),this.name=r==="vectors"?"StorageVectorsUnknownError":"StorageUnknownError",this.originalError=t}};const fd=e=>e?(...t)=>e(...t):(...t)=>fetch(...t),yd=e=>{if(typeof e!="object"||e===null)return!1;const t=Object.getPrototypeOf(e);return(t===null||t===Object.prototype||Object.getPrototypeOf(t)===null)&&!(Symbol.toStringTag in e)&&!(Symbol.iterator in e)},js=e=>{if(Array.isArray(e))return e.map(r=>js(r));if(typeof e=="function"||e!==Object(e))return e;const t={};return Object.entries(e).forEach(([r,n])=>{const s=r.replace(/([-_][a-z])/gi,a=>a.toUpperCase().replace(/[-_]/g,""));t[s]=js(n)}),t},bd=e=>!e||typeof e!="string"||e.length===0||e.length>100||e.trim()!==e||e.includes("/")||e.includes("\\")?!1:/^[\w!.\*'() &$@=;:+,?-]+$/.test(e);function Gr(e){"@babel/helpers - typeof";return Gr=typeof Symbol=="function"&&typeof Symbol.iterator=="symbol"?function(t){return typeof t}:function(t){return t&&typeof Symbol=="function"&&t.constructor===Symbol&&t!==Symbol.prototype?"symbol":typeof t},Gr(e)}function vd(e,t){if(Gr(e)!="object"||!e)return e;var r=e[Symbol.toPrimitive];if(r!==void 0){var n=r.call(e,t);if(Gr(n)!="object")return n;throw new TypeError("@@toPrimitive must return a primitive value.")}return(t==="string"?String:Number)(e)}function gd(e){var t=vd(e,"string");return Gr(t)=="symbol"?t:t+""}function _d(e,t,r){return(t=gd(t))in e?Object.defineProperty(e,t,{value:r,enumerable:!0,configurable:!0,writable:!0}):e[t]=r,e}function ni(e,t){var r=Object.keys(e);if(Object.getOwnPropertySymbols){var n=Object.getOwnPropertySymbols(e);t&&(n=n.filter(function(s){return Object.getOwnPropertyDescriptor(e,s).enumerable})),r.push.apply(r,n)}return r}function Z(e){for(var t=1;t<arguments.length;t++){var r=arguments[t]!=null?arguments[t]:{};t%2?ni(Object(r),!0).forEach(function(n){_d(e,n,r[n])}):Object.getOwnPropertyDescriptors?Object.defineProperties(e,Object.getOwnPropertyDescriptors(r)):ni(Object(r)).forEach(function(n){Object.defineProperty(e,n,Object.getOwnPropertyDescriptor(r,n))})}return e}const si=e=>{var t;return e.msg||e.message||e.error_description||(typeof e.error=="string"?e.error:(t=e.error)===null||t===void 0?void 0:t.message)||JSON.stringify(e)},wd=async(e,t,r,n)=>{if(e&&typeof e=="object"&&"status"in e&&"ok"in e&&typeof e.status=="number"&&!(r!=null&&r.noResolveJson)){const s=e,a=s.status||500;if(typeof s.json=="function")s.json().then(i=>{const o=(i==null?void 0:i.statusCode)||(i==null?void 0:i.code)||a+"";t(new Sn(si(i),a,o,n))}).catch(()=>{if(n==="vectors"){const i=a+"";t(new Sn(s.statusText||`HTTP ${a} error`,a,i,n))}else{const i=a+"";t(new Sn(s.statusText||`HTTP ${a} error`,a,i,n))}});else{const i=a+"";t(new Sn(s.statusText||`HTTP ${a} error`,a,i,n))}}else t(new Co(si(e),e,n))},Sd=(e,t,r,n)=>{const s={method:e,headers:(t==null?void 0:t.headers)||{}};return e==="GET"||e==="HEAD"||!n?Z(Z({},s),r):(yd(n)?(s.headers=Z({"Content-Type":"application/json"},t==null?void 0:t.headers),s.body=JSON.stringify(n)):s.body=n,t!=null&&t.duplex&&(s.duplex=t.duplex),Z(Z({},s),r))};async function xr(e,t,r,n,s,a,i){return new Promise((o,l)=>{e(r,Sd(t,n,s,a)).then(d=>{if(!d.ok)throw d;if(n!=null&&n.noResolveJson)return d;if(i==="vectors"){const c=d.headers.get("content-type");if(d.headers.get("content-length")==="0"||d.status===204)return{};if(!c||!c.includes("application/json"))return{}}return d.json()}).then(d=>o(d)).catch(d=>wd(d,l,n,i))})}function Ro(e="storage"){return{get:async(t,r,n,s)=>xr(t,"GET",r,n,s,void 0,e),post:async(t,r,n,s,a)=>xr(t,"POST",r,s,a,n,e),put:async(t,r,n,s,a)=>xr(t,"PUT",r,s,a,n,e),head:async(t,r,n,s)=>xr(t,"HEAD",r,Z(Z({},n),{},{noResolveJson:!0}),s,void 0,e),remove:async(t,r,n,s,a)=>xr(t,"DELETE",r,s,a,n,e)}}const kd=Ro("storage"),{get:Jr,post:ze,put:Ns,head:xd,remove:ga}=kd,Me=Ro("vectors");var Sr=class{constructor(e,t={},r,n="storage"){this.shouldThrowOnError=!1,this.url=e,this.headers=t,this.fetch=fd(r),this.namespace=n}throwOnError(){return this.shouldThrowOnError=!0,this}async handleOperation(e){var t=this;try{return{data:await e(),error:null}}catch(r){if(t.shouldThrowOnError)throw r;if(ss(r))return{data:null,error:r};throw r}}},qd=class{constructor(e,t){this.downloadFn=e,this.shouldThrowOnError=t}then(e,t){return this.execute().then(e,t)}async execute(){var e=this;try{return{data:(await e.downloadFn()).body,error:null}}catch(t){if(e.shouldThrowOnError)throw t;if(ss(t))return{data:null,error:t};throw t}}};let Io;Io=Symbol.toStringTag;var Ld=class{constructor(e,t){this.downloadFn=e,this.shouldThrowOnError=t,this[Io]="BlobDownloadBuilder",this.promise=null}asStream(){return new qd(this.downloadFn,this.shouldThrowOnError)}then(e,t){return this.getPromise().then(e,t)}catch(e){return this.getPromise().catch(e)}finally(e){return this.getPromise().finally(e)}getPromise(){return this.promise||(this.promise=this.execute()),this.promise}async execute(){var e=this;try{return{data:await(await e.downloadFn()).blob(),error:null}}catch(t){if(e.shouldThrowOnError)throw t;if(ss(t))return{data:null,error:t};throw t}}};const Td={limit:100,offset:0,sortBy:{column:"name",order:"asc"}},ai={cacheControl:"3600",contentType:"text/plain;charset=UTF-8",upsert:!1};var Ed=class extends Sr{constructor(e,t={},r,n){super(e,t,n,"storage"),this.bucketId=r}async uploadOrUpdate(e,t,r,n){var s=this;return s.handleOperation(async()=>{let a;const i=Z(Z({},ai),n);let o=Z(Z({},s.headers),e==="POST"&&{"x-upsert":String(i.upsert)});const l=i.metadata;typeof Blob<"u"&&r instanceof Blob?(a=new FormData,a.append("cacheControl",i.cacheControl),l&&a.append("metadata",s.encodeMetadata(l)),a.append("",r)):typeof FormData<"u"&&r instanceof FormData?(a=r,a.has("cacheControl")||a.append("cacheControl",i.cacheControl),l&&!a.has("metadata")&&a.append("metadata",s.encodeMetadata(l))):(a=r,o["cache-control"]=`max-age=${i.cacheControl}`,o["content-type"]=i.contentType,l&&(o["x-metadata"]=s.toBase64(s.encodeMetadata(l))),(typeof ReadableStream<"u"&&a instanceof ReadableStream||a&&typeof a=="object"&&"pipe"in a&&typeof a.pipe=="function")&&!i.duplex&&(i.duplex="half")),n!=null&&n.headers&&(o=Z(Z({},o),n.headers));const d=s._removeEmptyFolders(t),c=s._getFinalPath(d),u=await(e=="PUT"?Ns:ze)(s.fetch,`${s.url}/object/${c}`,a,Z({headers:o},i!=null&&i.duplex?{duplex:i.duplex}:{}));return{path:d,id:u.Id,fullPath:u.Key}})}async upload(e,t,r){return this.uploadOrUpdate("POST",e,t,r)}async uploadToSignedUrl(e,t,r,n){var s=this;const a=s._removeEmptyFolders(e),i=s._getFinalPath(a),o=new URL(s.url+`/object/upload/sign/${i}`);return o.searchParams.set("token",t),s.handleOperation(async()=>{let l;const d=Z({upsert:ai.upsert},n),c=Z(Z({},s.headers),{"x-upsert":String(d.upsert)});return typeof Blob<"u"&&r instanceof Blob?(l=new FormData,l.append("cacheControl",d.cacheControl),l.append("",r)):typeof FormData<"u"&&r instanceof FormData?(l=r,l.append("cacheControl",d.cacheControl)):(l=r,c["cache-control"]=`max-age=${d.cacheControl}`,c["content-type"]=d.contentType),{path:a,fullPath:(await Ns(s.fetch,o.toString(),l,{headers:c})).Key}})}async createSignedUploadUrl(e,t){var r=this;return r.handleOperation(async()=>{let n=r._getFinalPath(e);const s=Z({},r.headers);t!=null&&t.upsert&&(s["x-upsert"]="true");const a=await ze(r.fetch,`${r.url}/object/upload/sign/${n}`,{},{headers:s}),i=new URL(r.url+a.url),o=i.searchParams.get("token");if(!o)throw new ns("No token returned by API");return{signedUrl:i.toString(),path:e,token:o}})}async update(e,t,r){return this.uploadOrUpdate("PUT",e,t,r)}async move(e,t,r){var n=this;return n.handleOperation(async()=>await ze(n.fetch,`${n.url}/object/move`,{bucketId:n.bucketId,sourceKey:e,destinationKey:t,destinationBucket:r==null?void 0:r.destinationBucket},{headers:n.headers}))}async copy(e,t,r){var n=this;return n.handleOperation(async()=>({path:(await ze(n.fetch,`${n.url}/object/copy`,{bucketId:n.bucketId,sourceKey:e,destinationKey:t,destinationBucket:r==null?void 0:r.destinationBucket},{headers:n.headers})).Key}))}async createSignedUrl(e,t,r){var n=this;return n.handleOperation(async()=>{let s=n._getFinalPath(e),a=await ze(n.fetch,`${n.url}/object/sign/${s}`,Z({expiresIn:t},r!=null&&r.transform?{transform:r.transform}:{}),{headers:n.headers});const i=r!=null&&r.download?`&download=${r.download===!0?"":r.download}`:"";return{signedUrl:encodeURI(`${n.url}${a.signedURL}${i}`)}})}async createSignedUrls(e,t,r){var n=this;return n.handleOperation(async()=>{const s=await ze(n.fetch,`${n.url}/object/sign/${n.bucketId}`,{expiresIn:t,paths:e},{headers:n.headers}),a=r!=null&&r.download?`&download=${r.download===!0?"":r.download}`:"";return s.map(i=>Z(Z({},i),{},{signedUrl:i.signedURL?encodeURI(`${n.url}${i.signedURL}${a}`):null}))})}download(e,t,r){const n=typeof(t==null?void 0:t.transform)<"u"?"render/image/authenticated":"object",s=this.transformOptsToQueryString((t==null?void 0:t.transform)||{}),a=s?`?${s}`:"",i=this._getFinalPath(e),o=()=>Jr(this.fetch,`${this.url}/${n}/${i}${a}`,{headers:this.headers,noResolveJson:!0},r);return new Ld(o,this.shouldThrowOnError)}async info(e){var t=this;const r=t._getFinalPath(e);return t.handleOperation(async()=>js(await Jr(t.fetch,`${t.url}/object/info/${r}`,{headers:t.headers})))}async exists(e){var t=this;const r=t._getFinalPath(e);try{return await xd(t.fetch,`${t.url}/object/${r}`,{headers:t.headers}),{data:!0,error:null}}catch(n){if(t.shouldThrowOnError)throw n;if(ss(n)&&n instanceof Co){const s=n.originalError;if([400,404].includes(s==null?void 0:s.status))return{data:!1,error:n}}throw n}}getPublicUrl(e,t){const r=this._getFinalPath(e),n=[],s=t!=null&&t.download?`download=${t.download===!0?"":t.download}`:"";s!==""&&n.push(s);const a=typeof(t==null?void 0:t.transform)<"u"?"render/image":"object",i=this.transformOptsToQueryString((t==null?void 0:t.transform)||{});i!==""&&n.push(i);let o=n.join("&");return o!==""&&(o=`?${o}`),{data:{publicUrl:encodeURI(`${this.url}/${a}/public/${r}${o}`)}}}async remove(e){var t=this;return t.handleOperation(async()=>await ga(t.fetch,`${t.url}/object/${t.bucketId}`,{prefixes:e},{headers:t.headers}))}async list(e,t,r){var n=this;return n.handleOperation(async()=>{const s=Z(Z(Z({},Td),t),{},{prefix:e||""});return await ze(n.fetch,`${n.url}/object/list/${n.bucketId}`,s,{headers:n.headers},r)})}async listV2(e,t){var r=this;return r.handleOperation(async()=>{const n=Z({},e);return await ze(r.fetch,`${r.url}/object/list-v2/${r.bucketId}`,n,{headers:r.headers},t)})}encodeMetadata(e){return JSON.stringify(e)}toBase64(e){return typeof Buffer<"u"?Buffer.from(e).toString("base64"):btoa(e)}_getFinalPath(e){return`${this.bucketId}/${e.replace(/^\/+/,"")}`}_removeEmptyFolders(e){return e.replace(/^\/|\/$/g,"").replace(/\/+/g,"/")}transformOptsToQueryString(e){const t=[];return e.width&&t.push(`width=${e.width}`),e.height&&t.push(`height=${e.height}`),e.resize&&t.push(`resize=${e.resize}`),e.format&&t.push(`format=${e.format}`),e.quality&&t.push(`quality=${e.quality}`),t.join("&")}};const $d="2.95.3",pn={"X-Client-Info":`storage-js/${$d}`};var Ad=class extends Sr{constructor(e,t={},r,n){const s=new URL(e);n!=null&&n.useNewHostname&&/supabase\.(co|in|red)$/.test(s.hostname)&&!s.hostname.includes("storage.supabase.")&&(s.hostname=s.hostname.replace("supabase.","storage.supabase."));const a=s.href.replace(/\/$/,""),i=Z(Z({},pn),t);super(a,i,r,"storage")}async listBuckets(e){var t=this;return t.handleOperation(async()=>{const r=t.listBucketOptionsToQueryString(e);return await Jr(t.fetch,`${t.url}/bucket${r}`,{headers:t.headers})})}async getBucket(e){var t=this;return t.handleOperation(async()=>await Jr(t.fetch,`${t.url}/bucket/${e}`,{headers:t.headers}))}async createBucket(e,t={public:!1}){var r=this;return r.handleOperation(async()=>await ze(r.fetch,`${r.url}/bucket`,{id:e,name:e,type:t.type,public:t.public,file_size_limit:t.fileSizeLimit,allowed_mime_types:t.allowedMimeTypes},{headers:r.headers}))}async updateBucket(e,t){var r=this;return r.handleOperation(async()=>await Ns(r.fetch,`${r.url}/bucket/${e}`,{id:e,name:e,public:t.public,file_size_limit:t.fileSizeLimit,allowed_mime_types:t.allowedMimeTypes},{headers:r.headers}))}async emptyBucket(e){var t=this;return t.handleOperation(async()=>await ze(t.fetch,`${t.url}/bucket/${e}/empty`,{},{headers:t.headers}))}async deleteBucket(e){var t=this;return t.handleOperation(async()=>await ga(t.fetch,`${t.url}/bucket/${e}`,{},{headers:t.headers}))}listBucketOptionsToQueryString(e){const t={};return e&&("limit"in e&&(t.limit=String(e.limit)),"offset"in e&&(t.offset=String(e.offset)),e.search&&(t.search=e.search),e.sortColumn&&(t.sortColumn=e.sortColumn),e.sortOrder&&(t.sortOrder=e.sortOrder)),Object.keys(t).length>0?"?"+new URLSearchParams(t).toString():""}},Cd=class extends Sr{constructor(e,t={},r){const n=e.replace(/\/$/,""),s=Z(Z({},pn),t);super(n,s,r,"storage")}async createBucket(e){var t=this;return t.handleOperation(async()=>await ze(t.fetch,`${t.url}/bucket`,{name:e},{headers:t.headers}))}async listBuckets(e){var t=this;return t.handleOperation(async()=>{const r=new URLSearchParams;(e==null?void 0:e.limit)!==void 0&&r.set("limit",e.limit.toString()),(e==null?void 0:e.offset)!==void 0&&r.set("offset",e.offset.toString()),e!=null&&e.sortColumn&&r.set("sortColumn",e.sortColumn),e!=null&&e.sortOrder&&r.set("sortOrder",e.sortOrder),e!=null&&e.search&&r.set("search",e.search);const n=r.toString(),s=n?`${t.url}/bucket?${n}`:`${t.url}/bucket`;return await Jr(t.fetch,s,{headers:t.headers})})}async deleteBucket(e){var t=this;return t.handleOperation(async()=>await ga(t.fetch,`${t.url}/bucket/${e}`,{},{headers:t.headers}))}from(e){var t=this;if(!bd(e))throw new ns("Invalid bucket name: File, folder, and bucket names must follow AWS object key naming guidelines and should avoid the use of any other characters.");const r=new md({baseUrl:this.url,catalogName:e,auth:{type:"custom",getHeaders:async()=>t.headers},fetch:this.fetch}),n=this.shouldThrowOnError;return new Proxy(r,{get(s,a){const i=s[a];return typeof i!="function"?i:async(...o)=>{try{return{data:await i.apply(s,o),error:null}}catch(l){if(n)throw l;return{data:null,error:l}}}}})}},Rd=class extends Sr{constructor(e,t={},r){const n=e.replace(/\/$/,""),s=Z(Z({},pn),{},{"Content-Type":"application/json"},t);super(n,s,r,"vectors")}async createIndex(e){var t=this;return t.handleOperation(async()=>await Me.post(t.fetch,`${t.url}/CreateIndex`,e,{headers:t.headers})||{})}async getIndex(e,t){var r=this;return r.handleOperation(async()=>await Me.post(r.fetch,`${r.url}/GetIndex`,{vectorBucketName:e,indexName:t},{headers:r.headers}))}async listIndexes(e){var t=this;return t.handleOperation(async()=>await Me.post(t.fetch,`${t.url}/ListIndexes`,e,{headers:t.headers}))}async deleteIndex(e,t){var r=this;return r.handleOperation(async()=>await Me.post(r.fetch,`${r.url}/DeleteIndex`,{vectorBucketName:e,indexName:t},{headers:r.headers})||{})}},Id=class extends Sr{constructor(e,t={},r){const n=e.replace(/\/$/,""),s=Z(Z({},pn),{},{"Content-Type":"application/json"},t);super(n,s,r,"vectors")}async putVectors(e){var t=this;if(e.vectors.length<1||e.vectors.length>500)throw new Error("Vector batch size must be between 1 and 500 items");return t.handleOperation(async()=>await Me.post(t.fetch,`${t.url}/PutVectors`,e,{headers:t.headers})||{})}async getVectors(e){var t=this;return t.handleOperation(async()=>await Me.post(t.fetch,`${t.url}/GetVectors`,e,{headers:t.headers}))}async listVectors(e){var t=this;if(e.segmentCount!==void 0){if(e.segmentCount<1||e.segmentCount>16)throw new Error("segmentCount must be between 1 and 16");if(e.segmentIndex!==void 0&&(e.segmentIndex<0||e.segmentIndex>=e.segmentCount))throw new Error(`segmentIndex must be between 0 and ${e.segmentCount-1}`)}return t.handleOperation(async()=>await Me.post(t.fetch,`${t.url}/ListVectors`,e,{headers:t.headers}))}async queryVectors(e){var t=this;return t.handleOperation(async()=>await Me.post(t.fetch,`${t.url}/QueryVectors`,e,{headers:t.headers}))}async deleteVectors(e){var t=this;if(e.keys.length<1||e.keys.length>500)throw new Error("Keys batch size must be between 1 and 500 items");return t.handleOperation(async()=>await Me.post(t.fetch,`${t.url}/DeleteVectors`,e,{headers:t.headers})||{})}},Dd=class extends Sr{constructor(e,t={},r){const n=e.replace(/\/$/,""),s=Z(Z({},pn),{},{"Content-Type":"application/json"},t);super(n,s,r,"vectors")}async createBucket(e){var t=this;return t.handleOperation(async()=>await Me.post(t.fetch,`${t.url}/CreateVectorBucket`,{vectorBucketName:e},{headers:t.headers})||{})}async getBucket(e){var t=this;return t.handleOperation(async()=>await Me.post(t.fetch,`${t.url}/GetVectorBucket`,{vectorBucketName:e},{headers:t.headers}))}async listBuckets(e={}){var t=this;return t.handleOperation(async()=>await Me.post(t.fetch,`${t.url}/ListVectorBuckets`,e,{headers:t.headers}))}async deleteBucket(e){var t=this;return t.handleOperation(async()=>await Me.post(t.fetch,`${t.url}/DeleteVectorBucket`,{vectorBucketName:e},{headers:t.headers})||{})}},Od=class extends Dd{constructor(e,t={}){super(e,t.headers||{},t.fetch)}from(e){return new Pd(this.url,this.headers,e,this.fetch)}async createBucket(e){var t=()=>super.createBucket,r=this;return t().call(r,e)}async getBucket(e){var t=()=>super.getBucket,r=this;return t().call(r,e)}async listBuckets(e={}){var t=()=>super.listBuckets,r=this;return t().call(r,e)}async deleteBucket(e){var t=()=>super.deleteBucket,r=this;return t().call(r,e)}},Pd=class extends Rd{constructor(e,t,r,n){super(e,t,n),this.vectorBucketName=r}async createIndex(e){var t=()=>super.createIndex,r=this;return t().call(r,Z(Z({},e),{},{vectorBucketName:r.vectorBucketName}))}async listIndexes(e={}){var t=()=>super.listIndexes,r=this;return t().call(r,Z(Z({},e),{},{vectorBucketName:r.vectorBucketName}))}async getIndex(e){var t=()=>super.getIndex,r=this;return t().call(r,r.vectorBucketName,e)}async deleteIndex(e){var t=()=>super.deleteIndex,r=this;return t().call(r,r.vectorBucketName,e)}index(e){return new Md(this.url,this.headers,this.vectorBucketName,e,this.fetch)}},Md=class extends Id{constructor(e,t,r,n,s){super(e,t,s),this.vectorBucketName=r,this.indexName=n}async putVectors(e){var t=()=>super.putVectors,r=this;return t().call(r,Z(Z({},e),{},{vectorBucketName:r.vectorBucketName,indexName:r.indexName}))}async getVectors(e){var t=()=>super.getVectors,r=this;return t().call(r,Z(Z({},e),{},{vectorBucketName:r.vectorBucketName,indexName:r.indexName}))}async listVectors(e={}){var t=()=>super.listVectors,r=this;return t().call(r,Z(Z({},e),{},{vectorBucketName:r.vectorBucketName,indexName:r.indexName}))}async queryVectors(e){var t=()=>super.queryVectors,r=this;return t().call(r,Z(Z({},e),{},{vectorBucketName:r.vectorBucketName,indexName:r.indexName}))}async deleteVectors(e){var t=()=>super.deleteVectors,r=this;return t().call(r,Z(Z({},e),{},{vectorBucketName:r.vectorBucketName,indexName:r.indexName}))}},jd=class extends Ad{constructor(e,t={},r,n){super(e,t,r,n)}from(e){return new Ed(this.url,this.headers,e,this.fetch)}get vectors(){return new Od(this.url+"/vector",{headers:this.headers,fetch:this.fetch})}get analytics(){return new Cd(this.url+"/iceberg",this.headers,this.fetch)}};const Do="2.95.3",er=30*1e3,Hs=3,ys=Hs*er,Nd="http://localhost:9999",Hd="supabase.auth.token",Ud={"X-Client-Info":`gotrue-js/${Do}`},Us="X-Supabase-Api-Version",Oo={"2024-01-01":{timestamp:Date.parse("2024-01-01T00:00:00.0Z"),name:"2024-01-01"}},Fd=/^([a-z0-9_-]{4})*($|[a-z0-9_-]{3}$|[a-z0-9_-]{2}$)$/i,Bd=10*60*1e3;class Qr extends Error{constructor(t,r,n){super(t),this.__isAuthError=!0,this.name="AuthError",this.status=r,this.code=n}}function G(e){return typeof e=="object"&&e!==null&&"__isAuthError"in e}class Kd extends Qr{constructor(t,r,n){super(t,r,n),this.name="AuthApiError",this.status=r,this.code=n}}function zd(e){return G(e)&&e.name==="AuthApiError"}class At extends Qr{constructor(t,r){super(t),this.name="AuthUnknownError",this.originalError=r}}class at extends Qr{constructor(t,r,n,s){super(t,n,s),this.name=r,this.status=n}}class De extends at{constructor(){super("Auth session missing!","AuthSessionMissingError",400,void 0)}}function bs(e){return G(e)&&e.name==="AuthSessionMissingError"}class zt extends at{constructor(){super("Auth session or user missing","AuthInvalidTokenResponseError",500,void 0)}}class kn extends at{constructor(t){super(t,"AuthInvalidCredentialsError",400,void 0)}}class xn extends at{constructor(t,r=null){super(t,"AuthImplicitGrantRedirectError",500,void 0),this.details=null,this.details=r}toJSON(){return{name:this.name,message:this.message,status:this.status,details:this.details}}}function Wd(e){return G(e)&&e.name==="AuthImplicitGrantRedirectError"}class ii extends at{constructor(t,r=null){super(t,"AuthPKCEGrantCodeExchangeError",500,void 0),this.details=null,this.details=r}toJSON(){return{name:this.name,message:this.message,status:this.status,details:this.details}}}class Vd extends at{constructor(){super("PKCE code verifier not found in storage. This can happen if the auth flow was initiated in a different browser or device, or if the storage was cleared. For SSR frameworks (Next.js, SvelteKit, etc.), use @supabase/ssr on both the server and client to store the code verifier in cookies.","AuthPKCECodeVerifierMissingError",400,"pkce_code_verifier_not_found")}}class Fs extends at{constructor(t,r){super(t,"AuthRetryableFetchError",r,void 0)}}function vs(e){return G(e)&&e.name==="AuthRetryableFetchError"}class oi extends at{constructor(t,r,n){super(t,"AuthWeakPasswordError",r,"weak_password"),this.reasons=n}}class Bs extends at{constructor(t){super(t,"AuthInvalidJwtError",400,"invalid_jwt")}}const Kn="ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789-_".split(""),li=` 	
\r=`.split(""),Gd=(()=>{const e=new Array(128);for(let t=0;t<e.length;t+=1)e[t]=-1;for(let t=0;t<li.length;t+=1)e[li[t].charCodeAt(0)]=-2;for(let t=0;t<Kn.length;t+=1)e[Kn[t].charCodeAt(0)]=t;return e})();function di(e,t,r){if(e!==null)for(t.queue=t.queue<<8|e,t.queuedBits+=8;t.queuedBits>=6;){const n=t.queue>>t.queuedBits-6&63;r(Kn[n]),t.queuedBits-=6}else if(t.queuedBits>0)for(t.queue=t.queue<<6-t.queuedBits,t.queuedBits=6;t.queuedBits>=6;){const n=t.queue>>t.queuedBits-6&63;r(Kn[n]),t.queuedBits-=6}}function Po(e,t,r){const n=Gd[e];if(n>-1)for(t.queue=t.queue<<6|n,t.queuedBits+=6;t.queuedBits>=8;)r(t.queue>>t.queuedBits-8&255),t.queuedBits-=8;else{if(n===-2)return;throw new Error(`Invalid Base64-URL character "${String.fromCharCode(e)}"`)}}function ci(e){const t=[],r=i=>{t.push(String.fromCodePoint(i))},n={utf8seq:0,codepoint:0},s={queue:0,queuedBits:0},a=i=>{Yd(i,n,r)};for(let i=0;i<e.length;i+=1)Po(e.charCodeAt(i),s,a);return t.join("")}function Jd(e,t){if(e<=127){t(e);return}else if(e<=2047){t(192|e>>6),t(128|e&63);return}else if(e<=65535){t(224|e>>12),t(128|e>>6&63),t(128|e&63);return}else if(e<=1114111){t(240|e>>18),t(128|e>>12&63),t(128|e>>6&63),t(128|e&63);return}throw new Error(`Unrecognized Unicode codepoint: ${e.toString(16)}`)}function Qd(e,t){for(let r=0;r<e.length;r+=1){let n=e.charCodeAt(r);if(n>55295&&n<=56319){const s=(n-55296)*1024&65535;n=(e.charCodeAt(r+1)-56320&65535|s)+65536,r+=1}Jd(n,t)}}function Yd(e,t,r){if(t.utf8seq===0){if(e<=127){r(e);return}for(let n=1;n<6;n+=1)if(!(e>>7-n&1)){t.utf8seq=n;break}if(t.utf8seq===2)t.codepoint=e&31;else if(t.utf8seq===3)t.codepoint=e&15;else if(t.utf8seq===4)t.codepoint=e&7;else throw new Error("Invalid UTF-8 sequence");t.utf8seq-=1}else if(t.utf8seq>0){if(e<=127)throw new Error("Invalid UTF-8 sequence");t.codepoint=t.codepoint<<6|e&63,t.utf8seq-=1,t.utf8seq===0&&r(t.codepoint)}}function mr(e){const t=[],r={queue:0,queuedBits:0},n=s=>{t.push(s)};for(let s=0;s<e.length;s+=1)Po(e.charCodeAt(s),r,n);return new Uint8Array(t)}function Xd(e){const t=[];return Qd(e,r=>t.push(r)),new Uint8Array(t)}function Pt(e){const t=[],r={queue:0,queuedBits:0},n=s=>{t.push(s)};return e.forEach(s=>di(s,r,n)),di(null,r,n),t.join("")}function Zd(e){return Math.round(Date.now()/1e3)+e}function ec(){return Symbol("auth-callback")}const xe=()=>typeof window<"u"&&typeof document<"u",kt={tested:!1,writable:!1},Mo=()=>{if(!xe())return!1;try{if(typeof globalThis.localStorage!="object")return!1}catch{return!1}if(kt.tested)return kt.writable;const e=`lswt-${Math.random()}${Math.random()}`;try{globalThis.localStorage.setItem(e,e),globalThis.localStorage.removeItem(e),kt.tested=!0,kt.writable=!0}catch{kt.tested=!0,kt.writable=!1}return kt.writable};function tc(e){const t={},r=new URL(e);if(r.hash&&r.hash[0]==="#")try{new URLSearchParams(r.hash.substring(1)).forEach((s,a)=>{t[a]=s})}catch{}return r.searchParams.forEach((n,s)=>{t[s]=n}),t}const jo=e=>e?(...t)=>e(...t):(...t)=>fetch(...t),rc=e=>typeof e=="object"&&e!==null&&"status"in e&&"ok"in e&&"json"in e&&typeof e.json=="function",tr=async(e,t,r)=>{await e.setItem(t,JSON.stringify(r))},xt=async(e,t)=>{const r=await e.getItem(t);if(!r)return null;try{return JSON.parse(r)}catch{return r}},ke=async(e,t)=>{await e.removeItem(t)};class as{constructor(){this.promise=new as.promiseConstructor((t,r)=>{this.resolve=t,this.reject=r})}}as.promiseConstructor=Promise;function qn(e){const t=e.split(".");if(t.length!==3)throw new Bs("Invalid JWT structure");for(let n=0;n<t.length;n++)if(!Fd.test(t[n]))throw new Bs("JWT not in base64url format");return{header:JSON.parse(ci(t[0])),payload:JSON.parse(ci(t[1])),signature:mr(t[2]),raw:{header:t[0],payload:t[1]}}}async function nc(e){return await new Promise(t=>{setTimeout(()=>t(null),e)})}function sc(e,t){return new Promise((n,s)=>{(async()=>{for(let a=0;a<1/0;a++)try{const i=await e(a);if(!t(a,null,i)){n(i);return}}catch(i){if(!t(a,i)){s(i);return}}})()})}function ac(e){return("0"+e.toString(16)).substr(-2)}function ic(){const t=new Uint32Array(56);if(typeof crypto>"u"){const r="ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789-._~",n=r.length;let s="";for(let a=0;a<56;a++)s+=r.charAt(Math.floor(Math.random()*n));return s}return crypto.getRandomValues(t),Array.from(t,ac).join("")}async function oc(e){const r=new TextEncoder().encode(e),n=await crypto.subtle.digest("SHA-256",r),s=new Uint8Array(n);return Array.from(s).map(a=>String.fromCharCode(a)).join("")}async function lc(e){if(!(typeof crypto<"u"&&typeof crypto.subtle<"u"&&typeof TextEncoder<"u"))return console.warn("WebCrypto API is not supported. Code challenge method will default to use plain instead of sha256."),e;const r=await oc(e);return btoa(r).replace(/\+/g,"-").replace(/\//g,"_").replace(/=+$/,"")}async function Wt(e,t,r=!1){const n=ic();let s=n;r&&(s+="/PASSWORD_RECOVERY"),await tr(e,`${t}-code-verifier`,s);const a=await lc(n);return[a,n===a?"plain":"s256"]}const dc=/^2[0-9]{3}-(0[1-9]|1[0-2])-(0[1-9]|1[0-9]|2[0-9]|3[0-1])$/i;function cc(e){const t=e.headers.get(Us);if(!t||!t.match(dc))return null;try{return new Date(`${t}T00:00:00.0Z`)}catch{return null}}function uc(e){if(!e)throw new Error("Missing exp claim");const t=Math.floor(Date.now()/1e3);if(e<=t)throw new Error("JWT has expired")}function pc(e){switch(e){case"RS256":return{name:"RSASSA-PKCS1-v1_5",hash:{name:"SHA-256"}};case"ES256":return{name:"ECDSA",namedCurve:"P-256",hash:{name:"SHA-256"}};default:throw new Error("Invalid alg claim")}}const hc=/^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/;function Vt(e){if(!hc.test(e))throw new Error("@supabase/auth-js: Expected parameter to be UUID but is not")}function gs(){const e={};return new Proxy(e,{get:(t,r)=>{if(r==="__isUserNotAvailableProxy")return!0;if(typeof r=="symbol"){const n=r.toString();if(n==="Symbol(Symbol.toPrimitive)"||n==="Symbol(Symbol.toStringTag)"||n==="Symbol(util.inspect.custom)")return}throw new Error(`@supabase/auth-js: client was created with userStorage option and there was no user stored in the user storage. Accessing the "${r}" property of the session object is not supported. Please use getUser() instead.`)},set:(t,r)=>{throw new Error(`@supabase/auth-js: client was created with userStorage option and there was no user stored in the user storage. Setting the "${r}" property of the session object is not supported. Please use getUser() to fetch a user object you can manipulate.`)},deleteProperty:(t,r)=>{throw new Error(`@supabase/auth-js: client was created with userStorage option and there was no user stored in the user storage. Deleting the "${r}" property of the session object is not supported. Please use getUser() to fetch a user object you can manipulate.`)}})}function mc(e,t){return new Proxy(e,{get:(r,n,s)=>{if(n==="__isInsecureUserWarningProxy")return!0;if(typeof n=="symbol"){const a=n.toString();if(a==="Symbol(Symbol.toPrimitive)"||a==="Symbol(Symbol.toStringTag)"||a==="Symbol(util.inspect.custom)"||a==="Symbol(nodejs.util.inspect.custom)")return Reflect.get(r,n,s)}return!t.value&&typeof n=="string"&&(console.warn("Using the user object as returned from supabase.auth.getSession() or from some supabase.auth.onAuthStateChange() events could be insecure! This value comes directly from the storage medium (usually cookies on the server) and may not be authentic. Use supabase.auth.getUser() instead which authenticates the data by contacting the Supabase Auth server."),t.value=!0),Reflect.get(r,n,s)}})}function ui(e){return JSON.parse(JSON.stringify(e))}const qt=e=>e.msg||e.message||e.error_description||e.error||JSON.stringify(e),fc=[502,503,504];async function pi(e){var t;if(!rc(e))throw new Fs(qt(e),0);if(fc.includes(e.status))throw new Fs(qt(e),e.status);let r;try{r=await e.json()}catch(a){throw new At(qt(a),a)}let n;const s=cc(e);if(s&&s.getTime()>=Oo["2024-01-01"].timestamp&&typeof r=="object"&&r&&typeof r.code=="string"?n=r.code:typeof r=="object"&&r&&typeof r.error_code=="string"&&(n=r.error_code),n){if(n==="weak_password")throw new oi(qt(r),e.status,((t=r.weak_password)===null||t===void 0?void 0:t.reasons)||[]);if(n==="session_not_found")throw new De}else if(typeof r=="object"&&r&&typeof r.weak_password=="object"&&r.weak_password&&Array.isArray(r.weak_password.reasons)&&r.weak_password.reasons.length&&r.weak_password.reasons.reduce((a,i)=>a&&typeof i=="string",!0))throw new oi(qt(r),e.status,r.weak_password.reasons);throw new Kd(qt(r),e.status||500,n)}const yc=(e,t,r,n)=>{const s={method:e,headers:(t==null?void 0:t.headers)||{}};return e==="GET"?s:(s.headers=Object.assign({"Content-Type":"application/json;charset=UTF-8"},t==null?void 0:t.headers),s.body=JSON.stringify(n),Object.assign(Object.assign({},s),r))};async function X(e,t,r,n){var s;const a=Object.assign({},n==null?void 0:n.headers);a[Us]||(a[Us]=Oo["2024-01-01"].name),n!=null&&n.jwt&&(a.Authorization=`Bearer ${n.jwt}`);const i=(s=n==null?void 0:n.query)!==null&&s!==void 0?s:{};n!=null&&n.redirectTo&&(i.redirect_to=n.redirectTo);const o=Object.keys(i).length?"?"+new URLSearchParams(i).toString():"",l=await bc(e,t,r+o,{headers:a,noResolveJson:n==null?void 0:n.noResolveJson},{},n==null?void 0:n.body);return n!=null&&n.xform?n==null?void 0:n.xform(l):{data:Object.assign({},l),error:null}}async function bc(e,t,r,n,s,a){const i=yc(t,n,s,a);let o;try{o=await e(r,Object.assign({},i))}catch(l){throw console.error(l),new Fs(qt(l),0)}if(o.ok||await pi(o),n!=null&&n.noResolveJson)return o;try{return await o.json()}catch(l){await pi(l)}}function Ke(e){var t;let r=null;_c(e)&&(r=Object.assign({},e),e.expires_at||(r.expires_at=Zd(e.expires_in)));const n=(t=e.user)!==null&&t!==void 0?t:e;return{data:{session:r,user:n},error:null}}function hi(e){const t=Ke(e);return!t.error&&e.weak_password&&typeof e.weak_password=="object"&&Array.isArray(e.weak_password.reasons)&&e.weak_password.reasons.length&&e.weak_password.message&&typeof e.weak_password.message=="string"&&e.weak_password.reasons.reduce((r,n)=>r&&typeof n=="string",!0)&&(t.data.weak_password=e.weak_password),t}function ft(e){var t;return{data:{user:(t=e.user)!==null&&t!==void 0?t:e},error:null}}function vc(e){return{data:e,error:null}}function gc(e){const{action_link:t,email_otp:r,hashed_token:n,redirect_to:s,verification_type:a}=e,i=rs(e,["action_link","email_otp","hashed_token","redirect_to","verification_type"]),o={action_link:t,email_otp:r,hashed_token:n,redirect_to:s,verification_type:a},l=Object.assign({},i);return{data:{properties:o,user:l},error:null}}function mi(e){return e}function _c(e){return e.access_token&&e.refresh_token&&e.expires_in}const _s=["global","local","others"];class wc{constructor({url:t="",headers:r={},fetch:n}){this.url=t,this.headers=r,this.fetch=jo(n),this.mfa={listFactors:this._listFactors.bind(this),deleteFactor:this._deleteFactor.bind(this)},this.oauth={listClients:this._listOAuthClients.bind(this),createClient:this._createOAuthClient.bind(this),getClient:this._getOAuthClient.bind(this),updateClient:this._updateOAuthClient.bind(this),deleteClient:this._deleteOAuthClient.bind(this),regenerateClientSecret:this._regenerateOAuthClientSecret.bind(this)}}async signOut(t,r=_s[0]){if(_s.indexOf(r)<0)throw new Error(`@supabase/auth-js: Parameter scope must be one of ${_s.join(", ")}`);try{return await X(this.fetch,"POST",`${this.url}/logout?scope=${r}`,{headers:this.headers,jwt:t,noResolveJson:!0}),{data:null,error:null}}catch(n){if(G(n))return{data:null,error:n};throw n}}async inviteUserByEmail(t,r={}){try{return await X(this.fetch,"POST",`${this.url}/invite`,{body:{email:t,data:r.data},headers:this.headers,redirectTo:r.redirectTo,xform:ft})}catch(n){if(G(n))return{data:{user:null},error:n};throw n}}async generateLink(t){try{const{options:r}=t,n=rs(t,["options"]),s=Object.assign(Object.assign({},n),r);return"newEmail"in n&&(s.new_email=n==null?void 0:n.newEmail,delete s.newEmail),await X(this.fetch,"POST",`${this.url}/admin/generate_link`,{body:s,headers:this.headers,xform:gc,redirectTo:r==null?void 0:r.redirectTo})}catch(r){if(G(r))return{data:{properties:null,user:null},error:r};throw r}}async createUser(t){try{return await X(this.fetch,"POST",`${this.url}/admin/users`,{body:t,headers:this.headers,xform:ft})}catch(r){if(G(r))return{data:{user:null},error:r};throw r}}async listUsers(t){var r,n,s,a,i,o,l;try{const d={nextPage:null,lastPage:0,total:0},c=await X(this.fetch,"GET",`${this.url}/admin/users`,{headers:this.headers,noResolveJson:!0,query:{page:(n=(r=t==null?void 0:t.page)===null||r===void 0?void 0:r.toString())!==null&&n!==void 0?n:"",per_page:(a=(s=t==null?void 0:t.perPage)===null||s===void 0?void 0:s.toString())!==null&&a!==void 0?a:""},xform:mi});if(c.error)throw c.error;const u=await c.json(),p=(i=c.headers.get("x-total-count"))!==null&&i!==void 0?i:0,m=(l=(o=c.headers.get("link"))===null||o===void 0?void 0:o.split(","))!==null&&l!==void 0?l:[];return m.length>0&&(m.forEach(h=>{const f=parseInt(h.split(";")[0].split("=")[1].substring(0,1)),w=JSON.parse(h.split(";")[1].split("=")[1]);d[`${w}Page`]=f}),d.total=parseInt(p)),{data:Object.assign(Object.assign({},u),d),error:null}}catch(d){if(G(d))return{data:{users:[]},error:d};throw d}}async getUserById(t){Vt(t);try{return await X(this.fetch,"GET",`${this.url}/admin/users/${t}`,{headers:this.headers,xform:ft})}catch(r){if(G(r))return{data:{user:null},error:r};throw r}}async updateUserById(t,r){Vt(t);try{return await X(this.fetch,"PUT",`${this.url}/admin/users/${t}`,{body:r,headers:this.headers,xform:ft})}catch(n){if(G(n))return{data:{user:null},error:n};throw n}}async deleteUser(t,r=!1){Vt(t);try{return await X(this.fetch,"DELETE",`${this.url}/admin/users/${t}`,{headers:this.headers,body:{should_soft_delete:r},xform:ft})}catch(n){if(G(n))return{data:{user:null},error:n};throw n}}async _listFactors(t){Vt(t.userId);try{const{data:r,error:n}=await X(this.fetch,"GET",`${this.url}/admin/users/${t.userId}/factors`,{headers:this.headers,xform:s=>({data:{factors:s},error:null})});return{data:r,error:n}}catch(r){if(G(r))return{data:null,error:r};throw r}}async _deleteFactor(t){Vt(t.userId),Vt(t.id);try{return{data:await X(this.fetch,"DELETE",`${this.url}/admin/users/${t.userId}/factors/${t.id}`,{headers:this.headers}),error:null}}catch(r){if(G(r))return{data:null,error:r};throw r}}async _listOAuthClients(t){var r,n,s,a,i,o,l;try{const d={nextPage:null,lastPage:0,total:0},c=await X(this.fetch,"GET",`${this.url}/admin/oauth/clients`,{headers:this.headers,noResolveJson:!0,query:{page:(n=(r=t==null?void 0:t.page)===null||r===void 0?void 0:r.toString())!==null&&n!==void 0?n:"",per_page:(a=(s=t==null?void 0:t.perPage)===null||s===void 0?void 0:s.toString())!==null&&a!==void 0?a:""},xform:mi});if(c.error)throw c.error;const u=await c.json(),p=(i=c.headers.get("x-total-count"))!==null&&i!==void 0?i:0,m=(l=(o=c.headers.get("link"))===null||o===void 0?void 0:o.split(","))!==null&&l!==void 0?l:[];return m.length>0&&(m.forEach(h=>{const f=parseInt(h.split(";")[0].split("=")[1].substring(0,1)),w=JSON.parse(h.split(";")[1].split("=")[1]);d[`${w}Page`]=f}),d.total=parseInt(p)),{data:Object.assign(Object.assign({},u),d),error:null}}catch(d){if(G(d))return{data:{clients:[]},error:d};throw d}}async _createOAuthClient(t){try{return await X(this.fetch,"POST",`${this.url}/admin/oauth/clients`,{body:t,headers:this.headers,xform:r=>({data:r,error:null})})}catch(r){if(G(r))return{data:null,error:r};throw r}}async _getOAuthClient(t){try{return await X(this.fetch,"GET",`${this.url}/admin/oauth/clients/${t}`,{headers:this.headers,xform:r=>({data:r,error:null})})}catch(r){if(G(r))return{data:null,error:r};throw r}}async _updateOAuthClient(t,r){try{return await X(this.fetch,"PUT",`${this.url}/admin/oauth/clients/${t}`,{body:r,headers:this.headers,xform:n=>({data:n,error:null})})}catch(n){if(G(n))return{data:null,error:n};throw n}}async _deleteOAuthClient(t){try{return await X(this.fetch,"DELETE",`${this.url}/admin/oauth/clients/${t}`,{headers:this.headers,noResolveJson:!0}),{data:null,error:null}}catch(r){if(G(r))return{data:null,error:r};throw r}}async _regenerateOAuthClientSecret(t){try{return await X(this.fetch,"POST",`${this.url}/admin/oauth/clients/${t}/regenerate_secret`,{headers:this.headers,xform:r=>({data:r,error:null})})}catch(r){if(G(r))return{data:null,error:r};throw r}}}function fi(e={}){return{getItem:t=>e[t]||null,setItem:(t,r)=>{e[t]=r},removeItem:t=>{delete e[t]}}}const Gt={debug:!!(globalThis&&Mo()&&globalThis.localStorage&&globalThis.localStorage.getItem("supabase.gotrue-js.locks.debug")==="true")};class No extends Error{constructor(t){super(t),this.isAcquireTimeout=!0}}class Sc extends No{}async function kc(e,t,r){Gt.debug&&console.log("@supabase/gotrue-js: navigatorLock: acquire lock",e,t);const n=new globalThis.AbortController;return t>0&&setTimeout(()=>{n.abort(),Gt.debug&&console.log("@supabase/gotrue-js: navigatorLock acquire timed out",e)},t),await Promise.resolve().then(()=>globalThis.navigator.locks.request(e,t===0?{mode:"exclusive",ifAvailable:!0}:{mode:"exclusive",signal:n.signal},async s=>{if(s){Gt.debug&&console.log("@supabase/gotrue-js: navigatorLock: acquired",e,s.name);try{return await r()}finally{Gt.debug&&console.log("@supabase/gotrue-js: navigatorLock: released",e,s.name)}}else{if(t===0)throw Gt.debug&&console.log("@supabase/gotrue-js: navigatorLock: not immediately available",e),new Sc(`Acquiring an exclusive Navigator LockManager lock "${e}" immediately failed`);if(Gt.debug)try{const a=await globalThis.navigator.locks.query();console.log("@supabase/gotrue-js: Navigator LockManager state",JSON.stringify(a,null,"  "))}catch(a){console.warn("@supabase/gotrue-js: Error when querying Navigator LockManager state",a)}return console.warn("@supabase/gotrue-js: Navigator LockManager returned a null lock when using #request without ifAvailable set to true, it appears this browser is not following the LockManager spec https://developer.mozilla.org/en-US/docs/Web/API/LockManager/request"),await r()}}))}function xc(){if(typeof globalThis!="object")try{Object.defineProperty(Object.prototype,"__magic__",{get:function(){return this},configurable:!0}),__magic__.globalThis=__magic__,delete Object.prototype.__magic__}catch{typeof self<"u"&&(self.globalThis=self)}}function Ho(e){if(!/^0x[a-fA-F0-9]{40}$/.test(e))throw new Error(`@supabase/auth-js: Address "${e}" is invalid.`);return e.toLowerCase()}function qc(e){return parseInt(e,16)}function Lc(e){const t=new TextEncoder().encode(e);return"0x"+Array.from(t,n=>n.toString(16).padStart(2,"0")).join("")}function Tc(e){var t;const{chainId:r,domain:n,expirationTime:s,issuedAt:a=new Date,nonce:i,notBefore:o,requestId:l,resources:d,scheme:c,uri:u,version:p}=e;{if(!Number.isInteger(r))throw new Error(`@supabase/auth-js: Invalid SIWE message field "chainId". Chain ID must be a EIP-155 chain ID. Provided value: ${r}`);if(!n)throw new Error('@supabase/auth-js: Invalid SIWE message field "domain". Domain must be provided.');if(i&&i.length<8)throw new Error(`@supabase/auth-js: Invalid SIWE message field "nonce". Nonce must be at least 8 characters. Provided value: ${i}`);if(!u)throw new Error('@supabase/auth-js: Invalid SIWE message field "uri". URI must be provided.');if(p!=="1")throw new Error(`@supabase/auth-js: Invalid SIWE message field "version". Version must be '1'. Provided value: ${p}`);if(!((t=e.statement)===null||t===void 0)&&t.includes(`
`))throw new Error(`@supabase/auth-js: Invalid SIWE message field "statement". Statement must not include '\\n'. Provided value: ${e.statement}`)}const m=Ho(e.address),h=c?`${c}://${n}`:n,f=e.statement?`${e.statement}
...
`).map((N,F)=>T(N,F)).filter(N=>N.url)}function T(P,D){if(P&&typeof P=="object"&&!Array.isArray(P)){const F=String(P.url||"").trim(),Y=String(P.label||"").trim()||`Разписание ${D+1}`;return{url:F,label:Y}}return{url:String(P||"").trim(),label:`Разписание ${D+1}`}}function E(P){const D=String(P||"").trim();if(!D)return"ОТС";const N=D.toLowerCase(),F={бо:"БО",болничен:"БО","болничен отпуск":"БО",до:"ДО","допълнителен отпуск":"ДО",по:"ПО","платен отпуск":"ПО",нп:"НП","неплатен отпуск":"НП",командировка:"К"};if(F[N])return F[N];if(D.length<=4&&/^[\p{L}\p{N}]+$/u.test(D))return D.toUpperCase();const Y=D.split(/\s+/).map(H=>H.trim()).filter(Boolean);return Y.length>=2?Y.slice(0,3).map(H=>H.charAt(0).toUpperCase()).join(""):D.slice(0,2).toUpperCase()}function $(P){const D=E(P);return Iu[D]||b(P)}function A(P,D){const N=new Date(`${P}T00:00:00`),F=new Date(`${D}T00:00:00`);if(Number.isNaN(N.getTime())||Number.isNaN(F.getTime())||N>F)return[];const Y=[],H=new Date(N);for(;H<=F;)Y.push(a(H)),H.setDate(H.getDate()+1);return Y}function C(P){var D;return String(((D=P==null?void 0:P.absence_reasons)==null?void 0:D.name)||"").trim()||"Отсъствие"}function I(P){const D=String(P||"").trim();if(!D)return"";try{const F=new URL(D).pathname.split("/").pop()||"",Y=F.includes(".")?F.split(".").pop():"";return String(Y||"").toLowerCase()}catch{return""}}function R(P){const D=I(P);return["doc","docx","xls","xlsx","ppt","pptx"].includes(D)?`https://view.officeapps.live.com/op/embed.aspx?src=${encodeURIComponent(P)}`:P}function W(P,D,N){const F=P.querySelector("#index-timetable-preview-modal"),Y=P.querySelector("#index-timetable-preview-frame"),H=P.querySelector("#index-timetable-preview-title"),J=P.querySelector("#index-timetable-preview-fallback"),re=P.querySelector("#index-timetable-preview-open");if(!F||!Y||!H||!J||!re)return;const ce=String(D||"").trim();if(!ce){g("Липсва линк за преглед.","warning");return}const ae=R(ce);H.textContent=N?`Преглед: ${N}`:"Преглед на разписание",re.setAttribute("href",ce),J.classList.add("d-none"),Y.src="about:blank",Y.src=ae,Y.onload=()=>{if(ae!==ce){J.classList.add("d-none");return}const ne=I(ce),Le=["doc","docx","xls","xlsx","ppt","pptx"].includes(ne);J.classList.toggle("d-none",!Le)},Y.onerror=()=>{J.classList.remove("d-none")},F.classList.remove("d-none")}function K(P){const D=P.querySelector("#index-timetable-preview-modal"),N=P.querySelector("#index-timetable-preview-frame"),F=P.querySelector("#index-timetable-preview-fallback"),Y=P.querySelector("#index-timetable-preview-open");!D||!N||!F||!Y||(N.src="about:blank",Y.setAttribute("href","#"),F.classList.add("d-none"),D.classList.add("d-none"))}function V(P){const D=i(P),N=s(D),F=n(),Y=F.startsWith(N),H=new Set([...t.plannedRows.map(ne=>_(ne==null?void 0:ne.date)).filter(Boolean),...t.actualRows.map(ne=>_(ne==null?void 0:ne.date)).filter(Boolean),...t.absenceRows.flatMap(ne=>A(ne==null?void 0:ne.start_date,ne==null?void 0:ne.end_date)),...Array.from(t.pendingConfirmationDates||[]).map(ne=>_(ne)).filter(Boolean),...Array.from((t.changeCountByDate||new Map).keys()).map(ne=>_(ne)).filter(Boolean)]),J=_(t.selectedDate),re=J?H.has(J):!1,ce=H.size>0;if(J&&J.startsWith(N)&&(!ce||re))return;const ae=[...H].sort((ne,Le)=>String(ne).localeCompare(String(Le),"bg"));if(Y&&H.has(F)){t.selectedDate=F;return}if(ae.length){t.selectedDate=String(ae[0]);return}t.selectedDate=`${N}-01`}function j(){const P=new Map;return t.plannedRows.forEach(D=>{const N=_(D==null?void 0:D.date);if(!N)return;const F=P.get(N)||{planned:0,actual:0};F.planned+=1,P.set(N,F)}),t.actualRows.forEach(D=>{const N=_(D==null?void 0:D.date);if(!N)return;const F=P.get(N)||{planned:0,actual:0};F.actual+=1,P.set(N,F)}),t.pendingConfirmationDates.forEach(D=>{const N=_(D);if(!N)return;const F=P.get(N)||{planned:0,actual:0,absences:[]};F.pendingConfirmation=!0,P.set(N,F)}),t.changeCountByDate.forEach((D,N)=>{const F=_(N);if(!F)return;const Y=P.get(F)||{planned:0,actual:0,absences:[]};Y.changeCount=Number(D||0),P.set(F,Y)}),t.absenceRows.forEach(D=>{const N=C(D),F=$(N);A(D==null?void 0:D.start_date,D==null?void 0:D.end_date).forEach(Y=>{const H=P.get(Y)||{planned:0,actual:0,absences:[]};Array.isArray(H.absences)||(H.absences=[]),H.absences.some(re=>(re==null?void 0:re.reason)===N)||H.absences.push({reason:N,className:F}),P.set(Y,H)})}),P}function z(P){const D=P.querySelector("#index-crew-calendar-days");if(!D)return;const N=t.visibleMonth||s(new Date),F=i(N),Y=new Date(F.getFullYear(),F.getMonth(),1),H=Y.getDay(),J=H===0?6:H-1,re=new Date(Y);re.setDate(Y.getDate()-J);const ce=j(),ae=n();r(P,"#index-crew-month-label",o(N));const ne=[];for(let Le=0;Le<42;Le+=1){const O=new Date(re);O.setDate(re.getDate()+Le);const ue=`${s(O)}-${String(O.getDate()).padStart(2,"0")}`,Je=O.getMonth()!==F.getMonth(),Te=ue===t.selectedDate,Bt=ue===ae,Ce=ce.get(ue)||{planned:0,actual:0,absences:[]},St=Array.isArray(Ce.absences)?Ce.absences.map(Ye=>`<span class="badge ${u(Ye.className||"text-bg-danger")}" title="${u(Ye.reason||"Отсъствие")}">${u(Ye.reason||"Отсъствие")}</span>`).join(""):"";ne.push(`
      ...
      `)}D.innerHTML=ne.join("")}function ee(P){const D=_(t.selectedDate),N=P.querySelector("#index-crew-planned-body"),F=P.querySelector("#index-crew-actual-body"),Y=P.querySelector("#index-crew-change-body"),H=P.querySelector("#index-crew-absence-body");if(!N||!F||!Y||!H)return;r(P,"#index-crew-selected-date-label",`Детайли за ${d(D)}`);const J=t.plannedRows.filter(O=>_(O==null?void 0:O.date)===D).sort((O,se)=>{var ue,Je;return String(((ue=O==null?void 0:O.duties)==null?void 0:ue.start_time)||"").localeCompare(String(((Je=se==null?void 0:se.duties)==null?void 0:Je.start_time)||""),"bg")});J.length?N.innerHTML=J.map(O=>{var Bt,Ce,St,Ye;const se=((Bt=O==null?void 0:O.duties)==null?void 0:Bt.name)||"-",ue=v(O==null?void 0:O.assignment_role),Je=`${((Ce=O==null?void 0:O.duties)==null?void 0:Ce.start_time)||"-"} - ${((St=O==null?void 0:O.duties)==null?void 0:St.end_time)||"-"}${(Ye=O==null?void 0:O.duties)!=null&&Ye.second_day?" (+1)":""}`,Te=m(O==null?void 0:O.duties);return`
          ...
          `}).join(""):N.innerHTML='<p class="text-secondary mb-0">Няма планирани повески.</p>';const re=t.actualRows.filter(O=>_(O==null?void 0:O.date)===D).sort((O,se)=>String((se==null?void 0:se.reported_at)||"").localeCompare(String((O==null?void 0:O.reported_at)||""),"bg")),ce=t.confirmedDates.has(D),ae=t.pendingConfirmationDates.has(D);ce?re.length?F.innerHTML=re.map(O=>{var Ba,Ka,za,Wa;const se=((Ba=O==null?void 0:O.duties)==null?void 0:Ba.name)||"-",ue=v(O==null?void 0:O.assignment_role),Je=O!=null&&O.reported_at?c(new Date(O.reported_at)):"-",Te=h(O),Bt=Array.isArray((Ka=O==null?void 0:O.duties)==null?void 0:Ka.duty_trains)?[...O.duties.duty_trains].sort((Ee,it)=>Number((Ee==null?void 0:Ee.sequence_order)||0)-Number((it==null?void 0:it.sequence_order)||0)):(za=O==null?void 0:O.duties)!=null&&za.duty_trains?[O.duties.duty_trains]:[],Ce=q((Wa=O==null?void 0:O.duties)==null?void 0:Wa.duty_files),St=Bt.map(Ee=>{var Va,Ga;const it=(Va=Ee==null?void 0:Ee.trains)!=null&&Va.number?`Влак ${Ee.trains.number}`:"Влак",vn=q((Ga=Ee==null?void 0:Ee.trains)==null?void 0:Ga.timetable_url);if(!vn.length)return`<div class="small">${u(it)}: <span class="text-secondary">без разписание</span></div>`;const kr=vn.map(ps=>{const Al=encodeURIComponent(ps.url),Cl=encodeURIComponent(ps.label||"Разписание"),hs=ps.label||"Разписание";return`
                    <span class="d-inline-flex align-items-center gap-1 me-2">
                      ...
                      <button
                        ...
                        data-preview-url="${u(Al)}"
                        data-preview-label="${u(Cl)}"
                        title="Преглед: ${u(hs)}"
                  ...
                  `}).join(" ");return`<div class="small">${u(it)}: ${kr}</div>`}).join(""),Ye=Ce.map(Ee=>{const it=encodeURIComponent(Ee.url),vn=encodeURIComponent(Ee.label||"Файл"),kr=Ee.label||"Файл";return`
        ...
        `,n(e,"#index-welcome-subtitle","Административен преглед на потребители, роли и системно състояние."),n(e,"#index-management-title","Административен преглед"),xi(e,["Профили","Потребители с роля","Профили със служител","Роли"]),f==null||f.classList.remove("d-none")):_==="manager"?(b.innerHTML=`
      ...
      <a href="/planned-duties" data-link class="btn btn-outline-primary">Планирани повески</a>
      <a href="/actual-duties" data-link class="btn btn-outline-primary">Реални повески</a>
    `)}const ir={visibleMonth:"",selectedDate:"",plannedRows:[],actualRows:[],actualRowsById:new Map,absenceRows:[],confirmedDates:new Set,pendingConfirmationDates:new Set,changeCountByDate:new Map,changeEventsByDate:new Map,editingActualDutyId:""},Wu=-20*60,Vu=30*60;function or(){return new Date().toISOString().slice(0,10)}function Kr(e){return new Intl.DateTimeFormat("bg-BG",{dateStyle:"medium",timeStyle:"short"}).format(e)}function qi(e){return e?new Intl.DateTimeFormat("bg-BG",{dateStyle:"medium"}).format(new Date(`${e}T00:00:00`)):"-"}function Gu(e){return String(e||"").trim().toLowerCase()}function Ws(e){return String(e??"").replaceAll("&","&amp;").replaceAll("<","&lt;").replaceAll(">","&gt;").replaceAll('"',"&quot;").replaceAll("'","&#039;")}function jr(e){return e==null||e===""?"":Ht(String(e))||""}function $n(e){const t=jr(e);return t?t.slice(0,5):"-"}function Vn(e){const t=Number(e);if(!Number.isFinite(t)||t<0)return"-";const r=Math.floor(t/60),n=t%60;return`${String(r).padStart(2,"0")}:${String(n).padStart(2,"0")}`}function Li(e){const t=Number(e);if(!Number.isFinite(t))return"-";const r=t<0?"-":"+",n=Math.abs(t),s=Math.floor(n/60),a=n%60;return`${r}${String(s).padStart(2,"0")}:${String(a).padStart(2,"0")}`}function Ti(e){const t=Number(e||0);return Number.isFinite(t)?t<Wu||t>Vu?"text-bg-danger":"text-bg-success":"text-bg-secondary"}function Vs(e){const t=jr(e==null?void 0:e.start_time),r=jr(e==null?void 0:e.end_time),n=jr(e==null?void 0:e.break_start_time),s=jr(e==null?void 0:e.break_end_time),a=n&&s?Ie(n,s):null,i=t&&r?Ie(t,r):null,o=Number.isFinite(i)&&Number.isFinite(a)?Math.max(0,i-a):null;return{startTime:$n(e==null?void 0:e.start_time),endTime:$n(e==null?void 0:e.end_time),breakStartTime:$n(e==null?void 0:e.break_start_time),breakEndTime:$n(e==null?void 0:e.break_end_time),breakDuration:a===null?"-":Vn(a),duration:o===null?"-":Vn(o),breakDurationMinutes:a,durationMinutes:o}}function Ei(e){const t=(e==null?void 0:e.duties)||{};return Vs({start_time:(e==null?void 0:e.start_time_override)??(t==null?void 0:t.start_time),end_time:(e==null?void 0:e.end_time_override)??(t==null?void 0:t.end_time),break_start_time:(e==null?void 0:e.break_start_time_override)??(t==null?void 0:t.break_start_time),break_end_time:(e==null?void 0:e.break_end_time_override)??(t==null?void 0:t.break_end_time)})}function An(e){return e?`${String(e).slice(0,5)}:00`:null}async function Ju(e,t,r){var f,w,y,v,b;if((e.dataset.indexMode||"default")!=="crew")return;const s=e.dataset.indexEmployeeId||"",a=(((f=e.querySelector("#index-actual-duty-edit-id"))==null?void 0:f.value)||"").trim(),i=(((w=e.querySelector("#index-actual-duty-start"))==null?void 0:w.value)||"").trim(),o=(((y=e.querySelector("#index-actual-duty-end"))==null?void 0:y.value)||"").trim(),l=(((v=e.querySelector("#index-actual-duty-break-start"))==null?void 0:v.value)||"").trim(),d=(((b=e.querySelector("#index-actual-duty-break-end"))==null?void 0:b.value)||"").trim(),c=e.querySelector("#index-actual-duty-edit-save");if(!a||!i||!o||!l||!d){g("Попълни всички полета.","warning");return}const u=Ie(i,o);if(Ie(l,d)>u){g("Прекъсването не може да е по-голямо от продължителността.","warning");return}const m=(c==null?void 0:c.innerHTML)||"Запази";c&&(c.disabled=!0,c.innerHTML='<span class="spinner-border spinner-border-sm me-2" role="status" aria-hidden="true"></span>Запис...');const{error:h}=await S.from("actual_duties").update({start_time_override:An(i),end_time_override:An(o),break_start_time_override:An(l),break_end_time_override:An(d)}).eq("id",a);if(c&&(c.disabled=!1,c.innerHTML=m),h){g(h.message||"Редакцията не беше запазена.","error");return}r(e),await t(e,s,ir.visibleMonth),g("Реалната повеска е обновена.","success")}function Qu(e){const t=Gu(e);return t==="chief"?"Началник влак":t==="conductor"?"Кондуктор":t==="driver"?"Машинист":t==="assistant_driver"?"Пом. машинист":"Кондуктор"}function je(e,t,r){const n=e.querySelector(t);n&&(n.textContent=r)}async function Yu(e){const{data:t}=await S.auth.getSession(),r=(t==null?void 0:t.session)||null,n=(r==null?void 0:r.user)||null;if(!(n!=null&&n.id))return je(e,"#index-welcome-title","Добре дошъл в TrainCrewHub"),je(e,"#index-welcome-subtitle","Влез в профила си, за да видиш персонална информация."),{userId:"",employeeId:"",roles:[],crew:!1,mode:"default"};const[{data:s},{data:a}]=await Promise.all([S.from("user_profiles").select("username, employee_id, employees(first_name, last_name)").eq("id",n.id).maybeSingle(),S.from("user_roles").select("role").eq("user_id",n.id)]),i=(s==null?void 0:s.username)||n.email||n.id,o=s!=null&&s.employees?`${s.employees.first_name||""} ${s.employees.last_name||""}`.trim():"",l=[...new Set((a||[]).map(c=>String((c==null?void 0:c.role)||"").trim()).filter(Boolean))],d=Au(l);return je(e,"#index-welcome-title",`Здравей, ${i}${o?` | ${o}`:""}`),je(e,"#index-welcome-subtitle","Тук виждаш твоя профил и бърз оперативен преглед за деня."),{userId:n.id,employeeId:(s==null?void 0:s.employee_id)||"",roles:l,crew:Sa(l),mode:d}}function $i(e){const t=["text-bg-primary","text-bg-success","text-bg-warning","text-bg-danger","text-bg-info","text-bg-dark"],r=String(e||"").trim().toLowerCase();if(!r)return t[0];const n={бо:"text-bg-warning",do:"text-bg-danger",до:"text-bg-danger",болничен:"text-bg-warning",отпуск:"text-bg-primary",командировка:"text-bg-info"};if(n[r])return n[r];let s=0;for(let a=0;a<r.length;a+=1)s=(s*31+r.charCodeAt(a))%2147483647;return t[Math.abs(s)%t.length]}async function Gs(e){var c;const t=or(),[r,n,s,a,i]=await Promise.all([S.from("planned_duties").select("id",{count:"exact",head:!0}).eq("date",t),S.from("actual_duties").select("id",{count:"exact",head:!0}).eq("date",t),S.from("employee_absences").select("id",{count:"exact",head:!0}).lte("start_date",t).gte("end_date",t),S.from("employees").select("id",{count:"exact",head:!0}).eq("is_active",!0),S.from("schedule_publications").select("is_confirmed").eq("schedule_date",t).maybeSingle()]);[r,n,s,a,i].some(u=>u.error)&&g("Част от данните за индекс страницата не могат да се заредят.","warning");const d=!!((c=i==null?void 0:i.data)!=null&&c.is_confirmed)?Number(n.count??0):0;je(e,"#index-kpi-planned",String(r.count??0)),je(e,"#index-kpi-actual",String(d)),je(e,"#index-kpi-absences",String(s.count??0)),je(e,"#index-kpi-employees",String(a.count??0)),je(e,"#index-last-updated",`Последно обновяване: ${Kr(new Date)}`)}async function zo(e){await qu(e,{setText:je,formatDateTime:Kr,escapeHtml:Ws})}function Xu(e,t,r,n){const s=e.querySelector("#index-refresh"),a=e.querySelector("#index-certificates-panel"),i=e.querySelector("#index-absences-panel"),o=e.querySelector("#index-workload-date"),l=e.querySelector("#index-workload-refresh"),{loadCrewMonthlySnapshot:d,renderCrewCalendarAndDetails:c}=r,{openCrewActualDutyEditModal:u,openCrewTimetablePreview:p,closeCrewTimetablePreview:m,closeCrewActualDutyEditModal:h}=n;Cu(e,{loadCrewMonthlySnapshot:d,crewCalendarState:ir,toMonthKey:ar,shiftMonthKey:Ou,getTodayIsoDate:or,renderCrewCalendarAndDetails:c,openCrewActualDutyEditModal:u,openCrewTimetablePreview:p,closeCrewTimetablePreview:m,closeCrewActualDutyEditModal:h,saveCrewActualDutyEdits:f=>Ju(f,d,h)}),s==null||s.addEventListener("click",async()=>{const f=e.dataset.indexMode||"default";if(f==="admin"){s.disabled=!0,await zo(e),s.disabled=!1;return}if(It(f)){s.disabled=!0,await t.loadHeadOfTransportSnapshot(e),s.disabled=!1;return}f!=="crew"&&(s.disabled=!0,await Gs(e),s.disabled=!1)}),a==null||a.addEventListener("click",f=>{const w=f.target.closest("button[data-index-cert-action]");if(!w)return;const y=e.dataset.indexMode||"default";if(!It(y))return;const v=w.getAttribute("data-index-cert-action")||"";if(v==="toggle-soon"){t.toggleCertificateDetails(e,"soon");return}v==="toggle-expired"&&t.toggleCertificateDetails(e,"expired")}),i==null||i.addEventListener("click",f=>{const w=f.target.closest("button[data-index-absence-action]");if(!w)return;const y=e.dataset.indexMode||"default";if(!It(y)||(w.getAttribute("data-index-absence-action")||"")!=="toggle-reason")return;const b=w.getAttribute("data-index-absence-key")||"";t.toggleAbsenceReasonDetails(e,b)}),o==null||o.addEventListener("change",async()=>{const f=e.dataset.indexMode||"default";It(f)&&await t.loadHeadOfTransportWorkload(e)}),l==null||l.addEventListener("click",async()=>{const f=e.dataset.indexMode||"default";It(f)&&(l.disabled=!0,await t.loadHeadOfTransportWorkload(e),l.disabled=!1)})}async function Zu(e){e.innerHTML=xu;const t=Eu({loadKpiSnapshot:Gs,getTodayIsoDate:or,formatDate:qi,escapeHtml:Ws,formatMinutesAsClock:Vn,formatSignedMinutesAsClock:Li,getDeviationClassByThreshold:Ti,parseIsoDateSafe:Zr,toMonthKey:ar,getMonthBounds:Ss,countBulgarianWorkdays:Si,getDutyTimingSummary:Vs,getActualDutyTimingSummary:Ei,getDistinctBadgeClassByReason:$i}),r=Du({crewCalendarState:ir,setText:je,getTodayIsoDate:or,toMonthKey:ar,toIsoDateFromDate:wt,parseMonthKey:is,formatMonthLabel:Pu,getMonthBounds:Ss,formatDate:qi,formatDateTime:Kr,escapeHtml:Ws,countBulgarianWorkdays:Si,getDutyTimingSummary:Vs,getActualDutyTimingSummary:Ei,formatMinutesAsClock:Vn,formatSignedMinutesAsClock:Li,getDeviationClassByThreshold:Ti,formatRoleLabel:Qu,getDistinctBadgeClassByReason:$i}),n=Ru({crewCalendarState:ir,ensureCrewSelectedDate:r.ensureCrewSelectedDate,renderCrewCalendar:r.renderCrewCalendar,renderCrewHoursSummary:r.renderCrewHoursSummary,renderCrewSelectedDayDetails:r.renderCrewSelectedDayDetails,toMonthKey:ar,getMonthBounds:Ss,loadSchedulePublicationDates:(a,i)=>Bu(S,a,i),loadScheduleChangesSummary:(a,i)=>Ku(S,a,i,Kr),formatDateTime:Kr,setText:je});Xu(e,t,n,r);const s=await Yu(e);if(zu(e,s,{setText:je,isTransportAnalyticsMode:It,getTodayIsoDate:or}),(s==null?void 0:s.mode)==="crew"){const a=ar(new Date);ir.visibleMonth=a,ir.selectedDate=or(),await n.loadCrewMonthlySnapshot(e,s.employeeId||"",a);return}if((s==null?void 0:s.mode)==="admin"){await zo(e);return}if(It((s==null?void 0:s.mode)||"default")){await t.loadHeadOfTransportSnapshot(e);return}await Gs(e)}const ep=`<section class="card border-0 shadow-sm" style="max-width: 400px; margin: 0 auto;">\r
  <div class="card-body p-4">\r
...
`;async function tp({attempts:e=10,delayMs:t=120}={}){var r,n;for(let s=0;s<e;s+=1){const{data:a}=await S.auth.getSession();if((n=(r=a==null?void 0:a.session)==null?void 0:r.user)!=null&&n.id)return!0;await new Promise(i=>{window.setTimeout(i,t)})}return!1}async function rp(e){e.innerHTML=ep,np(e),sp(e)}function np(e){e.querySelectorAll("button[data-toggle-password]").forEach(t=>{t.addEventListener("click",()=>{const r=t.getAttribute("data-toggle-password")||"",n=e.querySelector(`#${r}`);if(!n)return;const s=n.type==="password";n.type=s?"text":"password",t.textContent=s?"🙈":"👁",t.setAttribute("aria-label",s?"Скрий паролата":"Покажи паролата")})})}function sp(e){const t=e.querySelector("#login-form");t&&t.addEventListener("submit",async r=>{var p,m,h,f,w;r.preventDefault();const n=t.querySelector('input[name="identifier"]').value.trim(),s=t.querySelector('input[name="password"]').value;if(!n||!s){g("Попълни имейл/потребителско име и парола.","warning");return}let a=n;if(!n.includes("@")){const{data:y}=await S.rpc("resolve_login_email",{input_username:n});y&&(a=String(y).trim())}const i=t.querySelector('button[type="submit"]'),o=i.innerHTML;i.disabled=!0,i.innerHTML='<span class="spinner-border spinner-border-sm me-2" role="status" aria-hidden="true"></span>Влизане...';const{data:l,error:d}=await S.auth.signInWithPassword({email:a,password:s});if(i.disabled=!1,i.innerHTML=o,d){g("Невалидни данни за вход.","error");return}const c=((p=l==null?void 0:l.user)==null?void 0:p.id)||((h=(m=l==null?void 0:l.session)==null?void 0:m.user)==null?void 0:h.id)||"";if(c&&!await Fo(c)){await S.auth.signOut(),g("Профилът е деактивиран. Свържи се с администратор.","warning");return}if(g("Успешен вход.","success"),!(!!((w=(f=l==null?void 0:l.session)==null?void 0:f.user)!=null&&w.id)||await tp())){g("Влизането е успешно, но сесията не е активирана. Опитай отново.","warning");return}window.history.pushState({},"","/"),window.dispatchEvent(new PopStateEvent("popstate"))})}const ap=`<section class="card border-0 shadow-sm" style="max-width: 400px; margin: 0 auto;">\r
  <div class="card-body p-4">\r
...
`;function cp(){return`${window.location.origin}/reset-password`}async function up(e){if(e.includes("@"))return e;const{data:t}=await S.rpc("resolve_login_email",{input_username:e});return String(t||"").trim()}async function pp(e){e.innerHTML=dp;const t=e.querySelector("#forgot-password-form");t&&t.addEventListener("submit",async r=>{var i;r.preventDefault();const n=(((i=t.querySelector('input[name="identifier"]'))==null?void 0:i.value)||"").trim();if(!n){g("Въведи имейл или потребителско име.","warning");return}const s=t.querySelector('button[type="submit"]'),a=(s==null?void 0:s.innerHTML)||"Изпрати линк";s&&(s.disabled=!0,s.innerHTML='<span class="spinner-border spinner-border-sm me-2" role="status" aria-hidden="true"></span>Изпращане...');try{const o=await up(n);o&&await S.auth.resetPasswordForEmail(o,{redirectTo:cp()})}catch{}s&&(s.disabled=!1,s.innerHTML=a),g("Ако акаунтът съществува, изпратихме инструкции за смяна на паролата.","success"),t.reset()})}const hp=`<section class="card border-0 shadow-sm" style="max-width: 460px; margin: 0 auto;">\r
  <div class="card-body p-4">\r
...
`,gp=3e4;let jn=null,zr=null;function Ai(){jn&&(window.clearInterval(jn),jn=null),zr&&(window.removeEventListener("route:changed",zr),zr=null)}function Ci(){window.location.pathname==="/pending-access"&&window.dispatchEvent(new PopStateEvent("popstate"))}async function _p(e){Ai(),e.innerHTML=vp;const t=e.querySelector("#pending-access-refresh"),r=e.querySelector("#pending-access-logout");t==null||t.addEventListener("click",()=>{Ci()}),r==null||r.addEventListener("click",async()=>{const{error:n}=await S.auth.signOut();if(n){g(n.message,"error");return}g("Излезе успешно от системата.","success"),window.history.replaceState({},"","/login"),window.dispatchEvent(new PopStateEvent("popstate"))}),jn=window.setInterval(()=>{Ci()},gp),zr=()=>{window.location.pathname!=="/pending-access"&&Ai()},window.addEventListener("route:changed",zr)}const wp=`<section class="card border-0 shadow-sm">\r
  <div class="card-body p-4 p-md-5">\r
    <div class="d-flex justify-content-between align-items-center mb-3">\r
...
`;function ks(e){e.classList.remove("d-none"),document.body.classList.add("overflow-hidden")}const Ri=new Map;function Sp(e,t){const r=Ri.get(e);r&&document.removeEventListener("keydown",r);const n=s=>{if(s.key==="Escape"){for(const a of t)if(a&&!a.classList.contains("d-none")){yr(a);return}}};Ri.set(e,n),document.addEventListener("keydown",n)}function yr(e){var t,r;e.classList.add("d-none"),(t=document.querySelector("#schedule-key-modal"))!=null&&t.classList.contains("d-none")&&((r=document.querySelector("#schedule-key-delete-modal"))!=null&&r.classList.contains("d-none"))&&document.body.classList.remove("overflow-hidden")}function Be(e){return String(e).replaceAll("&","&amp;").replaceAll("<","&lt;").replaceAll(">","&gt;").replaceAll('"',"&quot;").replaceAll("'","&#039;")}const he={rows:[],filters:{name:"",type:"",crewRole:"",isActive:"",validFrom:"",validTo:""}};async function ka(e){const{data:t,error:r}=await S.from("schedule_keys").select("id, name, is_active, type, crew_role, valid_from, valid_to").order("valid_from",{ascending:!1});if(r){g(r.message,"error"),he.rows=[],et(e,"Грешка при зареждане на Ключ-График.");return}he.rows=t||[],et(e)}function et(e,t){const r=e.querySelector("#schedule-keys-table-body"),n=e.querySelector("#schedule-keys-empty"),s=he.rows.filter(a=>{const i=!he.filters.name||(a.name||"").toLowerCase().includes(he.filters.name),o=!he.filters.type||a.type===he.filters.type,l=!he.filters.crewRole||a.crew_role===he.filters.crewRole,d=!he.filters.isActive||String(!!a.is_active)===he.filters.isActive,c=!he.filters.validFrom||a.valid_from===he.filters.validFrom,u=!he.filters.validTo||a.valid_to===he.filters.validTo;return i&&o&&l&&d&&c&&u});if(!s.length){r.innerHTML="",n.classList.remove("d-none"),n.textContent=t||"Няма записи по зададените филтри.";return}n.classList.add("d-none"),r.innerHTML=s.map(a=>`
        <tr>
          <td>${Be(a.name??"-")}</td>
          <td>${Be(a.type??"-")}</td>
...
`;function Vo({duty:e,scheduleKeyNames:t,trainNumbers:r,attachmentEntries:n,escapeHtml:s,intervalToTimeInput:a,formatInterval:i}){var u;const o=((e==null?void 0:e.start_time)||"-").slice(0,5)||"-",l=((e==null?void 0:e.end_time)||"-").slice(0,5)||"-",d=(a((e==null?void 0:e.break_start_time)||"00:00:00")||"-").slice(0,5)||"-",c=(a((e==null?void 0:e.break_end_time)||"00:00:00")||"-").slice(0,5)||"-";return`
    <div class="row g-3">
      <div class="col-md-6">
  ...
  `}function Ut(e){e.classList.remove("d-none"),document.body.classList.add("overflow-hidden")}const Ii=new Map;function $p(e,t){const r=Ii.get(e);r&&document.removeEventListener("keydown",r);const n=s=>{if(s.key==="Escape"){for(const a of t)if(a&&!a.classList.contains("d-none")){Ve(a);return}}};Ii.set(e,n),document.addEventListener("keydown",n)}function Ve(e){var t,r,n,s;e.classList.add("d-none"),(t=document.querySelector("#duty-modal"))!=null&&t.classList.contains("d-none")&&((r=document.querySelector("#duty-delete-modal"))!=null&&r.classList.contains("d-none"))&&((n=document.querySelector("#duty-profile-modal"))!=null&&n.classList.contains("d-none"))&&((s=document.querySelector("#duty-attachment-preview-modal"))!=null&&s.classList.contains("d-none"))&&document.body.classList.remove("overflow-hidden")}function Di(e){return e?typeof e=="string"?e.replace(".000000",""):String(e):"-"}function pe(e){return String(e).replaceAll("&","&amp;").replaceAll("<","&lt;").replaceAll(">","&gt;").replaceAll('"',"&quot;").replaceAll("'","&#039;")}const Cn=8,Q={allDuties:[],searchQuery:"",scheduleKeyFilter:"",dutyTypeFilter:"",currentPage:1,draggedDutyId:null,reorderEnabled:!0};async function os(e){const{data:t,error:r}=await S.from("duties").select("id, name, notes, duty_files, duty_type_id, start_time, end_time, second_day, break_start_time, break_end_time, break_duration_interval, duration_interval, display_order, duty_types(name), schedule_key_duties(schedule_key_id, schedule_keys(name)), duty_trains(train_id, sequence_order, trains(number))").order("display_order",{ascending:!0}).order("name",{ascending:!0});if(r){g(r.message,"error"),Q.allDuties=[],tt(e,"Грешка при зареждане на повеските.");return}Q.allDuties=t||[],tt(e)}async function Ap(){const e=Q.allDuties.map((n,s)=>S.from("duties").update({display_order:s+1}).eq("id",n.id)),r=(await Promise.all(e)).find(n=>n.error);return r!=null&&r.error?(g(r.error.message,"error"),!1):(Q.allDuties=Q.allDuties.map((n,s)=>({...n,display_order:s+1})),!0)}function tt(e,t){const r=e.querySelector("#duties-table-body"),n=e.querySelector("#duties-empty"),s=e.querySelector("#duties-pagination"),a=e.querySelector("#duties-page-info"),i=e.querySelector("#duties-prev-page"),o=e.querySelector("#duties-next-page");Op(e),Pp(e);const l=Q.allDuties.filter(h=>{var x;const f=(h.name||"").toLowerCase(),w=(((x=h.duty_types)==null?void 0:x.name)||"").toLowerCase(),y=Qs(h).map(L=>L.toLowerCase()),v=Rp(h).join(" ").toLowerCase(),b=!Q.searchQuery||f.includes(Q.searchQuery)||v.includes(Q.searchQuery),_=!Q.scheduleKeyFilter||y.includes(Q.scheduleKeyFilter),k=!Q.dutyTypeFilter||w===Q.dutyTypeFilter;return b&&_&&k});if(!l.length){r.innerHTML="",n.classList.remove("d-none"),n.textContent=t||"Няма въведени повески.",s.classList.add("d-none");return}n.classList.add("d-none");const d=Math.max(1,Math.ceil(l.length/Cn));Q.currentPage>d&&(Q.currentPage=d),Q.currentPage<1&&(Q.currentPage=1);const c=(Q.currentPage-1)*Cn,u=c+Cn,p=l.slice(c,u),m=Q.reorderEnabled!==!1;if(r.innerHTML=p.map(h=>{var _;const f=Qs(h),w=Cp(h);Ip(h);const y=Dp(h),v=`<span class="badge text-bg-info" title="${pe(f.join(", "))}">${w.length} кл-гр</span>`,b=y>0?`<span class="badge text-bg-secondary" title="Прикачени файлове">${y} док.</span>`:"";return`
        <tr data-duty-id="${h.id}" draggable="${m?"true":"false"}">
          ...
          <td>${pe(h.end_time??"-")}</td>
          <td>${pe(Di(h.break_duration_interval))}</td>
          <td>${pe(Di(h.duration_interval))}</td>
          <td class="text-end">
            <div class="d-inline-flex gap-2">
              <button
                type="button"
      ...
      `}).join(""),l.length<=Cn){s.classList.add("d-none");return}s.classList.remove("d-none"),a.textContent=`Страница ${Q.currentPage} от ${d}`,i.disabled=Q.currentPage<=1,o.disabled=Q.currentPage>=d}function Go(e){return Array.isArray(e.schedule_key_duties)?e.schedule_key_duties:e.schedule_key_duties?[e.schedule_key_duties]:[]}function Jo(e){return Array.isArray(e.duty_trains)?e.duty_trains:e.duty_trains?[e.duty_trains]:[]}function Qs(e){const t=Go(e).map(r=>{var n;return(n=r==null?void 0:r.schedule_keys)==null?void 0:n.name}).filter(Boolean);return[...new Set(t)]}function Cp(e){const t=Go(e).map(r=>r==null?void 0:r.schedule_key_id).filter(Boolean);return[...new Set(t)]}function Rp(e){const t=Jo(e).map(r=>{var n;return(n=r==null?void 0:r.trains)==null?void 0:n.number}).filter(Boolean);return[...new Set(t)]}function Ip(e){const t=Jo(e).map(r=>({trainId:r==null?void 0:r.train_id,sequenceOrder:Number.isFinite(Number(r==null?void 0:r.sequence_order))?Number(r.sequence_order):Number.MAX_SAFE_INTEGER})).filter(r=>!!r.trainId).sort((r,n)=>r.sequenceOrder-n.sequenceOrder);return[...new Set(t.map(r=>r.trainId))]}function Dp(e){const t=String((e==null?void 0:e.duty_files)||"").trim();if(!t)return 0;if(t.startsWith("["))try{const r=JSON.parse(t);if(Array.isArray(r))return r.filter(n=>String((n==null?void 0:n.url)||"").trim()).length}catch{return 1}return t.split(`
`).map(r=>r.trim()).filter(Boolean).length}function Op(e){const t=e.querySelector("#duties-type-filter");if(!t)return;const r=Q.dutyTypeFilter||"",n=[...new Set(Q.allDuties.map(s=>{var a;return String(((a=s==null?void 0:s.duty_types)==null?void 0:a.name)||"").trim()}).filter(Boolean))].sort((s,a)=>s.localeCompare(a,"bg"));t.innerHTML=`
    <option value="">Всички</option>
    ${n.map(s=>`<option value="${pe(s.toLowerCase())}">${pe(s)}</option>`).join("")}
  `,t.value=r}function Pp(e){const t=e.querySelector("#duties-schedule-key-filter");if(!t)return;const r=Q.scheduleKeyFilter||"",n=[...new Set(Q.allDuties.flatMap(s=>Qs(s)).map(s=>String(s||"").trim()).filter(Boolean))].sort((s,a)=>s.localeCompare(a,"bg"));t.innerHTML=`
    <option value="">Всички</option>
    ${n.map(s=>`<option value="${pe(s.toLowerCase())}">${pe(s)}</option>`).join("")}
  `,t.value=r}let Lr="",Yt=null;async function Mp(){var i,o;const{data:e,error:t}=await S.auth.getSession();if(t)return Lr="",Yt=[],[];const r=((o=(i=e==null?void 0:e.session)==null?void 0:i.user)==null?void 0:o.id)||"";if(!r)return Lr="",Yt=[],[];if(Yt&&Lr===r)return Yt;const{data:n,error:s}=await S.from("user_roles").select("role").eq("user_id",r);if(s)return Lr=r,Yt=[],[];const a=[...new Set((n||[]).map(l=>String((l==null?void 0:l.role)||"").trim()).filter(Boolean))];return Lr=r,Yt=a,a}async function hn(){const e=await Mp();return Sa(e)}const en="duty-files",tn=5;async function jp(e){e.innerHTML=Ep,Q.reorderEnabled=!await hn(),Np(e),Fp(e),await Up(e),await Bp(e),await Hp(e),await os(e)}function Np(e){const t=e.querySelector("#duty-form-fields");t&&(t.innerHTML=`
    ${Js({idPrefix:"duty"})}
    ...
    <div class="col-12">
      ...
      <div class="form-text">Може да добавиш до ${tn} файла общо.</div>
    </div>
  ...
  `)}async function Hp(e){const t=e.querySelector("#duty-trains"),{data:r,error:n}=await S.from("trains").select("id, number, origin_station, destination_station").order("number",{ascending:!0});if(n){g(eh(n),"error");return}const s=(r||[]).map(a=>{const i=`${a.origin_station||"-"} - ${a.destination_station||"-"}`;return`<option value="${a.id}">${pe(a.number||"-")} (${pe(i)})</option>`}).join("");t.innerHTML=s}async function Up(e){const t=e.querySelector("#duty-type"),{data:r,error:n}=await S.from("duty_types").select("id, name").order("name",{ascending:!0});if(n){g(n.message,"error");return}const s=(r||[]).map(a=>`<option value="${a.id}">${pe(a.name)}</option>`).join("");t.innerHTML='<option value="">Избери тип</option>'+s}function Fp(e){var q,T;const t=e.querySelector("#open-create-duty"),r=e.querySelector("#duty-form"),n=e.querySelector("#duty-cancel-btn"),s=e.querySelector("#duties-table-body"),a=e.querySelector("#duty-modal"),i=e.querySelector("#duty-delete-modal"),o=e.querySelector("#duty-profile-modal"),l=e.querySelector("#duty-attachment-preview-modal"),d=e.querySelector("#duty-modal-close"),c=e.querySelector("#duty-delete-confirm"),u=e.querySelector("#duty-delete-cancel"),p=e.querySelector("#duty-profile-close"),m=e.querySelector("#duty-profile-close-secondary"),h=e.querySelector("#duty-profile-duplicate"),f=e.querySelector("#duty-profile-edit"),w=e.querySelector("#duties-search"),y=e.querySelector("#duties-schedule-key-filter"),v=e.querySelector("#duties-type-filter"),b=e.querySelector("#duties-filter-reset"),_=e.querySelector("#duties-prev-page"),k=e.querySelector("#duties-next-page"),x=e.querySelector("#duty-attachment-file"),L=e.querySelector("#duty-current-attachments-links");t==null||t.addEventListener("click",()=>{qa(e),Ut(a)}),r==null||r.addEventListener("submit",async E=>{E.preventDefault(),await Kp(e)}),n==null||n.addEventListener("click",()=>{Ve(a)}),d==null||d.addEventListener("click",()=>{Ve(a)}),u==null||u.addEventListener("click",()=>{Ve(i)}),p==null||p.addEventListener("click",()=>{Ve(o)}),m==null||m.addEventListener("click",()=>{Ve(o)}),(q=e.querySelector("#duty-profile-content"))==null||q.addEventListener("click",E=>{const $=E.target.closest('button[data-duty-profile-action="preview-attachment"]');if(!$)return;const A=String($.getAttribute("data-url")||"").trim(),C=String($.getAttribute("data-label")||"").trim();Oi(e,A,C)}),(T=e.querySelector("#duty-attachment-preview-close"))==null||T.addEventListener("click",()=>{Jp(e)}),f==null||f.addEventListener("click",()=>{var $;const E=(($=o==null?void 0:o.dataset)==null?void 0:$.dutyId)||"";E&&(Ve(o),Pi(e,E))}),h==null||h.addEventListener("click",()=>{var $;const E=(($=o==null?void 0:o.dataset)==null?void 0:$.dutyId)||"";E&&(Ve(o),Mi(e,E))}),w==null||w.addEventListener("input",E=>{Q.searchQuery=E.target.value.trim().toLowerCase(),Q.currentPage=1,tt(e)}),v==null||v.addEventListener("change",E=>{Q.dutyTypeFilter=E.target.value||"",Q.currentPage=1,tt(e)}),y==null||y.addEventListener("change",E=>{Q.scheduleKeyFilter=E.target.value||"",Q.currentPage=1,tt(e)}),b==null||b.addEventListener("click",()=>{Q.searchQuery="",Q.scheduleKeyFilter="",Q.dutyTypeFilter="",Q.currentPage=1,w&&(w.value=""),y&&(y.value=""),v&&(v.value=""),tt(e)}),_==null||_.addEventListener("click",()=>{Q.currentPage-=1,tt(e)}),k==null||k.addEventListener("click",()=>{Q.currentPage+=1,tt(e)}),x==null||x.addEventListener("change",()=>{var E;(E=x.files)!=null&&E.length&&x.files.length>tn&&(g(`Може да избереш до ${tn} файла наведнъж.`,"warning"),x.value="")}),L==null||L.addEventListener("input",E=>{const $=E.target.closest(".duty-existing-attachment-label");if(!$)return;const A=Number($.getAttribute("data-index"));if(!Number.isInteger(A)||A<0)return;const C=e.querySelector("#duty-draft-attachments"),I=Ft((C==null?void 0:C.value)||"");I[A]&&(I[A].label=$.value,C&&(C.value=rn(I)||""))}),L==null||L.addEventListener("click",E=>{const $=E.target.closest(".duty-existing-attachment-preview");if($){const W=String($.getAttribute("data-url")||"").trim(),K=String($.getAttribute("data-label")||"").trim();Oi(e,W,K);return}const A=E.target.closest(".duty-existing-attachment-remove");if(!A)return;const C=Number(A.getAttribute("data-index"));if(!Number.isInteger(C)||C<0)return;const I=e.querySelector("#duty-draft-attachments"),R=Ft((I==null?void 0:I.value)||"");R[C]&&(R.splice(C,1),La(e,R))}),$p("duties",[l,o,i,a]),c==null||c.addEventListener("click",async()=>{const E=e.querySelector("#duty-delete-id").value;await zp(E,e)}),s==null||s.addEventListener("click",async E=>{const $=E.target.closest("button[data-action]");if(!$)return;const A=$.getAttribute("data-action");if(A==="profile"){const C=$.getAttribute("data-id");th(e,C);return}if(A==="edit"){const C=$.getAttribute("data-id");Pi(e,C);return}if(A==="duplicate"){const C=$.getAttribute("data-id");Mi(e,C);return}if(A==="delete"){const C=$.getAttribute("data-id");e.querySelector("#duty-delete-id").value=C,Ut(i)}}),s==null||s.addEventListener("dragstart",E=>{if(!Q.reorderEnabled)return;const $=E.target.closest("tr[data-duty-id]");$&&(Q.draggedDutyId=$.getAttribute("data-duty-id"),$.classList.add("table-active"))}),s==null||s.addEventListener("dragend",E=>{if(!Q.reorderEnabled)return;const $=E.target.closest("tr[data-duty-id]");$&&$.classList.remove("table-active"),Q.draggedDutyId=null}),s==null||s.addEventListener("dragover",E=>{Q.reorderEnabled&&E.preventDefault()}),s==null||s.addEventListener("drop",async E=>{if(!Q.reorderEnabled)return;E.preventDefault();const $=E.target.closest("tr[data-duty-id]"),A=Q.draggedDutyId;if(!$||!A)return;const C=$.getAttribute("data-duty-id");if(!C||C===A)return;const I=Q.allDuties.findIndex(V=>V.id===A),R=Q.allDuties.findIndex(V=>V.id===C);if(I<0||R<0)return;const[W]=Q.allDuties.splice(I,1);if(Q.allDuties.splice(R,0,W),tt(e),!await Ap()){await os(e);return}g("Редът на повеските е запазен.","success")})}async function Bp(e){const t=e.querySelector("#duty-schedule-keys"),{data:r,error:n}=await S.from("schedule_keys").select("id, name").order("name",{ascending:!0});if(n){g(n.message,"error");return}const s=(r||[]).map(a=>`<option value="${a.id}">${pe(a.name)}</option>`).join("");t.innerHTML=s}async function Kp(e){var F,Y;const t=e.querySelector("#duty-id"),r=e.querySelector("#duty-name"),n=e.querySelector("#duty-type"),s=e.querySelector("#duty-schedule-keys"),a=e.querySelector("#duty-trains"),i=Nr(e,"#duty-start","#duty-start-time"),o=Nr(e,"#duty-end","#duty-end-time"),l=e.querySelector("#duty-second-day"),d=Nr(e,"#duty-break-start","#duty-break-start-time"),c=Nr(e,"#duty-break-end","#duty-break-end-time"),u=e.querySelector("#duty-notes"),p=e.querySelector("#duty-attachment-file"),m=e.querySelector("#duty-existing-attachments"),h=e.querySelector("#duty-draft-attachments"),f=e.querySelector("#duty-save-btn"),w=r.value.trim(),y=n.value||null,v=Array.from(s.selectedOptions||[]).map(H=>H.value).filter(Boolean),b=Array.from(a.selectedOptions||[]).map(H=>H.value).filter(Boolean),_=v[0]||null,k=(i==null?void 0:i.value)||"",x=(o==null?void 0:o.value)||"",L=l.checked,q=(d==null?void 0:d.value)||"00:00",T=(c==null?void 0:c.value)||"00:00",E=u.value.trim()||null,$=Ft((m==null?void 0:m.value)||""),A=Ft((h==null?void 0:h.value)||""),C=Array.from((p==null?void 0:p.files)||[]),I=t.value;if(!w||!y||!k||!x){g("Моля, попълни всички задължителни полета.","warning");return}if(!v.length){g("Избери поне един ключ-график.","warning");return}const R=Ie(k,x);if(Ie(q,T)>R){g("Прекъсването не може да е по-голямо от продължителността на повеската.","warning");return}const K=f.innerHTML;f.disabled=!0,f.innerHTML='<span class="spinner-border spinner-border-sm me-2" role="status" aria-hidden="true"></span>Запис...';const V=I||crypto.randomUUID(),j=Gn(A),z=[];if(j.length+C.length>tn){f.disabled=!1,f.innerHTML=K,g(`Максимум ${tn} файла/линка за една повеска.`,"warning");return}if(C.length){const H=await Gp(C,V);if(!H){f.disabled=!1,f.innerHTML=K;return}H.forEach(J=>{J!=null&&J.url&&j.push({url:J.url,label:J.label||nn(J.url,j.length)}),J!=null&&J.objectPath&&z.push(J.objectPath)})}const le=Gn(j),P={name:w,duty_type_id:y,schedule_key_id:_,start_time:k,end_time:x,second_day:L,break_start_time:q,break_end_time:T,notes:E,duty_files:rn(le)};let D,N=I||null;if(I)({error:D}=await S.from("duties").update(P).eq("id",I));else{const{data:H}=await S.auth.getUser(),J=((F=H==null?void 0:H.user)==null?void 0:F.id)??((Y=H==null?void 0:H.user)==null?void 0:Y.email)??"web_app",re=Q.allDuties.reduce((ne,Le)=>Math.max(ne,Number(Le.display_order)||0),0),{data:ce,error:ae}=await S.from("duties").insert({...P,id:V,created_from:J,display_order:re+1}).select("id").single();D=ae,N=(ce==null?void 0:ce.id)??null}if(!D&&N&&(D=await Wp(N,v)),!D&&N&&(D=await Vp(N,b)),f.disabled=!1,f.innerHTML=K,D){z.length&&await sn(z),g(D.message,"error");return}if(I){const H=$.map(ae=>Ys(ae.url)).filter(Boolean),J=le.map(ae=>Ys(ae.url)).filter(Boolean),re=new Set(J),ce=H.filter(ae=>!re.has(ae));ce.length&&await sn(ce)}g(I?"Повеската е обновена.":"Повеската е създадена.","success"),Ve(e.querySelector("#duty-modal")),qa(e),await os(e)}function Qo(e,t){const r=Ft(t.dutyFiles);e.querySelector("#duty-id").value=t.id,e.querySelector("#duty-name").value=t.name??"",e.querySelector("#duty-type").value=t.dutyTypeId??"";const n=e.querySelector("#duty-schedule-keys"),s=t.scheduleKeyIds||[];Array.from(n.options).forEach(o=>{o.selected=s.includes(o.value)});const a=e.querySelector("#duty-trains"),i=t.trainIds||[];Array.from(a.options).forEach(o=>{o.selected=i.includes(o.value)}),bt(e,t.startTime??"","#duty-start","#duty-start-time"),bt(e,t.endTime??"","#duty-end","#duty-end-time"),e.querySelector("#duty-second-day").checked=!!t.secondDay,bt(e,Ht(t.breakStartTime),"#duty-break-start","#duty-break-start-time"),bt(e,Ht(t.breakEndTime),"#duty-break-end","#duty-break-end-time"),e.querySelector("#duty-notes").value=t.notes??"",e.querySelector("#duty-existing-attachments").value=rn(r)||"",e.querySelector("#duty-draft-attachments").value=rn(r)||"",e.querySelector("#duty-attachment-file").value="",La(e,r),e.querySelector("#duty-form-title").textContent="Редакция на Повеска",e.querySelector("#duty-save-btn").textContent="Запази"}function qa(e){e.querySelector("#duty-id").value="",e.querySelector("#duty-name").value="",e.querySelector("#duty-type").value="";const t=e.querySelector("#duty-schedule-keys");Array.from(t.options).forEach(n=>{n.selected=!1});const r=e.querySelector("#duty-trains");Array.from(r.options).forEach(n=>{n.selected=!1}),bt(e,"","#duty-start","#duty-start-time"),bt(e,"","#duty-end","#duty-end-time"),e.querySelector("#duty-second-day").checked=!1,bt(e,"00:00","#duty-break-start","#duty-break-start-time"),bt(e,"00:00","#duty-break-end","#duty-break-end-time"),e.querySelector("#duty-notes").value="",e.querySelector("#duty-existing-attachments").value="",e.querySelector("#duty-draft-attachments").value="",e.querySelector("#duty-attachment-file").value="",La(e,[]),e.querySelector("#duty-form-title").textContent="Нова Повеска",e.querySelector("#duty-save-btn").textContent="Създай"}async function zp(e,t){const r=t.querySelector("#duty-delete-confirm"),n=r.innerHTML;r.disabled=!0,r.innerHTML='<span class="spinner-border spinner-border-sm me-2" role="status" aria-hidden="true"></span>Изтриване...';const{data:s,error:a}=await S.from("duties").select("duty_files").eq("id",e).maybeSingle();if(a){r.disabled=!1,r.innerHTML=n,g(a.message,"error");return}const{error:i}=await S.from("duties").delete().eq("id",e);if(r.disabled=!1,r.innerHTML=n,i){g(i.message,"error");return}const o=Ft(s==null?void 0:s.duty_files).map(l=>Ys(l.url)).filter(Boolean);o.length&&await sn(o),g("Повеската е изтрита.","success"),Ve(t.querySelector("#duty-delete-modal")),qa(t),await os(t)}async function Wp(e,t){const{error:r}=await S.from("schedule_key_duties").delete().eq("duty_id",e);if(r)return r;const n=t.map(a=>({duty_id:e,schedule_key_id:a})),{error:s}=await S.from("schedule_key_duties").insert(n);return s}async function Vp(e,t){const{error:r}=await S.from("duty_trains").delete().eq("duty_id",e);if(r)return r;if(!t.length)return null;const n=t.map((a,i)=>({duty_id:e,train_id:a,sequence_order:i+1})),{error:s}=await S.from("duty_trains").insert(n);return s}function La(e,t){const r=e.querySelector("#duty-current-attachments-wrap"),n=e.querySelector("#duty-current-attachments-links"),s=e.querySelector("#duty-draft-attachments");if(!r||!n||!s)return;const a=Gn(t);if(s.value=rn(a)||"",!a.length){r.classList.add("d-none"),n.innerHTML="";return}r.classList.remove("d-none"),n.innerHTML=a.map((i,o)=>{const l=i.label||nn(i.url,o);return`
      ...
      `}).join("")}function Ft(e){if(Array.isArray(e))return e.map((r,n)=>Nn(r,n)).filter(r=>r.url);const t=String(e||"").trim();if(!t)return[];if(t.startsWith("["))try{const r=JSON.parse(t);if(Array.isArray(r))return r.map((n,s)=>Nn(n,s)).filter(n=>n.url)}catch{return[{url:t,label:nn(t,0)}]}return t.split(`
`).map((r,n)=>Nn(r,n)).filter(r=>r.url)}function Nn(e,t){if(e&&typeof e=="object"&&!Array.isArray(e)){const n=String(e.url||"").trim(),s=String(e.label||"").trim()||nn(n,t);return{url:n,label:s}}const r=String(e||"").trim();return{url:r,label:nn(r,t)}}function rn(e){const t=Gn(e);return t.length?JSON.stringify(t):""}function Gn(e){const t=[],r=new Set;for(const n of e||[]){const s=Nn(n,t.length);if(!s.url)continue;const a=s.url.toLowerCase();r.has(a)||(r.add(a),t.push(s))}return t}function nn(e,t){const r=String(e||"").trim();if(!r)return`Файл ${t+1}`;try{const s=new URL(r).pathname.split("/").pop()||"",a=decodeURIComponent(s);if(a)return a}catch{}return`Файл ${t+1}`}async function Gp(e,t){var n;if(!Array.isArray(e)||!e.length||!t)return[];const r=[];for(const s of e){const i=(((n=s.name)==null?void 0:n.split(".").pop())||"pdf").toLowerCase().replace(/[^a-z0-9]/g,"")||"pdf",o=Math.random().toString(36).slice(2,10),l=`${t}/${Date.now()}-${o}.${i}`,{error:d}=await S.storage.from(en).upload(l,s,{upsert:!0,contentType:s.type||void 0});if(d)return r.length&&await sn(r.map(u=>u.objectPath)),g(d.message,"error"),null;const{data:c}=S.storage.from(en).getPublicUrl(l);if(!(c!=null&&c.publicUrl))return await sn([l,...r.map(u=>u.objectPath)]),g("Файлът е качен, но не успях да генерирам публичен линк.","error"),null;r.push({url:c.publicUrl,label:s.name||"",objectPath:l})}return r}function Ys(e){const t=String(e||"").trim();if(!t)return"";if(!/^https?:\/\//i.test(t)){const r=t.replace(/^\/+/,""),n=`${en}/`;return r.startsWith(n)?r.slice(n.length):""}try{const r=new URL(t),n=`/storage/v1/object/public/${en}/`,s=r.pathname.indexOf(n);return s===-1?"":decodeURIComponent(r.pathname.slice(s+n.length))}catch{return""}}async function sn(e){const t=Array.from(new Set((e||[]).filter(Boolean)));t.length&&await S.storage.from(en).remove(t)}function Oi(e,t,r){const n=e.querySelector("#duty-attachment-preview-modal"),s=e.querySelector("#duty-attachment-preview-frame"),a=e.querySelector("#duty-attachment-preview-text-wrap"),i=e.querySelector("#duty-attachment-preview-text"),o=e.querySelector("#duty-attachment-preview-csv-wrap"),l=e.querySelector("#duty-attachment-preview-csv-note"),d=e.querySelector("#duty-attachment-preview-csv-head"),c=e.querySelector("#duty-attachment-preview-csv-body"),u=e.querySelector("#duty-attachment-preview-title"),p=e.querySelector("#duty-attachment-preview-fallback"),m=e.querySelector("#duty-attachment-preview-open");if(!n||!s||!a||!i||!o||!l||!d||!c||!u||!p||!m)return;const h=String(t||"").trim();if(!h){g("Липсва линк за преглед.","warning");return}const f=Zp(h),w=Xs(h),y=w==="csv",v=["txt","csv","json"].includes(w);u.textContent=r?`Преглед: ${r}`:"Преглед на файл",m.setAttribute("href",h),p.classList.add("d-none"),a.classList.add("d-none"),o.classList.add("d-none"),l.textContent="",d.innerHTML="",c.innerHTML="",i.textContent="",s.classList.remove("d-none"),s.src="about:blank",y?(o.classList.remove("d-none"),s.classList.add("d-none"),Qp(h,d,c,l,p)):v?(a.classList.remove("d-none"),s.classList.add("d-none"),i.textContent="Зареждане...",Xp(h,i,p)):(s.src=f,s.onload=()=>{if(f!==h){p.classList.add("d-none");return}const b=Xs(h),_=["doc","docx","xls","xlsx","ppt","pptx"].includes(b);p.classList.toggle("d-none",!_)},s.onerror=()=>{p.classList.remove("d-none")}),Ut(n)}function Jp(e){const t=e.querySelector("#duty-attachment-preview-modal"),r=e.querySelector("#duty-attachment-preview-frame"),n=e.querySelector("#duty-attachment-preview-text-wrap"),s=e.querySelector("#duty-attachment-preview-text"),a=e.querySelector("#duty-attachment-preview-csv-wrap"),i=e.querySelector("#duty-attachment-preview-csv-note"),o=e.querySelector("#duty-attachment-preview-csv-head"),l=e.querySelector("#duty-attachment-preview-csv-body"),d=e.querySelector("#duty-attachment-preview-fallback"),c=e.querySelector("#duty-attachment-preview-open");!t||!r||!n||!s||!a||!i||!o||!l||!d||!c||(r.src="about:blank",r.classList.remove("d-none"),n.classList.add("d-none"),a.classList.add("d-none"),s.textContent="",i.textContent="",o.innerHTML="",l.innerHTML="",c.setAttribute("href","#"),d.classList.add("d-none"),Ve(t))}async function Qp(e,t,r,n,s){try{const a=await fetch(e,{cache:"no-store"});if(!a.ok)throw new Error(`HTTP ${a.status}`);const i=await a.text(),o=Yp(i);if(!o.length){t.innerHTML="",r.innerHTML="",n.textContent="Файлът е празен.",s.classList.add("d-none");return}const l=200,d=o.slice(0,l),c=d[0]||[],u=d.slice(1);t.innerHTML=`
      <tr>${c.map(p=>`<th>${pe(p)}</th>`).join("")}</tr>
    `,r.innerHTML=u.map(p=>`<tr>${p.map(m=>`<td>${pe(m)}</td>`).join("")}</tr>`).join(""),o.length>l?n.textContent=`Показани са първите ${l} реда от общо ${o.length}.`:n.textContent=`Редове: ${o.length}.`,s.classList.add("d-none")}catch{t.innerHTML="",r.innerHTML="",n.textContent="",s.classList.remove("d-none")}}function Yp(e){const t=[];let r=[],n="",s=!1;for(let a=0;a<e.length;a+=1){const i=e[a],o=e[a+1];if(i==='"'){s&&o==='"'?(n+='"',a+=1):s=!s;continue}if(!s&&i===","){r.push(n),n="";continue}if(!s&&(i===`
`||i==="\r")){i==="\r"&&o===`
`&&(a+=1),r.push(n),t.push(r),r=[],n="";continue}n+=i}return(n.length||r.length)&&(r.push(n),t.push(r)),t}async function Xp(e,t,r){try{const n=await fetch(e,{cache:"no-store"});if(!n.ok)throw new Error(`HTTP ${n.status}`);const s=await n.text();t.textContent=s||"(Празен файл)",r.classList.add("d-none")}catch{t.textContent="Неуспешно зареждане на текстов преглед.",r.classList.remove("d-none")}}function Zp(e){const t=Xs(e);return["doc","docx","xls","xlsx","ppt","pptx"].includes(t)?`https://view.officeapps.live.com/op/embed.aspx?src=${encodeURIComponent(e)}`:e}function Xs(e){const t=String(e||"").trim();if(!t)return"";try{const n=new URL(t).pathname.split("/").pop()||"",s=n.includes(".")?n.split(".").pop():"";return String(s||"").toLowerCase()}catch{return""}}function eh(e){const t=String((e==null?void 0:e.message)||"").trim(),r=t.toLowerCase(),n=r.includes("row-level security")||r.includes("violates row-level security policy")||String((e==null?void 0:e.code)||"")==="42501";return n&&r.includes("duty_trains")?"Нямаш права да свързваш влакове към повески. Свържи се с администратор.":n&&r.includes("duties")?"Нямаш права да създаваш или редактираш повески. Свържи се с администратор.":n?"Достъпът е ограничен от права за сигурност (RLS).":t||"Възникна неочаквана грешка."}function th(e,t){const r=Q.allDuties.find(c=>c.id===t),n=e.querySelector("#duty-profile-content"),s=e.querySelector("#duty-profile-modal"),a=e.querySelector("#duty-profile-duplicate"),i=e.querySelector("#duty-profile-edit");if(!n||!s)return;if(!r){s.dataset.dutyId="",i&&(i.disabled=!0),a&&(a.disabled=!0),n.innerHTML='<p class="text-secondary mb-0">Няма данни за тази повеска.</p>',Ut(s);return}s.dataset.dutyId=r.id,i&&(i.disabled=!1),a&&(a.disabled=!1);const o=rh(r),l=nh(r),d=Ft(r==null?void 0:r.duty_files);n.innerHTML=Vo({duty:r,scheduleKeyNames:o,trainNumbers:l,attachmentEntries:d,escapeHtml:pe,intervalToTimeInput:Ht,formatInterval:sh}),Ut(s)}function Pi(e,t){const r=Q.allDuties.find(n=>n.id===t);if(!r){g("Не е намерена повеска за редакция.","warning");return}Qo(e,{id:r.id,name:r.name||"",dutyTypeId:r.duty_type_id||"",scheduleKeyIds:Yo(r),trainIds:Xo(r),startTime:Jn(r.start_time),endTime:Jn(r.end_time),secondDay:!!r.second_day,breakStartTime:r.break_start_time||"00:00:00",breakEndTime:r.break_end_time||"00:00:00",notes:r.notes||"",dutyFiles:r.duty_files||""}),Ut(e.querySelector("#duty-modal"))}function Mi(e,t){const r=Q.allDuties.find(n=>n.id===t);if(!r){g("Не е намерена повеска за копиране.","warning");return}Qo(e,{id:"",name:r.name?`${r.name} (копие)`:"",dutyTypeId:r.duty_type_id||"",scheduleKeyIds:Yo(r),trainIds:Xo(r),startTime:Jn(r.start_time),endTime:Jn(r.end_time),secondDay:!!r.second_day,breakStartTime:r.break_start_time||"00:00:00",breakEndTime:r.break_end_time||"00:00:00",notes:r.notes||"",dutyFiles:r.duty_files||""}),e.querySelector("#duty-id").value="",e.querySelector("#duty-form-title").textContent="Нова Повеска (копие)",e.querySelector("#duty-save-btn").textContent="Създай",Ut(e.querySelector("#duty-modal"))}function rh(e){const r=(Array.isArray(e==null?void 0:e.schedule_key_duties)?e.schedule_key_duties:e!=null&&e.schedule_key_duties?[e.schedule_key_duties]:[]).map(n=>{var s;return(s=n==null?void 0:n.schedule_keys)==null?void 0:s.name}).filter(Boolean);return[...new Set(r)]}function Yo(e){const r=(Array.isArray(e==null?void 0:e.schedule_key_duties)?e.schedule_key_duties:e!=null&&e.schedule_key_duties?[e.schedule_key_duties]:[]).map(n=>n==null?void 0:n.schedule_key_id).filter(Boolean);return[...new Set(r)]}function nh(e){return(Array.isArray(e==null?void 0:e.duty_trains)?e.duty_trains:e!=null&&e.duty_trains?[e.duty_trains]:[]).map(r=>{var n;return{number:(n=r==null?void 0:r.trains)==null?void 0:n.number,sequenceOrder:Number.isFinite(Number(r==null?void 0:r.sequence_order))?Number(r.sequence_order):Number.MAX_SAFE_INTEGER}}).filter(r=>!!r.number).sort((r,n)=>r.sequenceOrder-n.sequenceOrder).map(r=>r.number).filter((r,n,s)=>s.indexOf(r)===n)}function Xo(e){return(Array.isArray(e==null?void 0:e.duty_trains)?e.duty_trains:e!=null&&e.duty_trains?[e.duty_trains]:[]).map(r=>({id:r==null?void 0:r.train_id,sequenceOrder:Number.isFinite(Number(r==null?void 0:r.sequence_order))?Number(r.sequence_order):Number.MAX_SAFE_INTEGER})).filter(r=>!!r.id).sort((r,n)=>r.sequenceOrder-n.sequenceOrder).map(r=>r.id).filter((r,n,s)=>s.indexOf(r)===n)}function Jn(e){return e?String(e).slice(0,5):""}function Nr(e,...t){for(const r of t){const n=e.querySelector(r);if(n)return n}return null}function bt(e,t,...r){const n=Nr(e,...r);n&&(n.value=t)}function sh(e){return e?String(e).replace(".000000",""):"-"}const ah=`<section class="card border-0 shadow-sm">\r
  <div class="card-body p-4 p-md-5">\r
    <div class="d-flex justify-content-between align-items-center mb-3">\r
...
`;function Hn(e){e.classList.remove("d-none"),document.body.classList.add("overflow-hidden")}const ji=new Map;function ih(e,t){const r=ji.get(e);r&&document.removeEventListener("keydown",r);const n=s=>{if(s.key==="Escape"){for(const a of t)if(a&&!a.classList.contains("d-none")){yt(a);return}}};ji.set(e,n),document.addEventListener("keydown",n)}function yt(e){var t,r,n;e.classList.add("d-none"),(t=document.querySelector("#employee-modal"))!=null&&t.classList.contains("d-none")&&((r=document.querySelector("#employee-delete-modal"))!=null&&r.classList.contains("d-none"))&&((n=document.querySelector("#employee-profile-modal"))!=null&&n.classList.contains("d-none"))&&document.body.classList.remove("overflow-hidden")}function Oe(e){return String(e).replaceAll("&","&amp;").replaceAll("<","&lt;").replaceAll(">","&gt;").replaceAll('"',"&quot;").replaceAll("'","&#039;")}const Se={rows:[],searchQuery:"",positionFilter:"",activeFilter:""};async function Ta(e){const{data:t,error:r}=await S.from("employees").select("id, first_name, last_name, position_id, is_active, photo_url, psychological_assessment_expiry, medical_certificate_expiry, license_expiry, positions(title), user_profiles(id, username)").order("last_name",{ascending:!0}).order("first_name",{ascending:!0});if(r){g(r.message,"error"),Se.rows=[],lr(e,"Грешка при зареждане на служители.");return}Se.rows=t||[],lr(e)}function lr(e,t){const r=e.querySelector("#employees-table-body"),n=e.querySelector("#employees-empty");oh(e);const s=Se.rows.filter(a=>{var u;const i=`${a.first_name||""} ${a.last_name||""}`.toLowerCase(),o=(((u=a.positions)==null?void 0:u.title)||"").toLowerCase(),l=!Se.searchQuery||i.includes(Se.searchQuery)||o.includes(Se.searchQuery),d=!Se.positionFilter||o===Se.positionFilter,c=!Se.activeFilter||String(!!a.is_active)===Se.activeFilter;return l&&d&&c});if(!s.length){r.innerHTML="",n.classList.remove("d-none"),n.textContent=t||"Няма въведени служители.";return}n.classList.add("d-none"),r.innerHTML=s.map(a=>{var l;const i=Array.isArray(a.user_profiles)?a.user_profiles:a.user_profiles?[a.user_profiles]:[],o=i.length?i.map(d=>{const c=(d==null?void 0:d.username)||(d==null?void 0:d.id)||"";return c||""}).filter(Boolean).join(", "):"-";return`
      ...
      `}).join("")}function oh(e){const t=e.querySelector("#employees-position-filter");if(!t)return;const r=Se.positionFilter||"",n=[...new Set(Se.rows.map(s=>{var a;return String(((a=s==null?void 0:s.positions)==null?void 0:a.title)||"").trim()}).filter(Boolean))].sort((s,a)=>s.localeCompare(a,"bg"));t.innerHTML=`
    <option value="">Всички</option>
    ${n.map(s=>`<option value="${Oe(s.toLowerCase())}">${Oe(s)}</option>`).join("")}
  `,t.value=r}const Ni="employee-photos";let Tr=null;async function lh(e){e.innerHTML=ah,ch(e),await uh(e),await Ta(e);const t=dh();t&&await Zo(e,t)}function dh(){return new URLSearchParams(window.location.search).get("profile")||""}function ch(e){const t=e.querySelector("#open-create-employee"),r=e.querySelector("#employee-form"),n=e.querySelector("#employee-cancel-btn"),s=e.querySelector("#employees-table-body"),a=e.querySelector("#employee-modal"),i=e.querySelector("#employee-delete-modal"),o=e.querySelector("#employee-profile-modal"),l=e.querySelector("#employee-modal-close"),d=e.querySelector("#employee-profile-close"),c=e.querySelector("#employee-profile-close-btn"),u=e.querySelector("#employee-delete-confirm"),p=e.querySelector("#employee-delete-cancel"),m=e.querySelector("#employees-search"),h=e.querySelector("#employees-position-filter"),f=e.querySelector("#employees-active-filter"),w=e.querySelector("#employees-filter-reset"),y=e.querySelector("#employee-photo-file"),v=e.querySelector("#employee-photo-remove-btn");t==null||t.addEventListener("click",()=>{Ea(e),Hn(a)}),r==null||r.addEventListener("submit",async b=>{b.preventDefault(),await ph(e)}),n==null||n.addEventListener("click",()=>{yt(a)}),l==null||l.addEventListener("click",()=>{yt(a)}),p==null||p.addEventListener("click",()=>{yt(i)}),d==null||d.addEventListener("click",()=>{yt(o)}),c==null||c.addEventListener("click",()=>{yt(o)}),m==null||m.addEventListener("input",b=>{Se.searchQuery=b.target.value.trim().toLowerCase(),lr(e)}),h==null||h.addEventListener("change",b=>{Se.positionFilter=b.target.value||"",lr(e)}),f==null||f.addEventListener("change",b=>{Se.activeFilter=b.target.value||"",lr(e)}),w==null||w.addEventListener("click",()=>{Se.searchQuery="",Se.positionFilter="",Se.activeFilter="",m&&(m.value=""),h&&(h.value=""),f&&(f.value=""),lr(e)}),y==null||y.addEventListener("change",()=>{var _,k,x,L;const b=((_=y.files)==null?void 0:_[0])??null;if(!b){const q=((k=e.querySelector("#employee-photo-path"))==null?void 0:k.value)||"";dr(e,q);return}if(!((x=b.type)!=null&&x.startsWith("image/"))){g("Избери валиден файл с изображение.","warning"),y.value="";const q=((L=e.querySelector("#employee-photo-path"))==null?void 0:L.value)||"";dr(e,q);return}dr(e,b)}),v==null||v.addEventListener("click",()=>{const b=e.querySelector("#employee-photo-path");b&&(b.value=""),y&&(y.value=""),dr(e,null)}),ih("employees",[i,o,a]),u==null||u.addEventListener("click",async()=>{const b=e.querySelector("#employee-delete-id").value;await mh(b,e)}),s==null||s.addEventListener("click",async b=>{const _=b.target.closest("button[data-action]");if(!_)return;const k=_.getAttribute("data-action");if(k==="profile"){const x=_.getAttribute("data-id");await Zo(e,x);return}if(k==="edit"){hh(e,{id:_.getAttribute("data-id"),firstName:_.getAttribute("data-first-name"),lastName:_.getAttribute("data-last-name"),positionId:_.getAttribute("data-position-id"),isActive:_.getAttribute("data-active")==="true",psychExpiry:_.getAttribute("data-psych-expiry"),medicalExpiry:_.getAttribute("data-medical-expiry"),licenseExpiry:_.getAttribute("data-license-expiry"),photoUrl:_.getAttribute("data-photo-url")}),Hn(a);return}if(k==="delete"){const x=_.getAttribute("data-id");e.querySelector("#employee-delete-id").value=x,Hn(i)}})}async function Zo(e,t){var c;const{data:r,error:n}=await S.from("employees").select("id, first_name, last_name, is_active, photo_url, psychological_assessment_expiry, medical_certificate_expiry, license_expiry, other_certificates, positions(title), user_profiles(id, username)").eq("id",t).maybeSingle();if(n||!r){g((n==null?void 0:n.message)||"Служителят не е намерен.","error");return}const s=Array.isArray(r.user_profiles)?r.user_profiles:r.user_profiles?[r.user_profiles]:[],a=s.length?s.map(u=>{const p=(u==null?void 0:u.username)||(u==null?void 0:u.id)||"";return p||""}).filter(Boolean).join(", "):"-",i=r.other_certificates?JSON.stringify(r.other_certificates,null,2):"-",o=el(r.photo_url),l=e.querySelector("#employee-profile-photo"),d=e.querySelector("#employee-profile-photo-empty");l&&d&&(o?(l.src=o,l.classList.remove("d-none"),d.classList.add("d-none")):(l.src="",l.classList.add("d-none"),d.classList.remove("d-none"))),e.querySelector("#employee-profile-name").textContent=`${r.first_name??""} ${r.last_name??""}`.trim()||"-",e.querySelector("#employee-profile-position").textContent=((c=r.positions)==null?void 0:c.title)||"-",e.querySelector("#employee-profile-active").textContent=r.is_active?"Да":"Не",e.querySelector("#employee-profile-linked-profiles").textContent=a,e.querySelector("#employee-profile-psych").textContent=r.psychological_assessment_expiry||"-",e.querySelector("#employee-profile-medical").textContent=r.medical_certificate_expiry||"-",e.querySelector("#employee-profile-license").textContent=r.license_expiry||"-",e.querySelector("#employee-profile-certificates").textContent=i,Hn(e.querySelector("#employee-profile-modal"))}function el(e){if(!e)return null;const t=String(e).trim();if(!t)return null;if(/^https?:\/\//i.test(t))return t;const r=t.replace(/^\/+/,""),n=["employee-photos","employees","employee_photos"],s=[],a=r.split("/");if(a.length>1){const o=a[0],l=a.slice(1).join("/");s.push({bucket:o,objectPath:l})}n.forEach(o=>{s.push({bucket:o,objectPath:r})});const i=new Set;for(const o of s){const l=`${o.bucket}/${o.objectPath}`;if(i.has(l)||!o.bucket||!o.objectPath)continue;i.add(l);const{data:d}=S.storage.from(o.bucket).getPublicUrl(o.objectPath);if(d!=null&&d.publicUrl)return d.publicUrl}return null}async function uh(e){const t=e.querySelector("#employee-position"),{data:r,error:n}=await S.from("positions").select("id, title").order("title",{ascending:!0});if(n){g(n.message,"error");return}const s=(r||[]).map(a=>`<option value="${a.id}">${Oe(a.title)}</option>`).join("");t.innerHTML='<option value="">Без позиция</option>'+s}async function ph(e){var $,A,C;const t=e.querySelector("#employee-id"),r=e.querySelector("#employee-first-name"),n=e.querySelector("#employee-last-name"),s=e.querySelector("#employee-position"),a=e.querySelector("#employee-active"),i=e.querySelector("#employee-psych-expiry"),o=e.querySelector("#employee-medical-expiry"),l=e.querySelector("#employee-license-expiry"),d=e.querySelector("#employee-photo-file"),c=e.querySelector("#employee-photo-path"),u=e.querySelector("#employee-save-btn"),p=r.value.trim(),m=n.value.trim(),h=s.value||null,f=a.checked,w=i.value||null,y=o.value||null,v=l.value||null,b=(($=d.files)==null?void 0:$[0])??null,_=c.value.trim()||null,k=t.value;if(!p||!m){g("Моля, попълни име и фамилия.","warning");return}const x=u.innerHTML;u.disabled=!0,u.innerHTML='<span class="spinner-border spinner-border-sm me-2" role="status" aria-hidden="true"></span>Запис...';const L={first_name:p,last_name:m,position_id:h,is_active:f,psychological_assessment_expiry:w,medical_certificate_expiry:y,license_expiry:v,updated_at:new Date().toISOString()};if(b&&!((A=b.type)!=null&&A.startsWith("image/"))){u.disabled=!1,u.innerHTML=x,g("Избери валиден файл с изображение.","warning");return}let q,T=k||null,E=_;if(k){if(b){const I=await Hi(b,k);if(!I){u.disabled=!1,u.innerHTML=x;return}E=I}L.photo_url=E,{error:q}=await S.from("employees").update(L).eq("id",k)}else{const{data:I}=await S.auth.getUser(),R=((C=I==null?void 0:I.user)==null?void 0:C.email)??"web_app",{data:W,error:K}=await S.from("employees").insert({...L,created_from:R}).select("id").single();if(q=K,T=(W==null?void 0:W.id)??null,!q&&T&&b){const V=await Hi(b,T);if(!V){u.disabled=!1,u.innerHTML=x;return}const{error:j}=await S.from("employees").update({photo_url:V,updated_at:new Date().toISOString()}).eq("id",T);q=j,E=V}}if(u.disabled=!1,u.innerHTML=x,q){g(q.message,"error");return}g(k?"Служителят е обновен.":"Служителят е създаден.","success"),yt(e.querySelector("#employee-modal")),Ea(e),await Ta(e)}function hh(e,t){e.querySelector("#employee-id").value=t.id,e.querySelector("#employee-first-name").value=t.firstName??"",e.querySelector("#employee-last-name").value=t.lastName??"",e.querySelector("#employee-position").value=t.positionId??"",e.querySelector("#employee-active").checked=!!t.isActive,e.querySelector("#employee-psych-expiry").value=t.psychExpiry??"",e.querySelector("#employee-medical-expiry").value=t.medicalExpiry??"",e.querySelector("#employee-license-expiry").value=t.licenseExpiry??"",e.querySelector("#employee-photo-path").value=t.photoUrl??"",e.querySelector("#employee-photo-file").value="",dr(e,t.photoUrl??null),e.querySelector("#employee-form-title").textContent="Редакция на служител",e.querySelector("#employee-save-btn").textContent="Запази"}function Ea(e){e.querySelector("#employee-id").value="",e.querySelector("#employee-first-name").value="",e.querySelector("#employee-last-name").value="",e.querySelector("#employee-position").value="",e.querySelector("#employee-active").checked=!0,e.querySelector("#employee-psych-expiry").value="",e.querySelector("#employee-medical-expiry").value="",e.querySelector("#employee-license-expiry").value="",e.querySelector("#employee-photo-path").value="",e.querySelector("#employee-photo-file").value="",dr(e,null),e.querySelector("#employee-form-title").textContent="Нов служител",e.querySelector("#employee-save-btn").textContent="Създай"}async function mh(e,t){const r=t.querySelector("#employee-delete-confirm"),n=r.innerHTML;r.disabled=!0,r.innerHTML='<span class="spinner-border spinner-border-sm me-2" role="status" aria-hidden="true"></span>Изтриване...';const{error:s}=await S.from("employees").delete().eq("id",e);if(r.disabled=!1,r.innerHTML=n,s){g(s.message,"error");return}g("Служителят е изтрит.","success"),yt(t.querySelector("#employee-delete-modal")),Ea(t),await Ta(t)}async function Hi(e,t){var o;if(!e||!t)return null;const n=(((o=e.name)==null?void 0:o.split(".").pop())||"jpg").toLowerCase().replace(/[^a-z0-9]/g,"")||"jpg",s=Math.random().toString(36).slice(2,10),a=`${t}/${Date.now()}-${s}.${n}`,{error:i}=await S.storage.from(Ni).upload(a,e,{upsert:!0,contentType:e.type||void 0});return i?(g(i.message,"error"),null):`${Ni}/${a}`}function dr(e,t){const r=e.querySelector("#employee-photo-preview"),n=e.querySelector("#employee-photo-preview-empty");if(!r||!n)return;if(Tr&&(URL.revokeObjectURL(Tr),Tr=null),!t){r.src="",r.classList.add("d-none"),n.classList.remove("d-none");return}if(t instanceof File){Tr=URL.createObjectURL(t),r.src=Tr,r.classList.remove("d-none"),n.classList.add("d-none");return}const s=el(t);if(!s){r.src="",r.classList.add("d-none"),n.classList.remove("d-none");return}r.src=s,r.classList.remove("d-none"),n.classList.add("d-none")}const fh=`<section class="card border-0 shadow-sm">\r
  <div class="card-body p-4 p-md-5">\r
    <div class="d-flex justify-content-between align-items-center mb-3">\r
...
`;function xs(e){e.classList.remove("d-none"),document.body.classList.add("overflow-hidden")}const Ui=new Map;function yh(e,t){const r=Ui.get(e);r&&document.removeEventListener("keydown",r);const n=s=>{if(s.key==="Escape"){for(const a of t)if(a&&!a.classList.contains("d-none")){br(a);return}}};Ui.set(e,n),document.addEventListener("keydown",n)}function br(e){var t,r;e.classList.add("d-none"),(t=document.querySelector("#employee-absence-modal"))!=null&&t.classList.contains("d-none")&&((r=document.querySelector("#employee-absence-delete-modal"))!=null&&r.classList.contains("d-none"))&&document.body.classList.remove("overflow-hidden")}function Qe(e){return String(e).replaceAll("&","&amp;").replaceAll("<","&lt;").replaceAll(">","&gt;").replaceAll('"',"&quot;").replaceAll("'","&#039;")}const ge={rows:[],searchQuery:"",dateFrom:"",dateTo:""};function Fi(e){return e&&`${e.first_name??""} ${e.last_name??""}`.trim()||"-"}async function $a(e){const{data:t,error:r}=await S.from("employee_absences").select("id, employee_id, reason_id, start_date, end_date, notes, employees(first_name, last_name), absence_reasons(name)").order("start_date",{ascending:!1}).order("end_date",{ascending:!1});if(r){g(r.message,"error"),ge.rows=[],cr(e,"Грешка при зареждане на отсъствията.");return}ge.rows=t||[],cr(e)}function cr(e,t){const r=e.querySelector("#employee-absences-table-body"),n=e.querySelector("#employee-absences-empty"),s=ge.rows.filter(a=>{var h;const i=Fi(a.employees).toLowerCase(),o=(((h=a.absence_reasons)==null?void 0:h.name)||"").toLowerCase(),l=String(a.start_date||"").toLowerCase(),d=String(a.end_date||"").toLowerCase(),c=String(a.notes||"").toLowerCase(),u=!ge.searchQuery||i.includes(ge.searchQuery)||o.includes(ge.searchQuery)||l.includes(ge.searchQuery)||d.includes(ge.searchQuery)||c.includes(ge.searchQuery),p=!ge.dateFrom||String(a.end_date||"")>=ge.dateFrom,m=!ge.dateTo||String(a.start_date||"")<=ge.dateTo;return u&&p&&m});if(!s.length){r.innerHTML="",n.classList.remove("d-none"),n.textContent=t||"Няма въведени отсъствия.";return}n.classList.add("d-none"),r.innerHTML=s.map(a=>{var i;return`
        <tr>
          <td>${Qe(Fi(a.employees))}</td>
      ...
      `}).join("")}async function bh(e){e.innerHTML=fh,vh(e),await gh(e),await _h(e),await $a(e)}function vh(e){const t=e.querySelector("#open-create-employee-absence"),r=e.querySelector("#employee-absence-form"),n=e.querySelector("#employee-absence-cancel-btn"),s=e.querySelector("#employee-absences-table-body"),a=e.querySelector("#employee-absence-modal"),i=e.querySelector("#employee-absence-delete-modal"),o=e.querySelector("#employee-absence-modal-close"),l=e.querySelector("#employee-absence-delete-confirm"),d=e.querySelector("#employee-absence-delete-cancel"),c=e.querySelector("#employee-absences-search"),u=e.querySelector("#employee-absences-date-from"),p=e.querySelector("#employee-absences-date-to"),m=e.querySelector("#employee-absences-filter-reset");t==null||t.addEventListener("click",()=>{Aa(e),xs(a)}),r==null||r.addEventListener("submit",async h=>{h.preventDefault(),await wh(e)}),n==null||n.addEventListener("click",()=>{br(a)}),o==null||o.addEventListener("click",()=>{br(a)}),d==null||d.addEventListener("click",()=>{br(i)}),c==null||c.addEventListener("input",h=>{ge.searchQuery=h.target.value.trim().toLowerCase(),cr(e)}),u==null||u.addEventListener("change",h=>{ge.dateFrom=h.target.value||"",cr(e)}),p==null||p.addEventListener("change",h=>{ge.dateTo=h.target.value||"",cr(e)}),m==null||m.addEventListener("click",()=>{ge.searchQuery="",ge.dateFrom="",ge.dateTo="",c&&(c.value=""),u&&(u.value=""),p&&(p.value=""),cr(e)}),yh("employee-absences",[i,a]),l==null||l.addEventListener("click",async()=>{const h=e.querySelector("#employee-absence-delete-id").value;await kh(h,e)}),s==null||s.addEventListener("click",h=>{const f=h.target.closest("button[data-action]");if(!f)return;const w=f.getAttribute("data-action");if(w==="edit"){Sh(e,{id:f.getAttribute("data-id"),employeeId:f.getAttribute("data-employee-id"),reasonId:f.getAttribute("data-reason-id"),startDate:f.getAttribute("data-start-date"),endDate:f.getAttribute("data-end-date"),notes:f.getAttribute("data-notes")}),xs(a);return}if(w==="delete"){const y=f.getAttribute("data-id");e.querySelector("#employee-absence-delete-id").value=y,xs(i)}})}async function gh(e){const t=e.querySelector("#employee-absence-employee"),{data:r,error:n}=await S.from("employees").select("id, first_name, last_name").order("last_name",{ascending:!0}).order("first_name",{ascending:!0});if(n){g(n.message,"error");return}const s=(r||[]).map(a=>{const i=`${a.first_name??""} ${a.last_name??""}`.trim()||"-";return`<option value="${a.id}">${Qe(i)}</option>`}).join("");t.innerHTML='<option value="">Избери служител</option>'+s}async function _h(e){const t=e.querySelector("#employee-absence-reason"),{data:r,error:n}=await S.from("absence_reasons").select("id, name").order("name",{ascending:!0});if(n){g(n.message,"error");return}const s=(r||[]).map(a=>`<option value="${a.id}">${Qe(a.name??"-")}</option>`).join("");t.innerHTML='<option value="">Избери причина</option>'+s}async function wh(e){var y;const t=e.querySelector("#employee-absence-id"),r=e.querySelector("#employee-absence-employee"),n=e.querySelector("#employee-absence-reason"),s=e.querySelector("#employee-absence-start-date"),a=e.querySelector("#employee-absence-end-date"),i=e.querySelector("#employee-absence-notes"),o=e.querySelector("#employee-absence-save-btn"),l=r.value||null,d=n.value||null,c=s.value,u=a.value,p=i.value.trim()||null,m=t.value;if(!l||!d||!c||!u){g("Моля, попълни всички задължителни полета.","warning");return}if(u<c){g('Полето "До дата" трябва да е след или равно на "От дата".',"warning");return}const h=o.innerHTML;o.disabled=!0,o.innerHTML='<span class="spinner-border spinner-border-sm me-2" role="status" aria-hidden="true"></span>Запис...';const f={employee_id:l,reason_id:d,start_date:c,end_date:u,notes:p};let w;if(m)({error:w}=await S.from("employee_absences").update(f).eq("id",m));else{const{data:v}=await S.auth.getUser(),b=((y=v==null?void 0:v.user)==null?void 0:y.email)??"web_app";({error:w}=await S.from("employee_absences").insert({...f,created_from:b}))}if(o.disabled=!1,o.innerHTML=h,w){g(w.message,"error");return}g(m?"Отсъствието е обновено.":"Отсъствието е създадено.","success"),br(e.querySelector("#employee-absence-modal")),Aa(e),await $a(e)}function Sh(e,t){e.querySelector("#employee-absence-id").value=t.id,e.querySelector("#employee-absence-employee").value=t.employeeId??"",e.querySelector("#employee-absence-reason").value=t.reasonId??"",e.querySelector("#employee-absence-start-date").value=t.startDate??"",e.querySelector("#employee-absence-end-date").value=t.endDate??"",e.querySelector("#employee-absence-notes").value=t.notes??"",e.querySelector("#employee-absence-form-title").textContent="Редакция на отсъствие",e.querySelector("#employee-absence-save-btn").textContent="Запази"}function Aa(e){e.querySelector("#employee-absence-id").value="",e.querySelector("#employee-absence-employee").value="",e.querySelector("#employee-absence-reason").value="",e.querySelector("#employee-absence-start-date").value="",e.querySelector("#employee-absence-end-date").value="",e.querySelector("#employee-absence-notes").value="",e.querySelector("#employee-absence-form-title").textContent="Ново отсъствие",e.querySelector("#employee-absence-save-btn").textContent="Създай"}async function kh(e,t){const r=t.querySelector("#employee-absence-delete-confirm"),n=r.innerHTML;r.disabled=!0,r.innerHTML='<span class="spinner-border spinner-border-sm me-2" role="status" aria-hidden="true"></span>Изтриване...';const{error:s}=await S.from("employee_absences").delete().eq("id",e);if(r.disabled=!1,r.innerHTML=n,s){g(s.message,"error");return}g("Отсъствието е изтрито.","success"),br(t.querySelector("#employee-absence-delete-modal")),Aa(t),await $a(t)}const xh=`<section class="card border-0 shadow-sm">\r
  <div class="card-body p-4 p-md-5">\r
...
`;function Hr(e){e.classList.remove("d-none"),document.body.classList.add("overflow-hidden")}const Bi=new Map;function qh(e,t){const r=Bi.get(e);r&&document.removeEventListener("keydown",r);const n=s=>{if(s.key==="Escape"){for(const a of t)if(a&&!a.classList.contains("d-none")){Ue(a);return}}};Bi.set(e,n),document.addEventListener("keydown",n)}function Ue(e){var t,r,n,s;e.classList.add("d-none"),(t=document.querySelector("#planned-duty-modal"))!=null&&t.classList.contains("d-none")&&((r=document.querySelector("#planned-duty-delete-modal"))!=null&&r.classList.contains("d-none"))&&((n=document.querySelector("#planned-duty-auto-modal"))!=null&&n.classList.contains("d-none"))&&((s=document.querySelector("#planned-duty-bulk-delete-modal"))!=null&&s.classList.contains("d-none"))&&document.body.classList.remove("overflow-hidden")}function rt(e){return String(e).replaceAll("&","&amp;").replaceAll("<","&lt;").replaceAll(">","&gt;").replaceAll('"',"&quot;").replaceAll("'","&#039;")}const B={rows:[],searchQuery:"",dateFilter:"",roleFilter:"",selectedIds:[],visibleRowIds:[],selectionEnabled:!0};function Ki(e){return e&&`${e.first_name??""} ${e.last_name??""}`.trim()||"-"}async function vr(e){const{data:t,error:r}=await S.from("planned_duties").select("id, date, employee_id, duty_id, assignment_role, employees(first_name, last_name), duties(name, schedule_key_duties(schedule_key_id))").order("date",{ascending:!1});if(r){g(r.message,"error"),B.rows=[],dt(e,"Грешка при зареждане на планираните повески.");return}B.rows=t||[],dt(e)}function dt(e,t){var c,u;const r=e.querySelector("#planned-duties-table-body"),n=e.querySelector("#planned-duties-empty"),s=e.querySelector("#planned-duties-select-all"),a=e.querySelector("#open-bulk-delete-planned-duty"),i=e.querySelector("#add-selected-to-actual-duty"),o=B.selectionEnabled!==!1;o||(B.selectedIds=[]),B.selectedIds=B.selectedIds.filter(p=>B.rows.some(m=>m.id===p));const l=B.rows.filter(p=>{var b;const m=Ki(p.employees).toLowerCase(),h=(((b=p.duties)==null?void 0:b.name)||"").toLowerCase(),f=(p.date||"").toLowerCase(),w=!B.searchQuery||m.includes(B.searchQuery)||h.includes(B.searchQuery)||f.includes(B.searchQuery),y=!B.dateFilter||p.date===B.dateFilter,v=!B.roleFilter||p.assignment_role===B.roleFilter;return w&&y&&v});if(!l.length){B.visibleRowIds=[],r.innerHTML="",n.classList.remove("d-none"),n.textContent=t||"Няма планирани повески.",s&&(s.checked=!1,s.indeterminate=!1,s.disabled=!o,(c=s.closest("th"))==null||c.classList.toggle("d-none",!o)),a&&(a.disabled=!o||B.selectedIds.length===0,a.textContent=B.selectedIds.length?`Изтрий избраните (${B.selectedIds.length})`:"Изтрий избраните"),i&&(i.disabled=!o||B.selectedIds.length===0,i.textContent=B.selectedIds.length?`Към Актуални (${B.selectedIds.length})`:"Към Актуални");return}B.visibleRowIds=l.map(p=>p.id),n.classList.add("d-none"),s&&(s.disabled=!o,(u=s.closest("th"))==null||u.classList.toggle("d-none",!o)),r.innerHTML=l.map(p=>{var f;const m=Lh(p),h=o&&B.selectedIds.includes(p.id);return`
        <tr>
          ${o?`
          <td>
            <input
          ...
          `:""}
          <td>${rt(p.date??"-")}</td>
          <td>${rt(Ki(p.employees))}</td>
          <td>${rt(Th(p.assignment_role))}</td>
      ...
      `}).join("");const d=l.filter(p=>B.selectedIds.includes(p.id)).length;s&&(s.disabled=!o,s.checked=o&&d>0&&d===l.length,s.indeterminate=o&&d>0&&d<l.length),a&&(a.disabled=!o||B.selectedIds.length===0,a.textContent=B.selectedIds.length?`Изтрий избраните (${B.selectedIds.length})`:"Изтрий избраните"),i&&(i.disabled=!o||B.selectedIds.length===0,i.textContent=B.selectedIds.length?`Към Актуални (${B.selectedIds.length})`:"Към Актуални")}function Lh(e){var r,n,s;return((s=(Array.isArray((r=e==null?void 0:e.duties)==null?void 0:r.schedule_key_duties)?e.duties.schedule_key_duties:(n=e==null?void 0:e.duties)!=null&&n.schedule_key_duties?[e.duties.schedule_key_duties]:[]).find(a=>a==null?void 0:a.schedule_key_id))==null?void 0:s.schedule_key_id)||""}function Th(e){return e==="chief"?"Началник влак":"Кондуктор"}let Ca=[];async function Eh(e){const t=e.querySelector("#planned-duty-employee"),r=e.querySelector("#planned-duty-auto-employee"),{data:n,error:s}=await S.from("employees").select("id, first_name, last_name").order("last_name",{ascending:!0}).order("first_name",{ascending:!0});if(s){g(s.message,"error");return}const i='<option value="">Избери служител</option>'+(n||[]).map(o=>{const l=`${o.first_name??""} ${o.last_name??""}`.trim()||"-";return`<option value="${o.id}">${rt(l)}</option>`}).join("");t.innerHTML=i,r.innerHTML=i}async function $h(e){const t=e.querySelector("#planned-duty-schedule-key"),r=e.querySelector("#planned-duty-auto-schedule-key"),{data:n,error:s}=await S.from("schedule_keys").select("id, name, crew_role").order("name",{ascending:!0});if(s){g(s.message,"error");return}const i='<option value="">Избери ключ-график</option>'+(n||[]).map(o=>{const l=Ah(o.crew_role),d=l?`${o.name??"-"} (${l})`:o.name??"-";return`<option value="${o.id}">${rt(d)}</option>`}).join("");t.innerHTML=i,r.innerHTML=i}function Ah(e){return e==="началник влак"?"Началник влак":e==="кондуктор"?"Кондуктор":""}async function Ch(e){const{data:t,error:r}=await S.from("schedule_key_duties").select("schedule_key_id, duty_id, duties(id, name)");if(r){g(r.message,"error");return}const n=new Map;(t||[]).forEach(s=>{const a=s==null?void 0:s.duties;if(!(a!=null&&a.id))return;const i=n.get(a.id)||{id:a.id,name:a.name||"-",scheduleKeyIds:[]};s.schedule_key_id&&!i.scheduleKeyIds.includes(s.schedule_key_id)&&i.scheduleKeyIds.push(s.schedule_key_id),n.set(a.id,i)}),Ca=Array.from(n.values()).sort((s,a)=>String(s.name||"").localeCompare(String(a.name||""),"bg")),ls(e,"","")}function ls(e,t,r){const n=e.querySelector("#planned-duty-duty");if(!n)return;if(!t){n.innerHTML='<option value="">Първо избери ключ-график</option>',n.value="";return}const s=Ca.filter(a=>{var i;return(i=a.scheduleKeyIds)==null?void 0:i.includes(t)}).map(a=>{const i=a.id===r?"selected":"";return`<option value="${a.id}" ${i}>${rt(a.name??"-")}</option>`}).join("");n.innerHTML='<option value="">Избери повеска</option>'+s,r&&(n.value=r)}function Rh(e,t){var n;const r=Ca.find(s=>s.id===e);return!!(r&&((n=r.scheduleKeyIds)!=null&&n.includes(t)))}const qs=new Map;async function zi(e,t,r){const n=e.querySelector("#planned-duty-auto-start-duty");if(!n)return;if(!t){n.innerHTML='<option value="">Първо избери ключ-график</option>',n.value="";return}const s=await rl(t);if(!s.length){n.innerHTML='<option value="">Няма повески за този ключ-график</option>',n.value="";return}const a=s.map(i=>{const o=i.id===r?"selected":"";return`<option value="${i.id}" ${o}>${rt(i.name??"-")}</option>`}).join("");n.innerHTML='<option value="">Избери стартова повеска</option>'+a}function tl(e){e.querySelector("#planned-duty-auto-employee").value="",e.querySelector("#planned-duty-auto-assignment-role").value="conductor",e.querySelector("#planned-duty-auto-date-from").value="",e.querySelector("#planned-duty-auto-date-to").value="",e.querySelector("#planned-duty-auto-schedule-key").value="",e.querySelector("#planned-duty-auto-overwrite").checked=!1,e.querySelector("#planned-duty-auto-start-duty").innerHTML='<option value="">Първо избери ключ-график</option>'}async function Ih(e,t){var q;const r=e.querySelector("#planned-duty-auto-employee").value||null,n=e.querySelector("#planned-duty-auto-assignment-role").value||"",s=e.querySelector("#planned-duty-auto-date-from").value,a=e.querySelector("#planned-duty-auto-date-to").value,i=e.querySelector("#planned-duty-auto-schedule-key").value||null,o=e.querySelector("#planned-duty-auto-start-duty").value||null,l=e.querySelector("#planned-duty-auto-overwrite").checked,d=e.querySelector("#planned-duty-auto-save-btn");if(!r||!n||!s||!a||!i||!o){g("Моля, попълни всички полета за автоматично планиране.","warning");return}if(!["chief","conductor"].includes(n)){g("Невалидна роля. Избери Кондуктор или Началник влак.","warning");return}if(a<s){g('Полето "До дата" трябва да е след или равно на "От дата".',"warning");return}const c=await rl(i);if(!c.length){g("Няма повески за избрания ключ-график.","warning");return}const u=c.findIndex(T=>T.id===o);if(u<0){g("Избери валидна стартова повеска.","warning");return}const p=Dh(s,a);if(!p.length){g("Невалиден период.","warning");return}const m=d.innerHTML;d.disabled=!0,d.innerHTML='<span class="spinner-border spinner-border-sm me-2" role="status" aria-hidden="true"></span>Генериране...';const{data:h}=await S.auth.getUser(),f=((q=h==null?void 0:h.user)==null?void 0:q.email)??"web_app",{data:w,error:y}=await S.from("planned_duties").select("date").eq("employee_id",r).gte("date",s).lte("date",a);if(y){d.disabled=!1,d.innerHTML=m,g(y.message,"error");return}const v=new Set((w||[]).map(T=>T.date)),b=v.size,_=[];let k=0;if(p.forEach((T,E)=>{if(!l&&v.has(T)){k+=1;return}const $=c[(u+E)%c.length];_.push({date:T,employee_id:r,assignment_role:n,duty_id:$.id,created_from:f})}),!_.length){d.disabled=!1,d.innerHTML=m,g("Няма нови записи за създаване. За периода вече има планиране за служителя.","warning");return}if(l){const{error:T}=await S.from("planned_duties").delete().eq("employee_id",r).gte("date",s).lte("date",a);if(T){d.disabled=!1,d.innerHTML=m,g(T.message,"error");return}}let x=null;for(let T=0;T<_.length;T+=200){const E=_.slice(T,T+200),{error:$}=await S.from("planned_duties").insert(E);if($){x=$;break}}if(d.disabled=!1,d.innerHTML=m,x){g(x.message,"error");return}Ue(e.querySelector("#planned-duty-auto-modal")),tl(e),await t();const L=_.length;if(l){g(`Създадени записи: ${L}. Презаписани дати: ${b}.`,"success");return}if(k>0){g(`Създадени: ${L}. Пропуснати (вече съществуват за датата): ${k}.`,"success");return}g(`Създадени записи: ${L}.`,"success")}async function rl(e){if(!e)return[];if(qs.has(e))return qs.get(e);const{data:t,error:r}=await S.from("schedule_key_duties").select("duty_id, duties(id, name, display_order)").eq("schedule_key_id",e);if(r)return g(r.message,"error"),[];const n=(t||[]).map(s=>{var a,i,o;return{id:(a=s==null?void 0:s.duties)==null?void 0:a.id,name:((i=s==null?void 0:s.duties)==null?void 0:i.name)||"-",displayOrder:Number((o=s==null?void 0:s.duties)==null?void 0:o.display_order)||0}}).filter(s=>s.id).sort((s,a)=>s.displayOrder!==a.displayOrder?s.displayOrder-a.displayOrder:String(s.name||"").localeCompare(String(a.name||""),"bg"));return qs.set(e,n),n}function Dh(e,t){const r=[],n=new Date(`${e}T00:00:00`),s=new Date(`${t}T00:00:00`);if(Number.isNaN(n.getTime())||Number.isNaN(s.getTime())||n>s)return r;for(let a=new Date(n);a<=s;a.setDate(a.getDate()+1)){const i=a.getFullYear(),o=String(a.getMonth()+1).padStart(2,"0"),l=String(a.getDate()).padStart(2,"0");r.push(`${i}-${o}-${l}`)}return r}function Oh(e){if(!B.selectedIds.length){g("Избери поне едно планиране за изтриване.","warning");return}const t=e.querySelector("#planned-duty-bulk-delete-count");t&&(t.textContent=String(B.selectedIds.length)),Hr(e.querySelector("#planned-duty-bulk-delete-modal"))}function Ph(e){const t=B.visibleRowIds||[];if(e){const r=new Set(B.selectedIds);t.forEach(n=>r.add(n)),B.selectedIds=Array.from(r);return}B.selectedIds=B.selectedIds.filter(r=>!t.includes(r))}function Mh(e,t){if(e){if(t){B.selectedIds.includes(e)||(B.selectedIds=[...B.selectedIds,e]);return}B.selectedIds=B.selectedIds.filter(r=>r!==e)}}async function jh(e,t){const r=e.querySelector("#planned-duty-bulk-delete-confirm"),n=[...B.selectedIds];if(!n.length){Ue(e.querySelector("#planned-duty-bulk-delete-modal"));return}const s=r.innerHTML;r.disabled=!0,r.innerHTML='<span class="spinner-border spinner-border-sm me-2" role="status" aria-hidden="true"></span>Изтриване...';let a=null;for(let o=0;o<n.length;o+=200){const l=n.slice(o,o+200),{error:d}=await S.from("planned_duties").delete().in("id",l);if(d){a=d;break}}if(r.disabled=!1,r.innerHTML=s,a){g(a.message,"error");return}const i=n.length;B.selectedIds=[],Ue(e.querySelector("#planned-duty-bulk-delete-modal")),await t(),g(`Изтрити планирания: ${i}.`,"success")}async function Nh(e,t){const r=e.querySelector("#add-selected-to-actual-duty"),n=[...B.selectedIds];if(!n.length){g("Избери поне едно планиране за прехвърляне към Актуални.","warning");return}const s=new Set(n),a=B.rows.filter(c=>s.has(c.id));if(!a.length){g("Няма валидни избрани планирания за прехвърляне.","warning");return}const i=a.filter(c=>(c==null?void 0:c.date)&&(c==null?void 0:c.employee_id)&&(c==null?void 0:c.duty_id)).map(c=>({date:c.date,employee_id:c.employee_id,duty_id:c.duty_id,assignment_role:c.assignment_role||"conductor"}));if(!i.length){g("Избраните записи са невалидни за прехвърляне.","warning");return}const o=(r==null?void 0:r.innerHTML)||"Към Актуални";r&&(r.disabled=!0,r.innerHTML='<span class="spinner-border spinner-border-sm me-2" role="status" aria-hidden="true"></span>Прехвърляне...');let l=null;for(let c=0;c<i.length;c+=200){const u=i.slice(c,c+200),{error:p}=await S.from("actual_duties").upsert(u,{onConflict:"date,employee_id,duty_id",ignoreDuplicates:!0});if(p){l=p;break}}if(r&&(r.disabled=!1,r.innerHTML=o),l){g(l.message,"error");return}const d=i.length;B.selectedIds=[],await t(),g(`Прехвърлени към Актуални: ${d}. Съществуващите са пропуснати.`,"success")}async function Hh(e){e.innerHTML=xh,B.selectionEnabled=!await hn(),Uh(e),Fh(e),await Eh(e),await $h(e),await Ch(e),await vr(e)}function Uh(e){var r,n;if(B.selectionEnabled)return;(r=e.querySelector(".page-actions"))==null||r.classList.add("d-none");const t=e.querySelector("#planned-duties-select-all");(n=t==null?void 0:t.closest("th"))==null||n.classList.add("d-none")}function Fh(e){const t=e.querySelector("#open-create-planned-duty"),r=e.querySelector("#open-bulk-delete-planned-duty"),n=e.querySelector("#add-selected-to-actual-duty"),s=e.querySelector("#go-to-plan-schedule"),a=e.querySelector("#open-auto-plan-duty"),i=e.querySelector("#planned-duty-form"),o=e.querySelector("#planned-duty-auto-form"),l=e.querySelector("#planned-duty-cancel-btn"),d=e.querySelector("#planned-duty-auto-cancel-btn"),c=e.querySelector("#planned-duties-table-body"),u=e.querySelector("#planned-duty-modal"),p=e.querySelector("#planned-duty-auto-modal"),m=e.querySelector("#planned-duty-delete-modal"),h=e.querySelector("#planned-duty-bulk-delete-modal"),f=e.querySelector("#planned-duty-modal-close"),w=e.querySelector("#planned-duty-auto-modal-close"),y=e.querySelector("#planned-duty-delete-confirm"),v=e.querySelector("#planned-duty-delete-cancel"),b=e.querySelector("#planned-duty-bulk-delete-confirm"),_=e.querySelector("#planned-duty-bulk-delete-cancel"),k=e.querySelector("#planned-duties-select-all"),x=e.querySelector("#planned-duties-search"),L=e.querySelector("#planned-duties-date-filter"),q=e.querySelector("#planned-duties-role-filter"),T=e.querySelector("#planned-duties-filter-reset"),E=e.querySelector("#planned-duty-schedule-key"),$=e.querySelector("#planned-duty-auto-schedule-key");t==null||t.addEventListener("click",()=>{Ra(e),Hr(u)}),r==null||r.addEventListener("click",()=>{B.selectionEnabled&&Oh(e)}),n==null||n.addEventListener("click",async()=>{B.selectionEnabled&&await Nh(e,async()=>{await vr(e)})}),a==null||a.addEventListener("click",async()=>{tl(e),Hr(p),await zi(e,($==null?void 0:$.value)||"","")}),i==null||i.addEventListener("submit",async A=>{A.preventDefault(),await Bh(e)}),o==null||o.addEventListener("submit",async A=>{A.preventDefault(),await Ih(e,async()=>{await vr(e)})}),l==null||l.addEventListener("click",()=>{Ue(u)}),f==null||f.addEventListener("click",()=>{Ue(u)}),w==null||w.addEventListener("click",()=>{Ue(p)}),d==null||d.addEventListener("click",()=>{Ue(p)}),v==null||v.addEventListener("click",()=>{Ue(m)}),_==null||_.addEventListener("click",()=>{Ue(h)}),x==null||x.addEventListener("input",A=>{B.searchQuery=A.target.value.trim().toLowerCase(),dt(e)}),L==null||L.addEventListener("change",A=>{B.dateFilter=A.target.value||"",Ls(s,B.dateFilter),dt(e)}),q==null||q.addEventListener("change",A=>{B.roleFilter=A.target.value||"",dt(e)}),T==null||T.addEventListener("click",()=>{B.searchQuery="",B.dateFilter="",B.roleFilter="",x&&(x.value=""),L&&(L.value=""),q&&(q.value=""),Ls(s,B.dateFilter),dt(e)}),s==null||s.addEventListener("click",()=>{const A=B.dateFilter||(L==null?void 0:L.value)||"";if(!A){g("Избери дата от филтъра, за да отвориш План-График.","warning");return}const C=new URLSearchParams({date:A});window.history.pushState({},"",`/plan-schedule?${C.toString()}`),window.dispatchEvent(new PopStateEvent("popstate"))}),Ls(s,B.dateFilter||(L==null?void 0:L.value)||""),E==null||E.addEventListener("change",()=>{ls(e,E.value||"","")}),$==null||$.addEventListener("change",async()=>{await zi(e,$.value||"","")}),k==null||k.addEventListener("change",()=>{B.selectionEnabled&&(Ph(k.checked),dt(e))}),qh("planned-duties",[m,h,p,u]),y==null||y.addEventListener("click",async()=>{const A=e.querySelector("#planned-duty-delete-id").value;await zh(A,e)}),b==null||b.addEventListener("click",async()=>{await jh(e,async()=>{await vr(e)})}),c==null||c.addEventListener("change",A=>{if(!B.selectionEnabled)return;const C=A.target.closest("input[data-select-id]");if(!C)return;const I=C.getAttribute("data-select-id");Mh(I,C.checked),dt(e)}),c==null||c.addEventListener("click",A=>{const C=A.target.closest("button[data-action]");if(!C)return;const I=C.getAttribute("data-action");if(I==="edit"){Kh(e,{id:C.getAttribute("data-id"),date:C.getAttribute("data-date"),employeeId:C.getAttribute("data-employee-id"),assignmentRole:C.getAttribute("data-assignment-role")||"conductor",dutyId:C.getAttribute("data-duty-id"),dutyScheduleKeyId:C.getAttribute("data-duty-schedule-key-id")}),Hr(u);return}if(I==="delete"){const R=C.getAttribute("data-id");e.querySelector("#planned-duty-delete-id").value=R,Hr(m)}})}async function Bh(e){var y;const t=e.querySelector("#planned-duty-id"),r=e.querySelector("#planned-duty-date"),n=e.querySelector("#planned-duty-employee"),s=e.querySelector("#planned-duty-assignment-role"),a=e.querySelector("#planned-duty-schedule-key"),i=e.querySelector("#planned-duty-duty"),o=e.querySelector("#planned-duty-save-btn"),l=r.value,d=n.value||null,c=s.value||"",u=a.value||null,p=i.value||null,m=t.value;if(!l||!d||!c||!u||!p){g("Моля, попълни всички полета.","warning");return}if(!["chief","conductor"].includes(c)){g("Невалидна роля. Избери Кондуктор или Началник влак.","warning");return}if(!Rh(p,u)){g("Избери повеска от посочения ключ-график.","warning");return}const h=o.innerHTML;o.disabled=!0,o.innerHTML='<span class="spinner-border spinner-border-sm me-2" role="status" aria-hidden="true"></span>Запис...';const f={date:l,employee_id:d,assignment_role:c,duty_id:p};let w;if(m)({error:w}=await S.from("planned_duties").update(f).eq("id",m));else{const{data:v}=await S.auth.getUser(),b=((y=v==null?void 0:v.user)==null?void 0:y.email)??"web_app";({error:w}=await S.from("planned_duties").insert({...f,created_from:b}))}if(o.disabled=!1,o.innerHTML=h,w){if(w.code==="23505"){g("Това планиране вече съществува за тази дата.","warning");return}g(w.message,"error");return}g(m?"Планирането е обновено.":"Планирането е създадено.","success"),Ue(e.querySelector("#planned-duty-modal")),Ra(e),await vr(e)}function Kh(e,t){e.querySelector("#planned-duty-id").value=t.id,e.querySelector("#planned-duty-date").value=t.date??"",e.querySelector("#planned-duty-employee").value=t.employeeId??"",e.querySelector("#planned-duty-assignment-role").value=t.assignmentRole??"conductor",e.querySelector("#planned-duty-schedule-key").value=t.dutyScheduleKeyId??"",ls(e,t.dutyScheduleKeyId??"",t.dutyId??""),e.querySelector("#planned-duty-form-title").textContent="Редакция на планиране",e.querySelector("#planned-duty-save-btn").textContent="Запази"}function Ra(e){e.querySelector("#planned-duty-id").value="",e.querySelector("#planned-duty-date").value="",e.querySelector("#planned-duty-employee").value="",e.querySelector("#planned-duty-assignment-role").value="conductor",e.querySelector("#planned-duty-schedule-key").value="",ls(e,"",""),e.querySelector("#planned-duty-form-title").textContent="Ново планиране",e.querySelector("#planned-duty-save-btn").textContent="Създай"}async function zh(e,t){const r=t.querySelector("#planned-duty-delete-confirm"),n=r.innerHTML;r.disabled=!0,r.innerHTML='<span class="spinner-border spinner-border-sm me-2" role="status" aria-hidden="true"></span>Изтриване...';const{error:s}=await S.from("planned_duties").delete().eq("id",e);if(r.disabled=!1,r.innerHTML=n,s){g(s.message,"error");return}g("Планирането е изтрито.","success"),B.selectedIds=B.selectedIds.filter(a=>a!==e),Ue(t.querySelector("#planned-duty-delete-modal")),Ra(t),await vr(t)}function Ls(e,t){e&&(e.disabled=!t)}const Wh=`<section class="card border-0 shadow-sm">\r
  <div class="card-body p-4 p-md-5">\r
    <div class="d-flex flex-column flex-lg-row justify-content-between align-items-start align-items-lg-center gap-2 mb-3">\r
...
`;function Dt(e){e.classList.remove("d-none"),document.body.classList.add("overflow-hidden")}const Wi=new Map;function Vh(e,t){const r=Wi.get(e);r&&document.removeEventListener("keydown",r);const n=s=>{if(s.key==="Escape"){for(const a of t)if(a&&!a.classList.contains("d-none")){Re(a);return}}};Wi.set(e,n),document.addEventListener("keydown",n)}function Re(e){var t,r,n,s,a;e.classList.add("d-none"),(t=document.querySelector("#actual-duty-modal"))!=null&&t.classList.contains("d-none")&&((r=document.querySelector("#actual-duty-delete-modal"))!=null&&r.classList.contains("d-none"))&&((n=document.querySelector("#actual-duty-bulk-delete-modal"))!=null&&n.classList.contains("d-none"))&&((s=document.querySelector("#actual-duty-bulk-add-modal"))!=null&&s.classList.contains("d-none"))&&((a=document.querySelector("#actual-duty-profile-modal"))!=null&&a.classList.contains("d-none"))&&document.body.classList.remove("overflow-hidden")}function ie(e){return String(e).replaceAll("&","&amp;").replaceAll("<","&lt;").replaceAll(">","&gt;").replaceAll('"',"&quot;").replaceAll("'","&#039;")}const M={rows:[],searchQuery:"",dateFilter:"",roleFilter:"",selectedIds:[],visibleRowIds:[],selectionEnabled:!0,crewEditTimeOnly:!1,plannedRows:[],plannedSearchQuery:"",plannedDateFilter:"",plannedSelectedIds:[],plannedVisibleRowIds:[]};function Vi(e){return e&&`${e.first_name??""} ${e.last_name??""}`.trim()||"-"}async function mn(e){const{data:t,error:r}=await S.from("actual_duties").select("id, date, employee_id, duty_id, assignment_role, start_time_override, end_time_override, break_start_time_override, break_end_time_override, employees(first_name, last_name), duties(name, start_time, end_time, break_start_time, break_end_time, schedule_key_duties(schedule_key_id))").order("date",{ascending:!1});if(r){g(r.message,"error"),M.rows=[],ct(e,"Грешка при зареждане на реалните повески.");return}M.rows=t||[],ct(e)}function ct(e,t){var d,c;const r=e.querySelector("#actual-duties-table-body"),n=e.querySelector("#actual-duties-empty"),s=e.querySelector("#actual-duties-select-all"),a=e.querySelector("#open-bulk-delete-actual-duty"),i=M.selectionEnabled!==!1;i||(M.selectedIds=[]),M.selectedIds=M.selectedIds.filter(u=>M.rows.some(p=>p.id===u));const o=M.rows.filter(u=>{var v;const p=Vi(u.employees).toLowerCase(),m=(((v=u.duties)==null?void 0:v.name)||"").toLowerCase(),h=(u.date||"").toLowerCase(),f=!M.searchQuery||p.includes(M.searchQuery)||m.includes(M.searchQuery)||h.includes(M.searchQuery),w=!M.dateFilter||u.date===M.dateFilter,y=!M.roleFilter||u.assignment_role===M.roleFilter;return f&&w&&y});if(!o.length){M.visibleRowIds=[],r.innerHTML="",n.classList.remove("d-none"),n.textContent=t||"Няма записи за реално изпълнение.",s&&(s.checked=!1,s.indeterminate=!1,s.disabled=!i,(d=s.closest("th"))==null||d.classList.toggle("d-none",!i)),a&&(a.disabled=!i||M.selectedIds.length===0,a.textContent=M.selectedIds.length?`Изтрий избраните (${M.selectedIds.length})`:"Изтрий избраните");return}M.visibleRowIds=o.map(u=>u.id),n.classList.add("d-none"),s&&(s.disabled=!i,(c=s.closest("th"))==null||c.classList.toggle("d-none",!i)),r.innerHTML=o.map(u=>{var h,f,w,y,v;const p=Gh(u),m=i&&M.selectedIds.includes(u.id);return`
        <tr>
          ${i?`
          <td>
            <input
          ...
          `:""}
          <td>${ie(u.date??"-")}</td>
          <td>${ie(Vi(u.employees))}</td>
          <td>${ie(Jh(u.assignment_role))}</td>
      ...
      `}).join("");const l=o.filter(u=>M.selectedIds.includes(u.id)).length;s&&(s.disabled=!i,s.checked=i&&l>0&&l===o.length,s.indeterminate=i&&l>0&&l<o.length),a&&(a.disabled=!i||M.selectedIds.length===0,a.textContent=M.selectedIds.length?`Изтрий избраните (${M.selectedIds.length})`:"Изтрий избраните")}function Gh(e){var r,n,s;return((s=(Array.isArray((r=e==null?void 0:e.duties)==null?void 0:r.schedule_key_duties)?e.duties.schedule_key_duties:(n=e==null?void 0:e.duties)!=null&&n.schedule_key_duties?[e.duties.schedule_key_duties]:[]).find(a=>a==null?void 0:a.schedule_key_id))==null?void 0:s.schedule_key_id)||""}function Jh(e){return e==="chief"?"Началник влак":"Кондуктор"}let ds=[];async function Qh(e){e.innerHTML=Wh;const t=await hn();M.selectionEnabled=!t,M.crewEditTimeOnly=t,Yh(e);const r=um(),n=e.querySelector("#actual-duties-date-filter");r&&n&&(n.value=r,M.dateFilter=r),Xh(e),await Zh(e),await em(e),await tm(e),await mn(e)}function Yh(e){var t,r,n;M.selectionEnabled||((t=e.querySelector("#open-bulk-delete-actual-duty"))==null||t.classList.add("d-none"),(n=(r=e.querySelector("#actual-duties-select-all"))==null?void 0:r.closest("th"))==null||n.classList.add("d-none"))}function Ts(e,t){["actual-duty-date","actual-duty-employee","actual-duty-schedule-key","actual-duty-duty","actual-duty-assignment-role"].forEach(n=>{const s=e.querySelector(`#${n}`),a=(s==null?void 0:s.closest(".col-md-4"))||(s==null?void 0:s.closest(".col-md-3"))||(s==null?void 0:s.closest(".col-12"));a&&a.classList.toggle("d-none",t),s&&(s.disabled=t)})}function Xh(e){const t=e.querySelector("#open-create-actual-duty"),r=e.querySelector("#go-to-schedule"),n=e.querySelector("#open-bulk-delete-actual-duty"),s=e.querySelector("#open-bulk-add-actual-duty"),a=e.querySelector("#actual-duty-form"),i=e.querySelector("#actual-duties-table-body"),o=e.querySelector("#actual-duty-modal"),l=e.querySelector("#actual-duty-delete-modal"),d=e.querySelector("#actual-duty-bulk-delete-modal"),c=e.querySelector("#actual-duty-bulk-add-modal"),u=e.querySelector("#actual-duty-profile-modal"),p=e.querySelector("#actual-duty-modal-close"),m=e.querySelector("#actual-duty-cancel-btn"),h=e.querySelector("#actual-duty-profile-close"),f=e.querySelector("#actual-duty-profile-close-secondary"),w=e.querySelector("#actual-duty-delete-confirm"),y=e.querySelector("#actual-duty-delete-cancel"),v=e.querySelector("#actual-duty-bulk-delete-confirm"),b=e.querySelector("#actual-duty-bulk-delete-cancel"),_=e.querySelector("#actual-duty-bulk-add-modal-close"),k=e.querySelector("#actual-duty-bulk-add-cancel"),x=e.querySelector("#actual-duty-bulk-add-confirm"),L=e.querySelector("#actual-duty-bulk-add-search"),q=e.querySelector("#actual-duty-bulk-add-date-filter"),T=e.querySelector("#actual-duty-bulk-add-filter-reset"),E=e.querySelector("#actual-duty-bulk-add-select-all"),$=e.querySelector("#actual-duty-bulk-add-table-body"),A=e.querySelector("#actual-duties-search"),C=e.querySelector("#actual-duties-date-filter"),I=e.querySelector("#actual-duties-role-filter"),R=e.querySelector("#actual-duties-filter-reset"),W=e.querySelector("#actual-duty-schedule-key"),K=e.querySelector("#actual-duties-select-all");t==null||t.addEventListener("click",()=>{nl(e),Ts(e,!1),Dt(o)}),n==null||n.addEventListener("click",()=>{if(!M.selectionEnabled)return;if(!M.selectedIds.length){g("Избери поне един запис за изтриване.","warning");return}const j=e.querySelector("#actual-duty-bulk-delete-count");j&&(j.textContent=String(M.selectedIds.length)),Dt(d)}),s==null||s.addEventListener("click",async()=>{Qi(e),await dm(e),Dt(c)}),a==null||a.addEventListener("submit",async j=>{j.preventDefault(),await am(e)}),p==null||p.addEventListener("click",()=>{Re(o)}),m==null||m.addEventListener("click",()=>{Re(o)}),h==null||h.addEventListener("click",()=>{Re(u)}),f==null||f.addEventListener("click",()=>{Re(u)}),y==null||y.addEventListener("click",()=>{Re(l)}),b==null||b.addEventListener("click",()=>{Re(d)}),_==null||_.addEventListener("click",()=>{Re(c)}),k==null||k.addEventListener("click",()=>{Re(c)}),A==null||A.addEventListener("input",j=>{M.searchQuery=j.target.value.trim().toLowerCase(),ct(e)}),C==null||C.addEventListener("change",j=>{M.dateFilter=j.target.value||"",Es(r,M.dateFilter),ct(e)}),I==null||I.addEventListener("change",j=>{M.roleFilter=j.target.value||"",ct(e)}),R==null||R.addEventListener("click",()=>{M.searchQuery="",M.dateFilter="",M.roleFilter="",A&&(A.value=""),C&&(C.value=""),I&&(I.value=""),Es(r,M.dateFilter),ct(e)}),r==null||r.addEventListener("click",()=>{const j=M.dateFilter||(C==null?void 0:C.value)||"";if(!j){g("Избери дата от филтъра, за да отвориш График.","warning");return}const z=new URLSearchParams({date:j});window.history.pushState({},"",`/schedule?${z.toString()}`),window.dispatchEvent(new PopStateEvent("popstate"))}),Es(r,M.dateFilter||(C==null?void 0:C.value)||""),W==null||W.addEventListener("change",()=>{cs(e,W.value||"",""),Ji(e)});const V=e.querySelector("#actual-duty-duty");V==null||V.addEventListener("change",()=>{Ji(e)}),K==null||K.addEventListener("change",()=>{if(!M.selectionEnabled)return;const j=M.visibleRowIds||[];if(K.checked){const z=new Set(M.selectedIds);j.forEach(ee=>z.add(ee)),M.selectedIds=Array.from(z)}else M.selectedIds=M.selectedIds.filter(z=>!j.includes(z));ct(e)}),w==null||w.addEventListener("click",async()=>{const j=e.querySelector("#actual-duty-delete-id").value;await om(j,e)}),v==null||v.addEventListener("click",async()=>{await lm(e)}),x==null||x.addEventListener("click",async()=>{await cm(e)}),L==null||L.addEventListener("input",j=>{M.plannedSearchQuery=j.target.value.trim().toLowerCase(),Ot(e)}),q==null||q.addEventListener("change",j=>{M.plannedDateFilter=j.target.value||"",Ot(e)}),T==null||T.addEventListener("click",()=>{Qi(e),Ot(e)}),E==null||E.addEventListener("change",()=>{const j=M.plannedVisibleRowIds||[];if(E.checked){const z=new Set(M.plannedSelectedIds);j.forEach(ee=>z.add(ee)),M.plannedSelectedIds=Array.from(z)}else M.plannedSelectedIds=M.plannedSelectedIds.filter(z=>!j.includes(z));Ot(e)}),$==null||$.addEventListener("change",j=>{const z=j.target.closest("input[data-planned-select-id]");if(!z)return;const ee=z.getAttribute("data-planned-select-id");ee&&(z.checked?M.plannedSelectedIds.includes(ee)||(M.plannedSelectedIds=[...M.plannedSelectedIds,ee]):M.plannedSelectedIds=M.plannedSelectedIds.filter(le=>le!==ee),Ot(e))}),i==null||i.addEventListener("change",j=>{if(!M.selectionEnabled)return;const z=j.target.closest("input[data-select-id]");if(!z)return;const ee=z.getAttribute("data-select-id");ee&&(z.checked?M.selectedIds.includes(ee)||(M.selectedIds=[...M.selectedIds,ee]):M.selectedIds=M.selectedIds.filter(le=>le!==ee),ct(e))}),i==null||i.addEventListener("click",j=>{const z=j.target.closest("button[data-action]");if(!z)return;const ee=z.getAttribute("data-action");if(ee==="profile"){const le=z.getAttribute("data-id");pm(e,le);return}if(ee==="edit"){im(e,{id:z.getAttribute("data-id"),date:z.getAttribute("data-date"),employeeId:z.getAttribute("data-employee-id"),assignmentRole:z.getAttribute("data-assignment-role")||"conductor",dutyId:z.getAttribute("data-duty-id"),dutyScheduleKeyId:z.getAttribute("data-duty-schedule-key-id"),startTimeOverride:z.getAttribute("data-start-time-override")||"",endTimeOverride:z.getAttribute("data-end-time-override")||"",breakStartTimeOverride:z.getAttribute("data-break-start-time-override")||"",breakEndTimeOverride:z.getAttribute("data-break-end-time-override")||"",dutyStartTime:z.getAttribute("data-duty-start-time")||"",dutyEndTime:z.getAttribute("data-duty-end-time")||"",dutyBreakStartTime:z.getAttribute("data-duty-break-start-time")||"",dutyBreakEndTime:z.getAttribute("data-duty-break-end-time")||""}),M.crewEditTimeOnly?Ts(e,!0):Ts(e,!1),Dt(o);return}if(ee==="delete"){const le=z.getAttribute("data-id");e.querySelector("#actual-duty-delete-id").value=le,Dt(l)}}),Vh("actual-duties",[u,l,d,c,o])}async function Zh(e){const t=e.querySelector("#actual-duty-employee"),{data:r,error:n}=await S.from("employees").select("id, first_name, last_name").order("last_name",{ascending:!0}).order("first_name",{ascending:!0});if(n){g(n.message,"error");return}const s=(r||[]).map(a=>{const i=`${a.first_name??""} ${a.last_name??""}`.trim()||"-";return`<option value="${a.id}">${ie(i)}</option>`}).join("");t.innerHTML='<option value="">Избери служител</option>'+s}async function em(e){const t=e.querySelector("#actual-duty-schedule-key"),{data:r,error:n}=await S.from("schedule_keys").select("id, name").order("name",{ascending:!0});if(n){g(n.message,"error");return}const s=(r||[]).map(a=>`<option value="${a.id}">${ie(a.name??"-")}</option>`).join("");t.innerHTML='<option value="">Избери ключ-график</option>'+s}async function tm(e){const{data:t,error:r}=await S.from("schedule_key_duties").select("schedule_key_id, duty_id, duties(id, name, start_time, end_time, break_start_time, break_end_time)");if(r){g(r.message,"error");return}const n=new Map;(t||[]).forEach(s=>{const a=s==null?void 0:s.duties;if(!(a!=null&&a.id))return;const i=n.get(a.id)||{id:a.id,name:a.name||"-",scheduleKeyIds:[],startTime:vt(a.start_time),endTime:vt(a.end_time),breakStartTime:vt(a.break_start_time),breakEndTime:vt(a.break_end_time)};s.schedule_key_id&&!i.scheduleKeyIds.includes(s.schedule_key_id)&&i.scheduleKeyIds.push(s.schedule_key_id),n.set(a.id,i)}),ds=Array.from(n.values()).sort((s,a)=>String(s.name||"").localeCompare(String(a.name||""),"bg")),cs(e,"","")}function cs(e,t,r){const n=e.querySelector("#actual-duty-duty");if(!n)return;if(!t){n.innerHTML='<option value="">Първо избери ключ-график</option>',n.value="";return}const s=ds.filter(a=>{var i;return(i=a.scheduleKeyIds)==null?void 0:i.includes(t)}).map(a=>{const i=a.id===r?"selected":"";return`<option value="${a.id}" ${i}>${ie(a.name??"-")}</option>`}).join("");n.innerHTML='<option value="">Избери повеска</option>'+s,r&&(n.value=r)}function rm(e,t){var n;const r=ds.find(s=>s.id===e);return!!(r&&((n=r.scheduleKeyIds)!=null&&n.includes(t)))}function nm(e){return ds.find(t=>t.id===e)||null}function vt(e){return e?String(e).slice(0,5):""}function qe(e,t=""){const r=String(e||"").slice(0,5);return/^\d{2}:\d{2}$/.test(r)?r:t}function ot(e){const t=qe(e,"");return t?`${t}:00`:null}function Gi(e){const t=Number(e);if(!Number.isFinite(t)||t<0)return"-";const r=Math.floor(t/60),n=t%60;return`${String(r).padStart(2,"0")}:${String(n).padStart(2,"0")}`}function sm(e,t,r,n){const s=qe(e,""),a=qe(t,""),i=qe(r,"00:00"),o=qe(n,"00:00"),l=s&&a?Ie(s,a):null,d=Ie(i,o),c=Number.isFinite(l)?Math.max(0,l-d):null;return{startTime:s||"-",endTime:a||"-",breakStartTime:i||"-",breakEndTime:o||"-",breakDuration:Gi(d),duration:c===null?"-":Gi(c)}}function Ji(e){var o;const t=((o=e.querySelector("#actual-duty-duty"))==null?void 0:o.value)||"",r=nm(t);if(!r)return;const n=e.querySelector("#actual-duty-start-time"),s=e.querySelector("#actual-duty-end-time"),a=e.querySelector("#actual-duty-break-start-time"),i=e.querySelector("#actual-duty-break-end-time");n&&(n.value=r.startTime||""),s&&(s.value=r.endTime||""),a&&(a.value=r.breakStartTime||"00:00"),i&&(i.value=r.breakEndTime||"00:00")}async function am(e){const t=e.querySelector("#actual-duty-id"),r=e.querySelector("#actual-duty-date"),n=e.querySelector("#actual-duty-employee"),s=e.querySelector("#actual-duty-schedule-key"),a=e.querySelector("#actual-duty-duty"),i=e.querySelector("#actual-duty-assignment-role"),o=e.querySelector("#actual-duty-start-time"),l=e.querySelector("#actual-duty-end-time"),d=e.querySelector("#actual-duty-break-start-time"),c=e.querySelector("#actual-duty-break-end-time"),u=e.querySelector("#actual-duty-save-btn"),p=r.value,m=n.value||null,h=s.value||null,f=a.value||null,w=i.value||"conductor",y=qe((o==null?void 0:o.value)||"",""),v=qe((l==null?void 0:l.value)||"",""),b=qe((d==null?void 0:d.value)||"","00:00"),_=qe((c==null?void 0:c.value)||"","00:00"),k=t.value,x=!!k&&M.crewEditTimeOnly;if(x){if(!y||!v){g("Моля, попълни Начало и Край.","warning");return}}else{if(!p||!m||!h||!f||!y||!v){g("Моля, попълни всички полета.","warning");return}if(!rm(f,h)){g("Избери повеска от посочения ключ-график.","warning");return}if(!["chief","conductor"].includes(w)){g("Невалидна роля. Избери Кондуктор или Началник влак.","warning");return}}const L=Ie(y,v);if(Ie(b,_)>L){g("Прекъсването не може да е по-голямо от продължителността на повеската.","warning");return}const T=u.innerHTML;u.disabled=!0,u.innerHTML='<span class="spinner-border spinner-border-sm me-2" role="status" aria-hidden="true"></span>Запис...';const E=x?{start_time_override:ot(y),end_time_override:ot(v),break_start_time_override:ot(b),break_end_time_override:ot(_)}:{date:p,employee_id:m,duty_id:f,assignment_role:w,start_time_override:ot(y),end_time_override:ot(v),break_start_time_override:ot(b),break_end_time_override:ot(_)};let $;if(k?{error:$}=await S.from("actual_duties").update(E).eq("id",k):{error:$}=await S.from("actual_duties").insert(E),u.disabled=!1,u.innerHTML=T,$){if($.code==="23505"){g("Този запис вече съществува за тази дата.","warning");return}g($.message,"error");return}g(k?"Записът е обновен.":"Записът е създаден.","success"),Re(e.querySelector("#actual-duty-modal")),nl(e),await mn(e)}function im(e,t){e.querySelector("#actual-duty-id").value=t.id,e.querySelector("#actual-duty-date").value=t.date??"",e.querySelector("#actual-duty-employee").value=t.employeeId??"",e.querySelector("#actual-duty-assignment-role").value=t.assignmentRole??"conductor",e.querySelector("#actual-duty-schedule-key").value=t.dutyScheduleKeyId??"",cs(e,t.dutyScheduleKeyId??"",t.dutyId??""),e.querySelector("#actual-duty-start-time").value=qe(t.startTimeOverride,t.dutyStartTime||""),e.querySelector("#actual-duty-end-time").value=qe(t.endTimeOverride,t.dutyEndTime||""),e.querySelector("#actual-duty-break-start-time").value=qe(t.breakStartTimeOverride,t.dutyBreakStartTime||"00:00"),e.querySelector("#actual-duty-break-end-time").value=qe(t.breakEndTimeOverride,t.dutyBreakEndTime||"00:00"),e.querySelector("#actual-duty-form-title").textContent="Редакция на запис",e.querySelector("#actual-duty-save-btn").textContent="Запази"}function nl(e){e.querySelector("#actual-duty-id").value="",e.querySelector("#actual-duty-date").value="",e.querySelector("#actual-duty-employee").value="",e.querySelector("#actual-duty-assignment-role").value="conductor",e.querySelector("#actual-duty-schedule-key").value="",cs(e,"",""),e.querySelector("#actual-duty-start-time").value="",e.querySelector("#actual-duty-end-time").value="",e.querySelector("#actual-duty-break-start-time").value="00:00",e.querySelector("#actual-duty-break-end-time").value="00:00",e.querySelector("#actual-duty-form-title").textContent="Нов запис",e.querySelector("#actual-duty-save-btn").textContent="Създай"}async function om(e,t){const r=t.querySelector("#actual-duty-delete-confirm"),n=r.innerHTML;r.disabled=!0,r.innerHTML='<span class="spinner-border spinner-border-sm me-2" role="status" aria-hidden="true"></span>Изтриване...';const{error:s}=await S.from("actual_duties").delete().eq("id",e);if(r.disabled=!1,r.innerHTML=n,s){g(s.message,"error");return}M.selectedIds=M.selectedIds.filter(a=>a!==e),Re(t.querySelector("#actual-duty-delete-modal")),await mn(t),g("Записът е изтрит.","success")}async function lm(e){const t=e.querySelector("#actual-duty-bulk-delete-confirm"),r=[...M.selectedIds];if(!r.length){Re(e.querySelector("#actual-duty-bulk-delete-modal"));return}const n=t.innerHTML;t.disabled=!0,t.innerHTML='<span class="spinner-border spinner-border-sm me-2" role="status" aria-hidden="true"></span>Изтриване...';let s=null;for(let i=0;i<r.length;i+=200){const o=r.slice(i,i+200),{error:l}=await S.from("actual_duties").delete().in("id",o);if(l){s=l;break}}if(t.disabled=!1,t.innerHTML=n,s){g(s.message,"error");return}const a=r.length;M.selectedIds=[],Re(e.querySelector("#actual-duty-bulk-delete-modal")),await mn(e),g(`Изтрити записи: ${a}.`,"success")}async function dm(e){const{data:t,error:r}=await S.from("planned_duties").select("id, date, employee_id, duty_id, assignment_role, employees(first_name, last_name), duties(name)").order("date",{ascending:!1});if(r){g(r.message,"error"),M.plannedRows=[],Ot(e,"Грешка при зареждане на планираните повески.");return}M.plannedRows=t||[],M.plannedSelectedIds=[],Ot(e)}function Ot(e,t){const r=e.querySelector("#actual-duty-bulk-add-table-body"),n=e.querySelector("#actual-duty-bulk-add-empty"),s=e.querySelector("#actual-duty-bulk-add-select-all"),a=e.querySelector("#actual-duty-bulk-add-confirm");M.plannedSelectedIds=M.plannedSelectedIds.filter(l=>M.plannedRows.some(d=>d.id===l));const i=M.plannedRows.filter(l=>{var h,f,w;const d=`${((h=l.employees)==null?void 0:h.first_name)??""} ${((f=l.employees)==null?void 0:f.last_name)??""}`.trim().toLowerCase(),c=(((w=l.duties)==null?void 0:w.name)||"").toLowerCase(),u=(l.date||"").toLowerCase(),p=!M.plannedSearchQuery||d.includes(M.plannedSearchQuery)||c.includes(M.plannedSearchQuery)||u.includes(M.plannedSearchQuery),m=!M.plannedDateFilter||l.date===M.plannedDateFilter;return p&&m});if(!i.length){M.plannedVisibleRowIds=[],r.innerHTML="",n.classList.remove("d-none"),n.textContent=t||"Няма записи за добавяне.",s.checked=!1,s.indeterminate=!1,s.disabled=!0,a.disabled=M.plannedSelectedIds.length===0,a.textContent=M.plannedSelectedIds.length?`Добави избраните (${M.plannedSelectedIds.length})`:"Добави избраните";return}M.plannedVisibleRowIds=i.map(l=>l.id),n.classList.add("d-none"),r.innerHTML=i.map(l=>{var u,p,m;const d=M.plannedSelectedIds.includes(l.id),c=`${((u=l.employees)==null?void 0:u.first_name)??""} ${((p=l.employees)==null?void 0:p.last_name)??""}`.trim()||"-";return`
        <tr>
          ...
          <td>${ie(l.date??"-")}</td>
          <td>${ie(c)}</td>
          <td>${ie(sl(l.assignment_role))}</td>
          <td>${ie(((m=l.duties)==null?void 0:m.name)??"-")}</td>
        </tr>
      `}).join("");const o=i.filter(l=>M.plannedSelectedIds.includes(l.id)).length;s.disabled=!1,s.checked=o>0&&o===i.length,s.indeterminate=o>0&&o<i.length,a.disabled=M.plannedSelectedIds.length===0,a.textContent=M.plannedSelectedIds.length?`Добави избраните (${M.plannedSelectedIds.length})`:"Добави избраните"}function Qi(e){M.plannedSearchQuery="",M.plannedDateFilter="";const t=e.querySelector("#actual-duty-bulk-add-search"),r=e.querySelector("#actual-duty-bulk-add-date-filter");t&&(t.value=""),r&&(r.value="")}async function cm(e){const t=e.querySelector("#actual-duty-bulk-add-confirm"),r=[...M.plannedSelectedIds];if(!r.length){g("Избери поне един запис от планирани повески.","warning");return}const n=new Set(r),s=M.plannedRows.filter(d=>n.has(d.id));if(!s.length){g("Няма валидни записи за добавяне.","warning");return}const a=s.map(d=>({date:d.date,employee_id:d.employee_id,duty_id:d.duty_id,assignment_role:d.assignment_role||"conductor"})),i=t.innerHTML;t.disabled=!0,t.innerHTML='<span class="spinner-border spinner-border-sm me-2" role="status" aria-hidden="true"></span>Добавяне...';let o=null;for(let d=0;d<a.length;d+=200){const c=a.slice(d,d+200),{error:u}=await S.from("actual_duties").upsert(c,{onConflict:"date,employee_id,duty_id",ignoreDuplicates:!0});if(u){o=u;break}}if(t.disabled=!1,t.innerHTML=i,o){g(o.message,"error");return}const l=a.length;Re(e.querySelector("#actual-duty-bulk-add-modal")),await mn(e),g(`Обработени записи: ${l}. Съществуващите са пропуснати.`,"success")}function Es(e,t){e&&(e.disabled=!t)}function um(){const t=new URLSearchParams(window.location.search).get("date")||"";return/^\d{4}-\d{2}-\d{2}$/.test(t)?t:""}function sl(e){return e==="chief"?"Началник влак":"Кондуктор"}function pm(e,t){var m,h,f,w,y,v,b;const r=e.querySelector("#actual-duty-profile-modal"),n=e.querySelector("#actual-duty-profile-content");if(!r||!n)return;const s=(M.rows||[]).find(_=>_.id===t);if(!s){n.innerHTML='<p class="text-secondary mb-0">Няма данни за този запис.</p>',Dt(r);return}const a=`${((m=s.employees)==null?void 0:m.first_name)??""} ${((h=s.employees)==null?void 0:h.last_name)??""}`.trim()||"-",i=((f=s.duties)==null?void 0:f.name)||"-",o=sl(s.assignment_role),l=qe(s.start_time_override,vt((w=s.duties)==null?void 0:w.start_time)),d=qe(s.end_time_override,vt((y=s.duties)==null?void 0:y.end_time)),c=qe(s.break_start_time_override,vt((v=s.duties)==null?void 0:v.break_start_time)||"00:00"),u=qe(s.break_end_time_override,vt((b=s.duties)==null?void 0:b.break_end_time)||"00:00"),p=sm(l,d,c,u);n.innerHTML=`
    <div class="row g-3">
      <div class="col-md-6">
        <div class="border rounded p-3 h-100">
  ...
  `,Dt(r)}const hm=`<section class="card border-0 shadow-sm">\r
  <div class="card-body p-4 p-md-5">\r
...
`,mm="ПС - Стара Загора";function al(e,t){const r=e==null?void 0:e.querySelector(t);r&&(r.textContent=mm)}const Yi="id, name, notes, schedule_key_id, display_order, start_time, end_time, second_day, duty_types(name)";async function il(e){const{data:t,error:r}=await S.from("schedule_keys").select("id").lte("valid_from",e).gte("valid_to",e);if(r)return{data:[],error:r};const n=(t||[]).map(p=>p==null?void 0:p.id).filter(Boolean);if(!n.length)return{data:[],error:null};const{data:s,error:a}=await S.from("duties").select(Yi).in("schedule_key_id",n);if(a)return{data:[],error:a};const{data:i,error:o}=await S.from("schedule_key_duties").select("duty_id").in("schedule_key_id",n);if(o)return{data:[],error:o};const l=new Set((s||[]).map(p=>p==null?void 0:p.id).filter(Boolean)),d=[...new Set((i||[]).map(p=>p==null?void 0:p.duty_id).filter(Boolean))].filter(p=>!l.has(p));if(!d.length)return{data:s||[],error:null};const{data:c,error:u}=await S.from("duties").select(Yi).in("id",d);return u?{data:[],error:u}:{data:[...s||[],...c||[]],error:null}}function fm(e){const t=String(e||"").trim().toLowerCase();return t==="chief"||t==="conductor"?t:""}function ol(e){const t=(e==null?void 0:e.first_name)??"",r=(e==null?void 0:e.last_name)??"";return`${t} ${r}`.trim()}function ym(e){var r;const t=e==null?void 0:e.positions;return Array.isArray(t)?((r=t[0])==null?void 0:r.title)??"":t&&typeof t=="object"?t.title??"":""}function bm(e){var r;const t=e==null?void 0:e.duty_types;return Array.isArray(t)?((r=t[0])==null?void 0:r.name)??"":t&&typeof t=="object"?t.name??"":""}function vm(e){var r;const t=Array.isArray(e)?(r=e[0])==null?void 0:r.name:e&&typeof e=="object"?e.name:"";return String(t||"").trim()}function gm(e){const t=e==null?void 0:e.duties;return Array.isArray(t)?t[0]||null:t&&typeof t=="object"?t:null}function Xi(e){const t=String(e||"").trim();if(!t)return"99:99:99";const r=t.match(/^(\d{1,2}):(\d{2})(?::(\d{2}))?/);if(!r)return"99:99:99";const n=String(Number(r[1])).padStart(2,"0"),s=r[2],a=r[3]||"00";return`${n}:${s}:${a}`}function _m(){const t=new URLSearchParams(window.location.search).get("date")||"";return/^\d{4}-\d{2}-\d{2}$/.test(t)?t:""}function $e(e){return String(e).replaceAll("&","&amp;").replaceAll("<","&lt;").replaceAll(">","&gt;").replaceAll('"',"&quot;").replaceAll("'","&#039;")}const Zi=96/25.4;function wm(e,{orientation:t,compact:r,fitOnePage:n}){const s=document.documentElement,a=e.querySelector(".plan-schedule-sheet");if(s.classList.add("print-preparing"),s.classList.toggle("print-compact",r),s.classList.toggle("print-fit-one-page",n),a&&(a.classList.toggle("print-landscape-page",t==="landscape"),a.classList.toggle("print-portrait-page",t==="portrait")),!n||!a){s.style.setProperty("--plan-print-scale","1");return}s.style.setProperty("--plan-print-scale","1");const i=a.getBoundingClientRect(),o=t==="portrait"?210:297,l=t==="portrait"?297:210,d=(o-20)*Zi,c=(l-20)*Zi,u=d/Math.max(i.width,1),p=c/Math.max(i.height,1),m=Math.min(u,p,1);s.style.setProperty("--plan-print-scale",String(Math.max(.6,m)))}function Sm(){const e=document.documentElement;e.classList.remove("print-preparing","print-compact","print-fit-one-page","print-hide-second-day"),e.style.setProperty("--plan-print-scale","1"),document.querySelectorAll(".plan-schedule-sheet").forEach(t=>{t.classList.remove("print-landscape-page","print-portrait-page")})}function km(e){const t=new Map;return(e||[]).forEach(r=>{const n=r==null?void 0:r.employee_id;if(!n)return;const s=t.get(n)||{employeeId:n,employeeName:ol(r.employees),reasons:[]},a=vm(r.absence_reasons);a&&!s.reasons.includes(a)&&s.reasons.push(a),t.set(n,s)}),t}function xm(e){const t={train:[],businessTrip:[],dayOff:[]},r=new Map;return(e||[]).forEach(n=>{const s=gm(n);s!=null&&s.id&&(r.has(s.id)||r.set(s.id,s))}),Array.from(r.values()).forEach(n=>{const s=bm(n).toLowerCase();if(s.includes("на влак")){t.train.push(n);return}if(s.includes("командировка")){t.businessTrip.push(n);return}s.includes("свободен ден")&&t.dayOff.push(n)}),t.train.sort($m),t.businessTrip.sort(Zs),t.dayOff.sort(Zs),t}function qm(e,t){const r=new Map,n=new Map;return e.forEach(s=>{if(!(s!=null&&s.duty_id)||!(s!=null&&s.employees))return;if(s!=null&&s.employee_id&&(t!=null&&t.has(s.employee_id))){const l=t.get(s.employee_id);l&&!n.has(s.employee_id)&&n.set(s.employee_id,{employeeId:l.employeeId,employeeName:l.employeeName,reason:l.reasons.join(", ")});return}const a=r.get(s.duty_id)||{chiefs:[],conductors:[]},i=ol(s.employees),o=fm(s.assignment_role);if(o==="chief"&&i&&!a.chiefs.includes(i)&&a.chiefs.push(i),o==="conductor"&&i&&!a.conductors.includes(i)&&a.conductors.push(i),!o){const l=ym(s.employees).toLowerCase();l.includes("началник")&&l.includes("влак")&&i&&!a.chiefs.includes(i)&&a.chiefs.push(i),l.includes("кондуктор")&&i&&!a.conductors.includes(i)&&a.conductors.push(i)}r.set(s.duty_id,a)}),{assignmentsByDuty:r,absentAssignments:Array.from(n.values()).sort((s,a)=>String((s==null?void 0:s.employeeName)||"").localeCompare(String((a==null?void 0:a.employeeName)||""),"bg"))}}function Er(e,t,r){$s(e.querySelector("#plan-schedule-train"),t.train,r,{conductorRows:2,showHours:!0,separateSecondDay:!0,minPanels:2,printAsCards:!0}),$s(e.querySelector("#plan-schedule-business-trip"),t.businessTrip,r,{conductorRows:3,showHours:!1,minPanels:1,hideEmptyConductorRows:!0,printAsCards:!0}),$s(e.querySelector("#plan-schedule-day-off"),t.dayOff,r,{conductorRows:3,showHours:!1,minPanels:1,hideEmptyConductorRows:!0,printAsCards:!0})}function $r(e,t){if(!e)return;if(!t.length){e.innerHTML='<p class="text-secondary mb-0">Няма служители в разход.</p>';return}const r=t.map(n=>`
      <tr>
  ...
  `}function Ar(e,{hint:t,error:r,empty:n}){const s=e.querySelector("#plan-schedule-hint"),a=e.querySelector("#plan-schedule-error"),i=e.querySelector("#plan-schedule-empty");s&&(s.textContent=t||"",s.classList.toggle("d-none",!t)),a&&(a.textContent=r||"",a.classList.toggle("d-none",!r)),i&&(i.textContent=n||"",i.classList.toggle("d-none",!n))}function Lm(e){const t=new Date(`${e}T00:00:00`);return Number.isNaN(t.getTime())?e:new Intl.DateTimeFormat("bg-BG",{day:"2-digit",month:"long",year:"numeric"}).format(t)}function $s(e,t,r,n={}){if(!e)return;if(!t.length){e.innerHTML='<p class="text-secondary mb-0">Няма повески от този тип.</p>';return}const s=n.separateSecondDay?Am(t):t,a=Number.isInteger(n.conductorRows)&&n.conductorRows>=0?n.conductorRows:3,i=5,o=Em(s,i),l=Number.isInteger(n.minPanels)&&n.minPanels>0?n.minPanels:1;for(;o.length<l;)o.push([]);e.innerHTML=o.map(d=>{const c=[...d];for(;c.length<i;)c.push(null);const u=c.map(b=>{const _=In(b,"text-center"),k=ut(b)?"":(b==null?void 0:b.name)??"";if(!k)return`<th scope="col"${_}></th>`;const x=String((b==null?void 0:b.notes)||"").trim(),L=x?`<div class="schedule-duty-note" title="${$e(x)}">${$e(x)}</div>`:"";return`<th scope="col"${_}><span class="schedule-duty-name-wrap">${Rn("Влак","train")}<span class="schedule-duty-name-ellipsis" title="${$e(k)}">${$e(k)}</span></span>${L}</th>`}).join(""),p=c.map(b=>{const _=In(b),k=b&&!ut(b)?ll(b):"";return!b||ut(b)?`<td${_}></td>`:`<td${_}>${Rn("Час","hours")}${$e(k)}</td>`}).join(""),m=c.map(b=>{if(!b)return"<td></td>";const _=In(b);if(ut(b))return`<td${_}></td>`;const k=r.get(b.id)||{chiefs:[]},x=k.chiefs.length?k.chiefs.join(", "):"";return`<td${_}>${Rn("НВ","chief")}${$e(x)}</td>`}).join("");let h=a;if(n.hideEmptyConductorRows){const b=c.reduce((_,k)=>{if(!k||ut(k))return _;const x=r.get(k.id)||{conductors:[]},L=Array.isArray(x.conductors)?x.conductors.length:0;return Math.max(_,L)},0);h=Math.min(a,b)}const f=h>0?Array.from({length:h},(b,_)=>`
            <tr>
              ${c.map(x=>{if(!x)return"<td></td>";const L=In(x);if(ut(x))return`<td${L}></td>`;const T=(r.get(x.id)||{conductors:[]}).conductors[_]||"";return`<td${L}>${Rn("К-р","conductor")}${$e(T)}</td>`}).join("")}
            </tr>
      ...
      `;if(!n.printAsCards)return y;const v=Tm(c,r,h,n);return`
        <div class="print-as-cards">
          ${y}
          <div class="print-only-duty-cards mb-3">${v}</div>
        </div>
      `}).join("")}function Tm(e,t,r,n={}){return`<div class="print-duty-cards-grid">${e.map(a=>{const i=n.showHours!==!1,o=i?`
        ...
        <article class="print-duty-card">
          ...
          ${i?`
            <div class="print-duty-card-line">
              <span class="print-duty-card-key">Час</span>
              <span class="print-duty-card-value">${$e(ll(a))}</span>
            </div>
          `:""}
          <div class="print-duty-card-line">
            <span class="print-duty-card-key">НВ</span>
      ...
      `}).join("")}</div>`}function Em(e,t){const r=[];for(let n=0;n<e.length;n+=t)r.push(e.slice(n,n+t));return r}function ll(e){const t=((e==null?void 0:e.start_time)||"").slice(0,5),r=((e==null?void 0:e.end_time)||"").slice(0,5);return!t&&!r?"":t&&r?`${t} - ${r}`:t||r}function Rn(e,t){return`<span class="${t?`schedule-cell-key-badge schedule-cell-key-badge-${t}`:"schedule-cell-key-badge"}">${$e(e)}</span>`}function Zs(e,t){const r=(e==null?void 0:e.schedule_key_id)||"",n=(t==null?void 0:t.schedule_key_id)||"";if(r!==n)return String(r).localeCompare(String(n),"bg");const s=Number.isFinite(Number(e==null?void 0:e.display_order))?Number(e.display_order):Number.MAX_SAFE_INTEGER,a=Number.isFinite(Number(t==null?void 0:t.display_order))?Number(t.display_order):Number.MAX_SAFE_INTEGER;return s!==a?s-a:String((e==null?void 0:e.name)||"").localeCompare(String((t==null?void 0:t.name)||""),"bg")}function $m(e,t){const r=!!(e!=null&&e.second_day),n=!!(t!=null&&t.second_day);if(r!==n)return r?1:-1;const s=Xi(e==null?void 0:e.start_time),a=Xi(t==null?void 0:t.start_time);return s!==a?s.localeCompare(a,"bg"):Zs(e,t)}function Am(e){const t=[],r=[];return e.forEach(a=>{if(a!=null&&a.second_day){r.push(a);return}t.push(a)}),!t.length||!r.length?e:t.length%5!==0?[...t,{__separator:!0},...r]:[...t,...r]}function ut(e){return!!(e&&e.__separator)}function In(e,t=""){const r=[];return t&&r.push(t),ut(e)?r.push("separator-col"):e!=null&&e.second_day&&r.push("second-day-col"),r.length?` class="${r.join(" ")}"`:""}async function Cm(e){e.innerHTML=hm,al(e,"#plan-schedule-print-left-label");const t=e.querySelector("#plan-schedule-date"),r=e.querySelector("#plan-schedule-print"),n=e.querySelector("#plan-schedule-print-orientation"),s=e.querySelector("#plan-schedule-print-compact"),a=e.querySelector("#plan-schedule-print-fit-one-page"),i=_m();t&&i?t.value=i:t&&!t.value&&(t.value=new Date().toISOString().split("T")[0]),t==null||t.addEventListener("change",async()=>{await eo(e)}),r==null||r.addEventListener("click",()=>{const o=(n==null?void 0:n.value)==="portrait"?"portrait":"landscape",l=(s==null?void 0:s.checked)??!0,d=(a==null?void 0:a.checked)??!0;wm(e,{orientation:o,compact:l,fitOnePage:d}),window.addEventListener("afterprint",Sm,{once:!0}),window.print()}),await eo(e)}async function eo(e){const t=e.querySelector("#plan-schedule-date"),r=t==null?void 0:t.value,n=e.querySelector("#plan-schedule-sheet-date");if(n&&(n.textContent=r?Lm(r):""),!r){Er(e,{train:[],businessTrip:[],dayOff:[]},new Map),$r(e.querySelector("#plan-schedule-absence"),[]),Ar(e,{hint:"Избери дата.",error:"",empty:""});return}const{data:s,error:a}=await S.from("planned_duties").select("employee_id, duty_id, assignment_role, employees(first_name, last_name, positions(title)), duties(id, name, schedule_key_id, display_order, start_time, end_time, second_day, duty_types(name))").eq("date",r);if(a){g(a.message,"error"),Er(e,{train:[],businessTrip:[],dayOff:[]},new Map),$r(e.querySelector("#plan-schedule-absence"),[]),Ar(e,{hint:"",error:"Грешка при зареждане на планираните записи.",empty:""});return}const{data:i,error:o}=await il(r);if(o){g(o.message,"error"),Er(e,{train:[],businessTrip:[],dayOff:[]},new Map),$r(e.querySelector("#plan-schedule-absence"),[]),Ar(e,{hint:"",error:"Грешка при зареждане на повеските.",empty:""});return}const{data:l,error:d}=await S.from("employee_absences").select("employee_id, start_date, end_date, employees(first_name, last_name), absence_reasons(name)").lte("start_date",r).gte("end_date",r);if(d){g(d.message,"error"),Er(e,{train:[],businessTrip:[],dayOff:[]},new Map),$r(e.querySelector("#plan-schedule-absence"),[]),Ar(e,{hint:"",error:"Грешка при зареждане на отсъствията.",empty:""});return}const c=km(l||[]),u=xm((i||[]).map(f=>({duties:f}))),{assignmentsByDuty:p,absentAssignments:m}=qm(s||[],c);Er(e,u,p),$r(e.querySelector("#plan-schedule-absence"),m);const h=u.train.length+u.businessTrip.length+u.dayOff.length;Ar(e,{hint:"",error:"",empty:h||m.length?"":"Няма повески за показване по избраните типове."})}const Rm=`<section class="card border-0 shadow-sm schedule-page-root">\r
  <div class="card-body p-4 p-md-5">\r
...
`,to=new Map;function ro(e){const t=String(e||"").trim();if(!t)return"99:99:99";const r=t.match(/^(\d{1,2}):(\d{2})(?::(\d{2}))?/);if(!r)return"99:99:99";const n=String(Number(r[1])).padStart(2,"0"),s=r[2],a=r[3]||"00";return`${n}:${s}:${a}`}function Im(e){const t=(e==null?void 0:e.first_name)??"",r=(e==null?void 0:e.last_name)??"";return`${t} ${r}`.trim()}function Dm(e){var r;const t=e==null?void 0:e.positions;return Array.isArray(t)?((r=t[0])==null?void 0:r.title)??"":t&&typeof t=="object"?t.title??"":""}function ea(e){const t=String((e==null?void 0:e.assignment_role)||"").trim().toLowerCase();if(t==="chief"||t==="conductor")return t;const r=Dm(e==null?void 0:e.employees).toLowerCase();return r.includes("началник")&&r.includes("влак")?"chief":"conductor"}function jt(e){var r;const t=e==null?void 0:e.duty_types;return Array.isArray(t)?((r=t[0])==null?void 0:r.name)??"":t&&typeof t=="object"?t.name??"":""}function Qn(e){const t=e==null?void 0:e.duties;return Array.isArray(t)?t[0]||null:t&&typeof t=="object"?t:null}function Om(){const t=new URLSearchParams(window.location.search).get("date")||"";return/^\d{4}-\d{2}-\d{2}$/.test(t)?t:""}function _e(e){return String(e).replaceAll("&","&amp;").replaceAll("<","&lt;").replaceAll(">","&gt;").replaceAll('"',"&quot;").replaceAll("'","&#039;")}function Pm(e){e==null||e.classList.remove("d-none"),document.body.classList.add("overflow-hidden")}function Un(e){e==null||e.classList.add("d-none"),!!document.querySelector("#schedule-actual-edit-modal:not(.d-none)")||document.body.classList.remove("overflow-hidden")}function Mm(e,t){const r=to.get(e);r&&document.removeEventListener("keydown",r);const n=s=>{if(s.key==="Escape"){for(const a of t)if(a&&!a.classList.contains("d-none")){Un(a);return}}};to.set(e,n),document.addEventListener("keydown",n)}const no=96/25.4;function jm(e,{orientation:t,compact:r,fitOnePage:n}){const s=document.documentElement,a=e.querySelector(".plan-schedule-sheet");if(s.classList.add("print-preparing"),s.classList.add("print-hide-second-day"),s.classList.toggle("print-compact",r),s.classList.toggle("print-fit-one-page",n),a&&(a.classList.toggle("print-landscape-page",t==="landscape"),a.classList.toggle("print-portrait-page",t==="portrait")),!n||!a){s.style.setProperty("--plan-print-scale","1");return}s.style.setProperty("--plan-print-scale","1");const i=a.getBoundingClientRect(),o=t==="portrait"?210:297,l=t==="portrait"?297:210,d=(o-20)*no,c=(l-20)*no,u=d/Math.max(i.width,1),p=c/Math.max(i.height,1),m=Math.min(u,p,1);s.style.setProperty("--plan-print-scale",String(Math.max(.6,m)))}function Nm(){const e=document.documentElement;e.classList.remove("print-preparing","print-compact","print-fit-one-page","print-hide-second-day"),e.style.setProperty("--plan-print-scale","1"),document.querySelectorAll(".plan-schedule-sheet").forEach(t=>{t.classList.remove("print-landscape-page","print-portrait-page")})}function Hm(e,t){const r=new Map;return e.forEach(n=>{var l;if(!(n!=null&&n.duty_id)||!(n!=null&&n.employees)||!(n!=null&&n.id)||n!=null&&n.employee_id&&(t!=null&&t.has(n.employee_id)))return;const s=r.get(n.duty_id)||{chiefs:[],conductors:[]},a=Im(n.employees),i=ea(n),o={id:n.id,employeeId:n.employee_id,role:i,name:a,dutyName:((l=Qn(n))==null?void 0:l.name)||"",date:n.date||""};i==="chief"?a&&!s.chiefs.some(d=>d.id===o.id)&&s.chiefs.push(o):i==="conductor"&&a&&!s.conductors.some(d=>d.id===o.id)&&s.conductors.push(o),r.set(n.duty_id,s)}),r}function Cr(e,t,r,n){As(e.querySelector("#schedule-train"),t.train,r,n,{allowAdd:!0,allowEdit:!0,conductorRows:2,printConductorRows:3,printExtraCardRows:1,showHours:!0,separateSecondDay:!0,minPanels:2,printAsCards:!0,printHideSecondDay:!0}),As(e.querySelector("#schedule-business-trip"),t.businessTrip,r,n,{allowAdd:!0,allowEdit:!0,conductorRows:3,showHours:!1,minPanels:1,hideEmptyConductorRows:!0}),As(e.querySelector("#schedule-day-off"),t.dayOff,r,n,{allowAdd:!0,allowEdit:!0,conductorRows:3,showHours:!1,minPanels:1,hideEmptyConductorRows:!0})}function Rr(e,{hint:t,error:r,empty:n}){const s=e.querySelector("#schedule-hint"),a=e.querySelector("#schedule-error"),i=e.querySelector("#schedule-empty");s&&(s.textContent=t||"",s.classList.toggle("d-none",!t)),a&&(a.textContent=r||"",a.classList.toggle("d-none",!r)),i&&(i.textContent=n||"",i.classList.toggle("d-none",!n))}function Um(e){const t=new Date(`${e}T00:00:00`);return Number.isNaN(t.getTime())?e:new Intl.DateTimeFormat("bg-BG",{day:"2-digit",month:"long",year:"numeric"}).format(t)}function ta(e,t){const r=(e==null?void 0:e.schedule_key_id)||"",n=(t==null?void 0:t.schedule_key_id)||"";if(r!==n)return String(r).localeCompare(String(n),"bg");const s=Number.isFinite(Number(e==null?void 0:e.display_order))?Number(e.display_order):Number.MAX_SAFE_INTEGER,a=Number.isFinite(Number(t==null?void 0:t.display_order))?Number(t.display_order):Number.MAX_SAFE_INTEGER;return s!==a?s-a:String((e==null?void 0:e.name)||"").localeCompare(String((t==null?void 0:t.name)||""),"bg")}function Fm(e,t){const r=!!(e!=null&&e.second_day),n=!!(t!=null&&t.second_day);if(r!==n)return r?1:-1;const s=ro(e==null?void 0:e.start_time),a=ro(t==null?void 0:t.start_time);return s!==a?s.localeCompare(a,"bg"):ta(e,t)}function As(e,t,r,n,s={}){if(!e)return;if(!t.length){e.innerHTML='<p class="text-secondary mb-0">Няма повески от този тип.</p>';return}const a=s.separateSecondDay?Km(t):t,i=Number.isInteger(s.conductorRows)&&s.conductorRows>=0?s.conductorRows:3,o=5,l=zm(a,o),d=Number.isInteger(s.minPanels)&&s.minPanels>0?s.minPanels:1,c=Number.isInteger(s.printConductorRows)&&s.printConductorRows>0?s.printConductorRows:i;for(;l.length<d;)l.push([]);const u=l.map(h=>{const f=[...h];for(;f.length<o;)f.push(null);const w=f.map(q=>{const T=On(q,"text-center"),E=pt(q)?"":(q==null?void 0:q.name)??"";if(!E)return`<th scope="col"${T}></th>`;const $=String((q==null?void 0:q.notes)||"").trim(),A=$?`<div class="schedule-duty-note" title="${_e($)}">${_e($)}</div>`:"";return`<th scope="col"${T}><span class="schedule-duty-name-wrap">${Dn("Влак","train")}<span class="schedule-duty-name-ellipsis" title="${_e(E)}">${_e(E)}</span></span>${A}</th>`}).join(""),y=f.map(q=>{const T=On(q),E=q&&!pt(q)?ul(q):"";return!q||pt(q)?`<td${T}></td>`:`<td${T}>${Dn("Час","hours")}${_e(E)}</td>`}).join(""),v=f.map(q=>{if(!q)return"<td></td>";const T=On(q);if(pt(q))return`<td${T}></td>`;const E=r.get(q.id)||{chiefs:[]},$=jt(q).toLowerCase();return`<td${T} data-drop-duty-id="${q.id}" data-drop-duty-name="${_e((q==null?void 0:q.name)||"")}" data-drop-date="${n}" data-drop-role="chief" data-drop-duty-type="${_e($)}">${Dn("НВ","chief")}${Bm(E.chiefs,q,n,s)}</td>`}).join("");let b=i;if(s.hideEmptyConductorRows){const q=f.reduce((T,E)=>{if(!E||pt(E))return T;const $=r.get(E.id)||{conductors:[]},A=Array.isArray($.conductors)?$.conductors.length:0;return Math.max(T,A)},0);b=Math.min(i,q)}const _=b>0?Array.from({length:b},(q,T)=>`
          <tr>
            ${f.map($=>{if(!$)return"<td></td>";const A=On($);if(pt($))return`<td${A}></td>`;const C=r.get($.id)||{conductors:[]},I=Number.isInteger(s.conductorRowOffset)&&s.conductorRowOffset>0?s.conductorRowOffset:0,R=T-I,W=R>=0&&Array.isArray(C.conductors)?C.conductors[R]:void 0,K=jt($).toLowerCase();return`<td${A} data-drop-duty-id="${$.id}" data-drop-duty-name="${_e(($==null?void 0:$.name)||"")}" data-drop-date="${n}" data-drop-role="conductor" data-drop-duty-type="${_e(K)}">${Dn("К-р","conductor")}${dl(W,$,n,s)}</td>`}).join("")}
          </tr>
      ...
      `;if(!s.printAsCards)return x;const L=so(f,r,c,s);return`
        <div class="print-as-cards">
          ${x}
      ...
      `}).join(""),p=Number.isInteger(s.printExtraCardRows)&&s.printExtraCardRows>0?s.printExtraCardRows:0,m=s.printAsCards&&p>0?Array.from({length:p},()=>`
        <div class="print-as-cards">
          <div class="print-only-duty-cards mb-3">${so(Array.from({length:o},()=>null),new Map,c,s)}</div>
        </div>
      `).join(""):"";e.innerHTML=u+m}function so(e,t,r,n={}){return`<div class="print-duty-cards-grid">${e.map(a=>{const i=n.printHideSecondDay&&(a!=null&&a.second_day)?null:a;if(!i||pt(i))return`
          <article class="print-duty-card">
            <div class="print-duty-card-title"></div>
            <div class="print-duty-card-note"></div>
            <div class="print-duty-card-line">
        ...
        <article class="print-duty-card">
          ...
          <div class="print-duty-card-line">
            <span class="print-duty-card-key">Час</span>
            <span class="print-duty-card-value">${_e(ul(i))}</span>
          </div>
          <div class="print-duty-card-line">
            <span class="print-duty-card-key">НВ</span>
...
`;function Rs(e){e.classList.remove("d-none"),document.body.classList.add("overflow-hidden")}const io=new Map;function ef(e,t){const r=io.get(e);r&&document.removeEventListener("keydown",r);const n=s=>{if(s.key==="Escape"){for(const a of t)if(a&&!a.classList.contains("d-none")){gr(a);return}}};io.set(e,n),document.addEventListener("keydown",n)}function gr(e){var t,r;e.classList.add("d-none"),(t=document.querySelector("#duty-type-modal"))!=null&&t.classList.contains("d-none")&&((r=document.querySelector("#duty-type-delete-modal"))!=null&&r.classList.contains("d-none"))&&document.body.classList.remove("overflow-hidden")}function oo(e){return String(e).replaceAll("&","&amp;").replaceAll("<","&lt;").replaceAll(">","&gt;").replaceAll('"',"&quot;").replaceAll("'","&#039;")}const nt={rows:[],searchQuery:"",actionsEnabled:!0};async function Ia(e){const{data:t,error:r}=await S.from("duty_types").select("id, name").order("name",{ascending:!0});if(r){g(r.message,"error"),nt.rows=[],ra(e,"Грешка при зареждане на типовете повески.");return}nt.rows=t||[],ra(e)}function ra(e,t){var i;const r=e.querySelector("#duty-types-table-body"),n=e.querySelector("#duty-types-empty"),s=nt.actionsEnabled!==!1;(i=e.querySelector("thead th.text-end"))==null||i.classList.toggle("d-none",!s);const a=nt.rows.filter(o=>nt.searchQuery?(o.name||"").toLowerCase().includes(nt.searchQuery):!0);if(!a.length){r.innerHTML="",n.classList.remove("d-none"),n.textContent=t||"Няма въведени типове повески.";return}n.classList.add("d-none"),r.innerHTML=a.map(o=>`
        <tr>
          <td>${oo(o.name??"-")}</td>
          ${s?`
          <td class="text-end">
      ...
      `).join("")}async function tf(e){e.innerHTML=Zm,nt.actionsEnabled=!await hn(),rf(e),nf(e),await Ia(e)}function rf(e){var t,r;nt.actionsEnabled||((t=e.querySelector("#open-create-duty-type"))==null||t.classList.add("d-none"),(r=e.querySelector("thead th.text-end"))==null||r.classList.add("d-none"))}function nf(e){const t=e.querySelector("#open-create-duty-type"),r=e.querySelector("#duty-type-form"),n=e.querySelector("#duty-type-cancel-btn"),s=e.querySelector("#duty-types-table-body"),a=e.querySelector("#duty-type-modal"),i=e.querySelector("#duty-type-delete-modal"),o=e.querySelector("#duty-type-modal-close"),l=e.querySelector("#duty-type-delete-confirm"),d=e.querySelector("#duty-type-delete-cancel"),c=e.querySelector("#duty-types-search");t==null||t.addEventListener("click",()=>{Da(e),Rs(a)}),r==null||r.addEventListener("submit",async u=>{u.preventDefault(),await sf(e)}),n==null||n.addEventListener("click",()=>{gr(a)}),o==null||o.addEventListener("click",()=>{gr(a)}),d==null||d.addEventListener("click",()=>{gr(i)}),c==null||c.addEventListener("input",u=>{nt.searchQuery=u.target.value.trim().toLowerCase(),ra(e)}),ef("duty-types",[i,a]),l==null||l.addEventListener("click",async()=>{const u=e.querySelector("#duty-type-delete-id").value;await of(u,e)}),s==null||s.addEventListener("click",u=>{const p=u.target.closest("button[data-action]");if(!p)return;const m=p.getAttribute("data-action");if(m==="edit"){af(e,{id:p.getAttribute("data-id"),name:p.getAttribute("data-name")}),Rs(a);return}if(m==="delete"){const h=p.getAttribute("data-id");e.querySelector("#duty-type-delete-id").value=h,Rs(i)}})}async function sf(e){var l;const t=e.querySelector("#duty-type-id"),r=e.querySelector("#duty-type-name"),n=e.querySelector("#duty-type-save-btn"),s=r.value.trim(),a=t.value;if(!s){g("Моля, попълни наименование.","warning");return}const i=n.innerHTML;n.disabled=!0,n.innerHTML='<span class="spinner-border spinner-border-sm me-2" role="status" aria-hidden="true"></span>Запис...';let o;if(a)({error:o}=await S.from("duty_types").update({name:s}).eq("id",a));else{const{data:d}=await S.auth.getUser(),c=((l=d==null?void 0:d.user)==null?void 0:l.email)??"web_app";({error:o}=await S.from("duty_types").insert({name:s,created_from:c}))}if(n.disabled=!1,n.innerHTML=i,o){if(o.code==="23505"){g("Такъв тип вече съществува.","warning");return}g(o.message,"error");return}g(a?"Типът е обновен.":"Типът е създаден.","success"),gr(e.querySelector("#duty-type-modal")),Da(e),await Ia(e)}function af(e,t){e.querySelector("#duty-type-id").value=t.id,e.querySelector("#duty-type-name").value=t.name??"",e.querySelector("#duty-type-form-title").textContent="Редакция на тип",e.querySelector("#duty-type-save-btn").textContent="Запази"}function Da(e){e.querySelector("#duty-type-id").value="",e.querySelector("#duty-type-name").value="",e.querySelector("#duty-type-form-title").textContent="Нов тип",e.querySelector("#duty-type-save-btn").textContent="Създай"}async function of(e,t){const r=t.querySelector("#duty-type-delete-confirm"),n=r.innerHTML;r.disabled=!0,r.innerHTML='<span class="spinner-border spinner-border-sm me-2" role="status" aria-hidden="true"></span>Изтриване...';const{count:s,error:a}=await S.from("duties").select("id",{count:"exact",head:!0}).eq("duty_type_id",e);if(a){r.disabled=!1,r.innerHTML=n,g(a.message,"error");return}if((s||0)>0){r.disabled=!1,r.innerHTML=n,g("Типът не може да се изтрие, защото се използва от повески.","warning");return}const{error:i}=await S.from("duty_types").delete().eq("id",e);if(r.disabled=!1,r.innerHTML=n,i){if(i.code==="23503"){g("Типът не може да се изтрие, защото се използва от повески.","warning");return}g(i.message,"error");return}g("Типът е изтрит.","success"),gr(t.querySelector("#duty-type-delete-modal")),Da(t),await Ia(t)}const lf=`<section class="card border-0 shadow-sm">\r
  <div class="card-body p-4 p-md-5">\r
    <div class="d-flex justify-content-between align-items-center mb-3">\r
    ...
    <div class="card w-100" style="max-width: 960px;">\r
      <div class="card-body p-4" style="max-height: calc(100vh - 64px); overflow: auto;">\r
...
</div>`;function Ge(e){return String(e).replaceAll("&","&amp;").replaceAll("<","&lt;").replaceAll(">","&gt;").replaceAll('"',"&quot;").replaceAll("'","&#039;")}function lo(e){return e?typeof e=="string"?e.replace(".000000",""):String(e):"-"}function df(){return new URLSearchParams(window.location.search).get("scheduleKeyId")||""}function cf(){return new URLSearchParams(window.location.search).get("scheduleKeyName")||""}function an(e){e==null||e.classList.remove("d-none"),document.body.classList.add("overflow-hidden")}const co=new Map;function uf(e,t){const r=co.get(e);r&&document.removeEventListener("keydown",r);const n=s=>{if(s.key==="Escape"){for(const a of t)if(a&&!a.classList.contains("d-none")){He(a);return}}};co.set(e,n),document.addEventListener("keydown",n)}function He(e){var a,i,o,l;e==null||e.classList.add("d-none");const t=(a=document.querySelector("#schedule-key-duty-create-modal"))==null?void 0:a.classList.contains("d-none"),r=(i=document.querySelector("#schedule-key-duty-edit-modal"))==null?void 0:i.classList.contains("d-none"),n=(o=document.querySelector("#schedule-key-duty-delete-modal"))==null?void 0:o.classList.contains("d-none"),s=(l=document.querySelector("#schedule-key-duty-profile-modal"))==null?void 0:l.classList.contains("d-none");t&&r&&n&&s&&document.body.classList.remove("overflow-hidden")}const te={scheduleKeyId:"",scheduleKeyName:"",duties:[],draggedDutyId:null,reorderEnabled:!0,lastCreatedDutyTypeId:"",lastCreatedScheduleKeyIds:[]},uo="id, name, notes, duty_type_id, schedule_key_id, start_time, end_time, second_day, break_start_time, break_end_time, break_duration_interval, duration_interval, display_order, duty_types(name), schedule_key_duties(schedule_key_id, schedule_keys(name)), duty_trains(train_id, sequence_order, trains(number))";async function fn(e){const t=te.scheduleKeyId,{data:r,error:n}=await S.from("duties").select(uo).eq("schedule_key_id",t).order("display_order",{ascending:!0}).order("name",{ascending:!0});if(n){te.duties=[],ur(e,"Грешка при зареждане на повеските."),g(n.message,"error");return}const{data:s,error:a}=await S.from("schedule_key_duties").select("duty_id").eq("schedule_key_id",t);if(a){te.duties=[],ur(e,"Грешка при зареждане на повеските."),g(a.message,"error");return}const i=r||[],o=new Set(i.map(c=>c==null?void 0:c.id).filter(Boolean)),l=[...new Set((s||[]).map(c=>c==null?void 0:c.duty_id).filter(Boolean))].filter(c=>!o.has(c));let d=[];if(l.length){const{data:c,error:u}=await S.from("duties").select(uo).in("id",l);if(u){te.duties=[],ur(e,"Грешка при зареждане на повеските."),g(u.message,"error");return}d=c||[]}te.duties=[...i,...d].sort((c,u)=>{const p=Number.isFinite(Number(c==null?void 0:c.display_order))?Number(c.display_order):Number.MAX_SAFE_INTEGER,m=Number.isFinite(Number(u==null?void 0:u.display_order))?Number(u.display_order):Number.MAX_SAFE_INTEGER;return p!==m?p-m:String((c==null?void 0:c.name)||"").localeCompare(String((u==null?void 0:u.name)||""),"bg")}),ur(e)}function ur(e,t){const r=e.querySelector("#schedule-key-duties-body"),n=e.querySelector("#schedule-key-duties-empty");if(!te.duties.length){r.innerHTML="",n.classList.remove("d-none"),n.textContent=t||"Няма повески към този Ключ-График.";return}const s=te.reorderEnabled!==!1;n.classList.add("d-none"),r.innerHTML=te.duties.map(a=>{const i=hf(a),o=mf(a),l=`<span class="badge text-bg-info" title="${Ge(o.join(", "))}">${i.length} кл-гр</span>`;return`
        <tr data-duty-id="${a.id}" draggable="${s?"true":"false"}">
          ...
          <td>${Ge(lo(a.break_duration_interval))}</td>
          <td>${Ge(lo(a.duration_interval))}</td>
          <td class="text-end">
            <div class="d-inline-flex gap-2">
              <button
                type="button"
      ...
      `}).join("")}async function pf(){const e=te.duties.map((n,s)=>S.from("duties").update({display_order:s+1}).eq("id",n.id)),r=(await Promise.all(e)).find(n=>n.error);return r!=null&&r.error?(g(r.error.message,"error"),!1):(te.duties=te.duties.map((n,s)=>({...n,display_order:s+1})),!0)}function pl(e){return Array.isArray(e.schedule_key_duties)?e.schedule_key_duties:e.schedule_key_duties?[e.schedule_key_duties]:[]}function hf(e){const t=pl(e).map(n=>n==null?void 0:n.schedule_key_id).filter(Boolean),r=t.length?t:e.schedule_key_id?[e.schedule_key_id]:[];return[...new Set(r)]}function mf(e){const t=pl(e).map(r=>{var n;return(n=r==null?void 0:r.schedule_keys)==null?void 0:n.name}).filter(Boolean);return[...new Set(t)]}async function ff(e){e.innerHTML=lf,te.reorderEnabled=!await hn(),yf(e),bf(e),await gf(e),await _f(e),await wf(e),await vf(e)}function yf(e){const t=e.querySelector("#schedule-key-duty-create-form-fields");t&&(t.innerHTML=Js({idPrefix:"schedule-key-duty-create"}));const r=e.querySelector("#schedule-key-duty-edit-form-fields");r&&(r.innerHTML=Js({idPrefix:"schedule-key-duty-edit"}))}function bf(e){const t=e.querySelector("#open-create-schedule-key-duty"),r=e.querySelector("#schedule-key-duty-create-modal"),n=e.querySelector("#schedule-key-duty-create-form"),s=e.querySelector("#schedule-key-duty-create-modal-close"),a=e.querySelector("#schedule-key-duty-create-cancel"),i=e.querySelector("#schedule-key-duties-body"),o=e.querySelector("#schedule-key-duty-edit-modal"),l=e.querySelector("#schedule-key-duty-delete-modal"),d=e.querySelector("#schedule-key-duty-edit-form"),c=e.querySelector("#schedule-key-duty-edit-modal-close"),u=e.querySelector("#schedule-key-duty-edit-cancel"),p=e.querySelector("#schedule-key-duty-delete-cancel"),m=e.querySelector("#schedule-key-duty-delete-confirm"),h=e.querySelector("#schedule-key-duty-profile-modal"),f=e.querySelector("#schedule-key-duty-profile-close"),w=e.querySelector("#schedule-key-duty-profile-close-secondary"),y=e.querySelector("#schedule-key-duty-profile-edit");t==null||t.addEventListener("click",()=>{Oa(e),an(r)}),s==null||s.addEventListener("click",()=>{He(r)}),a==null||a.addEventListener("click",()=>{He(r)}),n==null||n.addEventListener("submit",async v=>{v.preventDefault(),await Sf(e)}),d==null||d.addEventListener("submit",async v=>{v.preventDefault(),await kf(e)}),c==null||c.addEventListener("click",()=>{He(o)}),u==null||u.addEventListener("click",()=>{He(o)}),p==null||p.addEventListener("click",()=>{He(l)}),m==null||m.addEventListener("click",async()=>{const v=e.querySelector("#schedule-key-duty-delete-id").value;await Cf(e,v)}),f==null||f.addEventListener("click",()=>{He(h)}),w==null||w.addEventListener("click",()=>{He(h)}),y==null||y.addEventListener("click",()=>{var b;const v=((b=h==null?void 0:h.dataset)==null?void 0:b.dutyId)||"";v&&(He(h),po(e,v))}),uf("schedule-key-duties",[h,l,o,r]),i==null||i.addEventListener("dragstart",v=>{if(!te.reorderEnabled)return;const b=v.target.closest("tr[data-duty-id]");b&&(te.draggedDutyId=b.getAttribute("data-duty-id"),b.classList.add("table-active"))}),i==null||i.addEventListener("dragend",v=>{if(!te.reorderEnabled)return;const b=v.target.closest("tr[data-duty-id]");b&&b.classList.remove("table-active"),te.draggedDutyId=null}),i==null||i.addEventListener("dragover",v=>{te.reorderEnabled&&v.preventDefault()}),i==null||i.addEventListener("drop",async v=>{if(!te.reorderEnabled)return;v.preventDefault();const b=v.target.closest("tr[data-duty-id]"),_=te.draggedDutyId;if(!b||!_)return;const k=b.getAttribute("data-duty-id");if(!k||k===_)return;const x=te.duties.findIndex(E=>E.id===_),L=te.duties.findIndex(E=>E.id===k);if(x<0||L<0)return;const[q]=te.duties.splice(x,1);if(te.duties.splice(L,0,q),ur(e),!await pf()){await fn(e);return}g("Редът на повеските е запазен.","success")}),i==null||i.addEventListener("click",async v=>{const b=v.target.closest("button[data-duty-action]");if(!b)return;const _=b.getAttribute("data-duty-action");if(_==="profile"){const k=b.getAttribute("data-id");xf(e,k);return}if(_==="edit"){const k=b.getAttribute("data-id");po(e,k);return}if(_==="delete"){const k=b.getAttribute("data-id");Af(e,k)}})}async function vf(e){if(te.scheduleKeyId=df(),te.scheduleKeyName=cf(),!te.scheduleKeyId){ur(e,"Няма избран Ключ-График. Върни се и избери запис."),e.querySelector("#open-create-schedule-key-duty").classList.add("d-none");return}if(!te.scheduleKeyName){const{data:t,error:r}=await S.from("schedule_keys").select("name").eq("id",te.scheduleKeyId).single();r&&g(r.message,"error"),te.scheduleKeyName=(t==null?void 0:t.name)||te.scheduleKeyId}e.querySelector("#schedule-key-duties-title").textContent=te.scheduleKeyName,Oa(e),await fn(e)}async function gf(e){const t=e.querySelector("#schedule-key-duty-create-type"),r=e.querySelector("#schedule-key-duty-edit-type"),{data:n,error:s}=await S.from("duty_types").select("id, name").order("name",{ascending:!0});if(s){g(s.message,"error");return}const a=(n||[]).map(i=>`<option value="${i.id}">${Ge(i.name)}</option>`).join("");t.innerHTML='<option value="">Избери тип</option>'+a,r.innerHTML='<option value="">Избери тип</option>'+a}async function _f(e){const t=e.querySelector("#schedule-key-duty-create-schedule-keys"),r=e.querySelector("#schedule-key-duty-edit-schedule-keys"),{data:n,error:s}=await S.from("schedule_keys").select("id, name").order("name",{ascending:!0});if(s){g(s.message,"error");return}const a=(n||[]).map(i=>`<option value="${i.id}">${Ge(i.name)}</option>`).join("");t.innerHTML=a,r.innerHTML=a}async function wf(e){const t=e.querySelector("#schedule-key-duty-create-trains"),r=e.querySelector("#schedule-key-duty-edit-trains"),{data:n,error:s}=await S.from("trains").select("id, number, origin_station, destination_station").order("number",{ascending:!0});if(s){g(s.message,"error");return}const a=(n||[]).map(i=>{const o=`${i.origin_station||"-"} - ${i.destination_station||"-"}`;return`<option value="${i.id}">${Ge(i.number||"-")} (${Ge(o)})</option>`}).join("");t&&(t.innerHTML=a),r&&(r.innerHTML=a)}async function Sf(e){var W,K;const t=e.querySelector("#schedule-key-duty-create-name"),r=e.querySelector("#schedule-key-duty-create-type"),n=e.querySelector("#schedule-key-duty-create-schedule-keys"),s=st(e,"#schedule-key-duty-create-start","#schedule-key-duty-create-start-time"),a=st(e,"#schedule-key-duty-create-end","#schedule-key-duty-create-end-time"),i=e.querySelector("#schedule-key-duty-create-second-day"),o=st(e,"#schedule-key-duty-create-break-start","#schedule-key-duty-create-break-start-time"),l=st(e,"#schedule-key-duty-create-break-end","#schedule-key-duty-create-break-end-time"),d=e.querySelector("#schedule-key-duty-create-trains"),c=e.querySelector("#schedule-key-duty-create-save"),u=t.value.trim(),p=r.value||null,m=Array.from(n.selectedOptions||[]).map(V=>V.value).filter(Boolean),h=m[0]||null,f=(s==null?void 0:s.value)||"",w=(a==null?void 0:a.value)||"",y=i.checked,v=(o==null?void 0:o.value)||"00:00",b=(l==null?void 0:l.value)||"00:00",_=e.querySelector("#schedule-key-duty-create-notes").value.trim()||null,k=Array.from(d.selectedOptions||[]).map(V=>V.value).filter(Boolean);if(!te.scheduleKeyId||!u||!p||!f||!w){g("Моля, попълни всички полета за повеската.","warning");return}if(!m.length){g("Избери поне един ключ-график.","warning");return}const x=Ie(f,w);if(Ie(v,b)>x){g("Прекъсването не може да е по-голямо от продължителността на повеската.","warning");return}const q=c.innerHTML;c.disabled=!0,c.innerHTML='<span class="spinner-border spinner-border-sm me-2" role="status" aria-hidden="true"></span>Добавяне...';const{data:T}=await S.auth.getUser(),E=((W=T==null?void 0:T.user)==null?void 0:W.id)??((K=T==null?void 0:T.user)==null?void 0:K.email)??"web_app",$=te.duties.reduce((V,j)=>Math.max(V,Number(j.display_order)||0),0),{data:A,error:C}=await S.from("duties").insert({schedule_key_id:h,duty_type_id:p,name:u,start_time:f,end_time:w,second_day:y,break_start_time:v,break_end_time:b,notes:_,created_from:E,display_order:$+1}).select("id").single(),I=C?null:await hl(A==null?void 0:A.id,m),R=C||I?null:await ml(A==null?void 0:A.id,k);if(c.disabled=!1,c.innerHTML=q,C||I||R){g((C||I||R).message,"error");return}te.lastCreatedDutyTypeId=p,te.lastCreatedScheduleKeyIds=[...m],He(e.querySelector("#schedule-key-duty-create-modal")),Oa(e),g("Повеската е добавена към Ключ-График.","success"),await fn(e)}function po(e,t){const r=te.duties.find(o=>o.id===t);if(!r){g("Повеската не е намерена.","warning");return}e.querySelector("#schedule-key-duty-edit-id").value=r.id,e.querySelector("#schedule-key-duty-edit-name").value=r.name??"",e.querySelector("#schedule-key-duty-edit-type").value=r.duty_type_id??"";const n=e.querySelector("#schedule-key-duty-edit-schedule-keys"),s=Lf(r);Array.from(n.options).forEach(o=>{o.selected=s.includes(o.value)});const a=e.querySelector("#schedule-key-duty-edit-trains"),i=Ef(r);Array.from(a.options).forEach(o=>{o.selected=i.includes(o.value)}),gt(e,(r.start_time||"").slice(0,5),"#schedule-key-duty-edit-start","#schedule-key-duty-edit-start-time"),gt(e,(r.end_time||"").slice(0,5),"#schedule-key-duty-edit-end","#schedule-key-duty-edit-end-time"),e.querySelector("#schedule-key-duty-edit-second-day").checked=!!r.second_day,gt(e,Ht(r.break_start_time),"#schedule-key-duty-edit-break-start","#schedule-key-duty-edit-break-start-time"),gt(e,Ht(r.break_end_time),"#schedule-key-duty-edit-break-end","#schedule-key-duty-edit-break-end-time"),e.querySelector("#schedule-key-duty-edit-notes").value=r.notes??"",an(e.querySelector("#schedule-key-duty-edit-modal"))}function Oa(e){var s;e.querySelector("#schedule-key-duty-create-name").value="",e.querySelector("#schedule-key-duty-create-type").value=te.lastCreatedDutyTypeId||"";const t=(s=te.lastCreatedScheduleKeyIds)!=null&&s.length?te.lastCreatedScheduleKeyIds:[te.scheduleKeyId],r=e.querySelector("#schedule-key-duty-create-schedule-keys");Array.from(r.options).forEach(a=>{a.selected=t.includes(a.value)}),gt(e,"","#schedule-key-duty-create-start","#schedule-key-duty-create-start-time"),gt(e,"","#schedule-key-duty-create-end","#schedule-key-duty-create-end-time"),e.querySelector("#schedule-key-duty-create-second-day").checked=!1,gt(e,"00:00","#schedule-key-duty-create-break-start","#schedule-key-duty-create-break-start-time"),gt(e,"00:00","#schedule-key-duty-create-break-end","#schedule-key-duty-create-break-end-time"),e.querySelector("#schedule-key-duty-create-notes").value="";const n=e.querySelector("#schedule-key-duty-create-trains");Array.from(n.options).forEach(a=>{a.selected=!1})}async function kf(e){var _,k,x,L;const t=e.querySelector("#schedule-key-duty-edit-id").value,r=e.querySelector("#schedule-key-duty-edit-name").value.trim(),n=e.querySelector("#schedule-key-duty-edit-type").value||null,s=Array.from(e.querySelector("#schedule-key-duty-edit-schedule-keys").selectedOptions||[]).map(q=>q.value).filter(Boolean),a=s[0]||null,i=((_=st(e,"#schedule-key-duty-edit-start","#schedule-key-duty-edit-start-time"))==null?void 0:_.value)||"",o=((k=st(e,"#schedule-key-duty-edit-end","#schedule-key-duty-edit-end-time"))==null?void 0:k.value)||"",l=e.querySelector("#schedule-key-duty-edit-second-day").checked,d=((x=st(e,"#schedule-key-duty-edit-break-start","#schedule-key-duty-edit-break-start-time"))==null?void 0:x.value)||"00:00",c=((L=st(e,"#schedule-key-duty-edit-break-end","#schedule-key-duty-edit-break-end-time"))==null?void 0:L.value)||"00:00",u=e.querySelector("#schedule-key-duty-edit-notes").value.trim()||null,p=Array.from(e.querySelector("#schedule-key-duty-edit-trains").selectedOptions||[]).map(q=>q.value).filter(Boolean),m=e.querySelector("#schedule-key-duty-edit-save");if(!t||!r||!n||!i||!o){g("Моля, попълни всички полета за повеската.","warning");return}if(!s.length){g("Избери поне един ключ-график.","warning");return}const h=Ie(i,o);if(Ie(d,c)>h){g("Прекъсването не може да е по-голямо от продължителността на повеската.","warning");return}const w=m.innerHTML;m.disabled=!0,m.innerHTML='<span class="spinner-border spinner-border-sm me-2" role="status" aria-hidden="true"></span>Запис...';const{error:y}=await S.from("duties").update({name:r,duty_type_id:n,schedule_key_id:a,start_time:i,end_time:o,second_day:l,break_start_time:d,break_end_time:c,notes:u}).eq("id",t),v=y?null:await hl(t,s),b=y||v?null:await ml(t,p);if(m.disabled=!1,m.innerHTML=w,y||v||b){g((y||v||b).message,"error");return}He(e.querySelector("#schedule-key-duty-edit-modal")),g("Повеската е обновена.","success"),await fn(e)}async function hl(e,t){if(!e)return{message:"Липсва идентификатор на повеска за запис на ключ-графици."};const{error:r}=await S.from("schedule_key_duties").delete().eq("duty_id",e);if(r)return r;const n=t.map(a=>({duty_id:e,schedule_key_id:a})),{error:s}=await S.from("schedule_key_duties").insert(n);return s}async function ml(e,t){if(!e)return{message:"Липсва идентификатор на повеска за запис на влакове."};const{error:r}=await S.from("duty_trains").delete().eq("duty_id",e);if(r)return r;if(!t.length)return null;const n=t.map((a,i)=>({duty_id:e,train_id:a,sequence_order:i+1})),{error:s}=await S.from("duty_trains").insert(n);return s}function xf(e,t){const r=te.duties.find(l=>l.id===t),n=e.querySelector("#schedule-key-duty-profile-content"),s=e.querySelector("#schedule-key-duty-profile-modal"),a=e.querySelector("#schedule-key-duty-profile-edit");if(!n||!s)return;if(!r){s.dataset.dutyId="",a&&(a.disabled=!0),n.innerHTML='<p class="text-secondary mb-0">Няма данни за тази повеска.</p>',an(s);return}s.dataset.dutyId=r.id,a&&(a.disabled=!1);const i=Tf(r),o=$f(r);n.innerHTML=Vo({duty:r,scheduleKeyNames:i,trainNumbers:o,escapeHtml:Ge,intervalToTimeInput:Ht,formatInterval:qf}),an(s)}function qf(e){return e?String(e).replace(".000000",""):"-"}function st(e,...t){for(const r of t){const n=e.querySelector(r);if(n)return n}return null}function gt(e,t,...r){const n=st(e,...r);n&&(n.value=t)}function fl(e){return Array.isArray(e==null?void 0:e.schedule_key_duties)?e.schedule_key_duties:e!=null&&e.schedule_key_duties?[e.schedule_key_duties]:[]}function Lf(e){const t=fl(e).map(n=>n==null?void 0:n.schedule_key_id).filter(Boolean),r=t.length?t:e!=null&&e.schedule_key_id?[e.schedule_key_id]:[];return[...new Set(r)]}function Tf(e){const t=fl(e).map(r=>{var n;return(n=r==null?void 0:r.schedule_keys)==null?void 0:n.name}).filter(Boolean);return[...new Set(t)]}function yl(e){return Array.isArray(e==null?void 0:e.duty_trains)?e.duty_trains:e!=null&&e.duty_trains?[e.duty_trains]:[]}function Ef(e){return yl(e).map(t=>({id:t==null?void 0:t.train_id,sequenceOrder:Number.isFinite(Number(t==null?void 0:t.sequence_order))?Number(t.sequence_order):Number.MAX_SAFE_INTEGER})).filter(t=>!!t.id).sort((t,r)=>t.sequenceOrder-r.sequenceOrder).map(t=>t.id).filter((t,r,n)=>n.indexOf(t)===r)}function $f(e){return yl(e).map(t=>{var r;return{number:(r=t==null?void 0:t.trains)==null?void 0:r.number,sequenceOrder:Number.isFinite(Number(t==null?void 0:t.sequence_order))?Number(t.sequence_order):Number.MAX_SAFE_INTEGER}}).filter(t=>!!t.number).sort((t,r)=>t.sequenceOrder-r.sequenceOrder).map(t=>t.number).filter((t,r,n)=>n.indexOf(t)===r)}function Af(e,t){e.querySelector("#schedule-key-duty-delete-id").value=t,an(e.querySelector("#schedule-key-duty-delete-modal"))}async function Cf(e,t){const r=e.querySelector("#schedule-key-duty-delete-confirm"),n=r.innerHTML;r.disabled=!0,r.innerHTML='<span class="spinner-border spinner-border-sm me-2" role="status" aria-hidden="true"></span>Изтриване...';const{error:s}=await S.from("duties").delete().eq("id",t).eq("schedule_key_id",te.scheduleKeyId);if(r.disabled=!1,r.innerHTML=n,s){g(s.message,"error");return}He(e.querySelector("#schedule-key-duty-delete-modal")),g("Повеската е изтрита.","success"),await fn(e)}const Rf=`<section class="card border-0 shadow-sm">\r
  <div class="card-body p-4 p-md-5">\r
    <div class="d-flex justify-content-between align-items-center mb-3">\r
...
`;function Fn(e){e.classList.remove("d-none"),document.body.classList.add("overflow-hidden")}const ho=new Map;function If(e,t){const r=ho.get(e);r&&document.removeEventListener("keydown",r);const n=s=>{if(s.key==="Escape"){for(const a of t)if(a&&!a.classList.contains("d-none")){Nt(a);return}}};ho.set(e,n),document.addEventListener("keydown",n)}function Nt(e){var t,r,n;e.classList.add("d-none"),(t=document.querySelector("#train-modal"))!=null&&t.classList.contains("d-none")&&((r=document.querySelector("#train-delete-modal"))!=null&&r.classList.contains("d-none"))&&((n=document.querySelector("#train-timetable-preview-modal"))!=null&&n.classList.contains("d-none"))&&document.body.classList.remove("overflow-hidden")}function ve(e){return String(e).replaceAll("&","&amp;").replaceAll("<","&lt;").replaceAll(">","&gt;").replaceAll('"',"&quot;").replaceAll("'","&#039;")}function mo(e){const t=String(e||"").trim();if(!t)return"";const r=t.match(/^(\d{1,2}:\d{2})/);return r?r[1]:t.slice(0,5)}const we={rows:[],searchQuery:"",originFilter:"",destinationFilter:""};async function Pa(e){const{data:t,error:r}=await S.from("trains").select("id, number, origin_station, destination_station, departure_time, arrival_time, timetable_url").order("number",{ascending:!0});if(r){g(r.message,"error"),we.rows=[],pr(e,"Грешка при зареждане на влаковете.");return}we.rows=t||[],pr(e)}function pr(e,t){const r=e.querySelector("#trains-table-body"),n=e.querySelector("#trains-empty");Of(e);const s=we.rows.filter(a=>{const i=`${a.number||""} ${a.origin_station||""} ${a.destination_station||""}`.toLowerCase(),o=String(a.origin_station||"").trim().toLowerCase(),l=String(a.destination_station||"").trim().toLowerCase(),d=!we.searchQuery||i.includes(we.searchQuery),c=!we.originFilter||o===we.originFilter,u=!we.destinationFilter||l===we.destinationFilter;return d&&c&&u});if(!s.length){r.innerHTML="",n.classList.remove("d-none"),n.textContent=t||"Няма въведени влакове.";return}n.classList.add("d-none"),r.innerHTML=s.map(a=>{const i=Df(a.timetable_url),o=i.length?`<div class="d-flex flex-column gap-0">${i.map((l,d)=>{const c=l.label||`Файл ${d+1}`,u=encodeURIComponent(l.url),p=encodeURIComponent(c);return`
              <div class="d-flex align-items-center gap-2 flex-wrap">
...
`).map((r,n)=>Bn(r,n)).filter(r=>r.url)}function Bn(e,t){if(e&&typeof e=="object"&&!Array.isArray(e)){const n=String(e.url||"").trim(),s=String(e.label||"").trim()||wr(n,t);return{url:n,label:s}}const r=String(e||"").trim();return{url:r,label:wr(r,t)}}function Zn(e){const t=new Set;return(e||[]).map((r,n)=>Bn(r,n)).filter(r=>!r.url||t.has(r.url)?!1:(t.add(r.url),!0))}function dn(e){const t=Zn(e);return t.length?JSON.stringify(t):null}function wr(e,t){const r=String(e||"").trim();if(!r)return`Файл ${t+1}`;try{const s=new URL(r).pathname.split("/").pop()||"",a=decodeURIComponent(s);if(a)return a}catch{}return`Файл ${t+1}`}function hr(e){return String(e).replaceAll("&","&amp;").replaceAll("<","&lt;").replaceAll(">","&gt;").replaceAll('"',"&quot;").replaceAll("'","&#039;")}function fo(e,t,r){const n=e.querySelector("#train-timetable-preview-modal"),s=e.querySelector("#train-timetable-preview-frame"),a=e.querySelector("#train-timetable-preview-text-wrap"),i=e.querySelector("#train-timetable-preview-text"),o=e.querySelector("#train-timetable-preview-csv-wrap"),l=e.querySelector("#train-timetable-preview-csv-note"),d=e.querySelector("#train-timetable-preview-csv-head"),c=e.querySelector("#train-timetable-preview-csv-body"),u=e.querySelector("#train-timetable-preview-title"),p=e.querySelector("#train-timetable-preview-fallback"),m=e.querySelector("#train-timetable-preview-open");if(!n||!s||!a||!i||!o||!l||!d||!c||!u||!p||!m)return;const h=String(t||"").trim();if(!h){g("Липсва линк за преглед.","warning");return}const f=Wf(h),w=aa(h),y=w==="csv",v=["txt","csv","json"].includes(w);u.textContent=r?`Преглед: ${r}`:"Преглед на разписание",m.setAttribute("href",h),p.classList.add("d-none"),a.classList.add("d-none"),o.classList.add("d-none"),l.textContent="",d.innerHTML="",c.innerHTML="",i.textContent="",s.classList.remove("d-none"),s.src="about:blank",y?(o.classList.remove("d-none"),s.classList.add("d-none"),Bf(h,d,c,l,p)):v?(a.classList.remove("d-none"),s.classList.add("d-none"),i.textContent="Зареждане...",zf(h,i,p)):(s.src=f,s.onload=()=>{if(f!==h){p.classList.add("d-none");return}const b=aa(h),_=["doc","docx","xls","xlsx","ppt","pptx"].includes(b);p.classList.toggle("d-none",!_)},s.onerror=()=>{p.classList.remove("d-none")}),Fn(n)}function Ff(e){const t=e.querySelector("#train-timetable-preview-modal"),r=e.querySelector("#train-timetable-preview-frame"),n=e.querySelector("#train-timetable-preview-text-wrap"),s=e.querySelector("#train-timetable-preview-text"),a=e.querySelector("#train-timetable-preview-csv-wrap"),i=e.querySelector("#train-timetable-preview-csv-note"),o=e.querySelector("#train-timetable-preview-csv-head"),l=e.querySelector("#train-timetable-preview-csv-body"),d=e.querySelector("#train-timetable-preview-fallback"),c=e.querySelector("#train-timetable-preview-open");!t||!r||!n||!s||!a||!i||!o||!l||!d||!c||(r.src="about:blank",r.classList.remove("d-none"),n.classList.add("d-none"),a.classList.add("d-none"),s.textContent="",i.textContent="",o.innerHTML="",l.innerHTML="",c.setAttribute("href","#"),d.classList.add("d-none"),Nt(t))}async function Bf(e,t,r,n,s){try{const a=await fetch(e,{cache:"no-store"});if(!a.ok)throw new Error(`HTTP ${a.status}`);const i=await a.text(),o=Kf(i);if(!o.length){t.innerHTML="",r.innerHTML="",n.textContent="Файлът е празен.",s.classList.add("d-none");return}const l=200,d=o.slice(0,l),c=d[0]||[],u=d.slice(1);t.innerHTML=`
      <tr>${c.map(p=>`<th>${hr(p)}</th>`).join("")}</tr>
    `,r.innerHTML=u.map(p=>`<tr>${p.map(m=>`<td>${hr(m)}</td>`).join("")}</tr>`).join(""),o.length>l?n.textContent=`Показани са първите ${l} реда от общо ${o.length}.`:n.textContent=`Редове: ${o.length}.`,s.classList.add("d-none")}catch{t.innerHTML="",r.innerHTML="",n.textContent="",s.classList.remove("d-none")}}function Kf(e){const t=[];let r=[],n="",s=!1;for(let a=0;a<e.length;a+=1){const i=e[a],o=e[a+1];if(i==='"'){s&&o==='"'?(n+='"',a+=1):s=!s;continue}if(!s&&i===","){r.push(n),n="";continue}if(!s&&(i===`
`||i==="\r")){i==="\r"&&o===`
`&&(a+=1),r.push(n),t.push(r),r=[],n="";continue}n+=i}return(n.length||r.length)&&(r.push(n),t.push(r)),t}async function zf(e,t,r){try{const n=await fetch(e,{cache:"no-store"});if(!n.ok)throw new Error(`HTTP ${n.status}`);const s=await n.text();t.textContent=s||"(Празен файл)",r.classList.add("d-none")}catch{t.textContent="Неуспешно зареждане на текстов преглед.",r.classList.remove("d-none")}}function Wf(e){const t=aa(e);return["doc","docx","xls","xlsx","ppt","pptx"].includes(t)?`https://view.officeapps.live.com/op/embed.aspx?src=${encodeURIComponent(e)}`:e}function aa(e){const t=String(e||"").trim();if(!t)return"";try{const n=new URL(t).pathname.split("/").pop()||"",s=n.includes(".")?n.split(".").pop():"";return String(s||"").toLowerCase()}catch{return""}}const Vf=`<section class="card border-0 shadow-sm">\r
  <div class="card-body p-4 p-md-5">\r
    <div class="d-flex justify-content-between align-items-center mb-3">\r
...
`,U={profiles:[],employees:[],roleCatalog:[],availableRoles:[],roles:[],roleAuditLogs:[],currentUserId:"",currentUserProtectedAdminIds:[],permissionsRole:"admin",permissions:[]},Gf={admin:"Админ",crew_manager:"Ръководител екип",head_of_transport:"Ръководител транспорт",crew_instructor:"Инструктор екип",instructor:"Инструктор",crew:"Екип",crew_member:"Член екип",user:"Потребител"};function fe(e){return String(e??"").replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/\"/g,"&quot;").replace(/'/g,"&#39;")}function bl(e){const t=String((e==null?void 0:e.username)||"").trim(),r=String((e==null?void 0:e.id)||"").trim(),n=(e==null?void 0:e.is_active)===!1;if(t&&r)return`${t} (${r})${n?" (деактивиран)":""}`;const s=t||r||"-";return n&&s!=="-"?`${s} (деактивиран)`:s}function vl(e){const t=String((e==null?void 0:e.first_name)||"").trim(),r=String((e==null?void 0:e.last_name)||"").trim(),n=`${t} ${r}`.trim();return n?(e==null?void 0:e.is_active)===!1?`${n} (неактивен)`:n:"-"}function cn(e){const t=String(e||"").trim().toLowerCase();return t?Gf[t]||e:"-"}const gl=[{value:"none",label:"без достъп"},{value:"all",label:"всички"},{value:"own",label:"собствени"},{value:"role_attached_employees",label:"към прикачени служители по роля"}],Jf=[{value:"none",label:"Не"},{value:"all",label:"Да"}],Qf=[{value:"none",label:"Не"},{value:"all",label:"Да"}];function ia(e){const t=e.querySelector("#admin-role-profile-id"),r=e.querySelector("#admin-profile-link-id"),n=e.querySelector("#admin-profile-link-employee-id"),s=(t==null?void 0:t.value)||"",a=(r==null?void 0:r.value)||"",i=(n==null?void 0:n.value)||"",o=U.profiles.map(d=>{const c=bl(d);return`<option value="${d.id}">${fe(c)}</option>`}).join("");t&&(t.innerHTML=`<option value="">Избери профил</option>${o}`,t.value=U.profiles.some(d=>d.id===s)?s:""),r&&(r.innerHTML=`<option value="">Избери профил</option>${o}`,r.value=U.profiles.some(d=>d.id===a)?a:"");const l=U.employees.map(d=>{const c=vl(d);return`<option value="${d.id}">${fe(c)}</option>`}).join("");n&&(n.innerHTML=`<option value="">Избери служител</option>${l}`,n.value=U.employees.some(d=>d.id===i)?i:"")}function oa(e,t){const r=e.querySelector("#admin-roles-body"),n=e.querySelector("#admin-roles-empty");if(!r||!n)return;if(!U.roles.length){r.innerHTML="",n.classList.remove("d-none"),n.textContent=t||"Няма добавени роли.";return}n.classList.add("d-none");const s=U.roles.filter(a=>String((a==null?void 0:a.role)||"").trim().toLowerCase()==="admin").length;r.innerHTML=U.roles.map(a=>{const i=(a==null?void 0:a.username)||(a==null?void 0:a.user_id)||"-",o=a!=null&&a.role?Yf(a.role):"-",l=(a==null?void 0:a.granted_by_username)||(a==null?void 0:a.granted_by_user_id)||"-",d=(a==null?void 0:a.user_id)||"",c=!!(a!=null&&a.role),u=String((a==null?void 0:a.role)||"").trim().toLowerCase()==="admin",p=u&&s<=1,m=u&&d&&U.currentUserProtectedAdminIds.includes(d),h=!c||p||m,f=[c?`<span class="badge text-bg-secondary">${fe(o)}</span>`:'<span class="text-secondary">-</span>',m?'<span class="badge text-bg-info">Твой grantor lineage</span>':""].filter(Boolean).join(" "),w=p?"Не може да се премахне последният администратор.":m?"Не можеш да премахнеш админ права нагоре по grantor веригата.":"";return`
        <tr>
          <td>${fe(i)}</td>
          <td>${f}</td>
      ...
      `}).join("")}function la(e,t){const r=e.querySelector("#admin-role-catalog-body"),n=e.querySelector("#admin-role-catalog-empty");if(!(!r||!n)){if(!U.roleCatalog.length){r.innerHTML="",n.classList.remove("d-none"),n.textContent=t||"Няма налични роли.";return}n.classList.add("d-none"),r.innerHTML=U.roleCatalog.map(s=>{const a=String((s==null?void 0:s.name)||"").trim(),i=String((s==null?void 0:s.display_name_bg)||"").trim()||cn(a),o=a==="admin";return`
        <tr>
          <td>${fe(a)}</td>
          <td>${fe(i)}</td>
          <td class="text-end">
      ...
      `}).join("")}}function da(e,t){const r=e.querySelector("#admin-profiles-body"),n=e.querySelector("#admin-profiles-empty");if(!(!r||!n)){if(!U.profiles.length){r.innerHTML="",n.classList.remove("d-none"),n.textContent=t||"Няма налични профили.";return}n.classList.add("d-none"),r.innerHTML=U.profiles.map(s=>{const a=bl(s),i=(s==null?void 0:s.is_active)!==!1,o=i?'<span class="badge text-bg-success">Активен</span>':'<span class="badge text-bg-secondary">Деактивиран</span>',l=s!=null&&s.employees?vl(s.employees):"-",d=!!(s!=null&&s.employee_id),c=String((s==null?void 0:s.id)||"")===String(U.currentUserId||""),u=!i||c,p=i,m=c;return`
        <tr>
          <td>${fe(a)}</td>
          <td>${o}</td>
          <td>${fe(l)}</td>
      ...
      `}).join("")}}function ca(e,t){const r=e.querySelector("#admin-permissions-body"),n=e.querySelector("#admin-permissions-empty");if(!(!r||!n)){if(!U.permissions.length){r.innerHTML="",n.classList.remove("d-none"),n.textContent=t||"Няма данни за права.";return}n.classList.add("d-none"),r.innerHTML=U.permissions.map(s=>{const a=String((s==null?void 0:s.resource)||"-"),i=String((s==null?void 0:s.display_name_bg)||"").trim()||wa(a),o=Or(s==null?void 0:s.view_screen_scope),l=Or(s==null?void 0:s.view_records_scope),d=Or(s==null?void 0:s.create_records_scope),c=Or(s==null?void 0:s.edit_records_scope),u=Or(s==null?void 0:s.delete_records_scope);return`
      ...
      `}).join("")}}function ua(e,t){const r=e.querySelector("#admin-role-audit-body"),n=e.querySelector("#admin-role-audit-empty");if(!(!r||!n)){if(!U.roleAuditLogs.length){r.innerHTML="",n.classList.remove("d-none"),n.textContent=t||"Няма записани промени по роли.";return}n.classList.add("d-none"),r.innerHTML=U.roleAuditLogs.map(s=>{const a=String((s==null?void 0:s.action)||"").trim(),i=a==="grant"?"Добавяне":a==="revoke"?"Премахване":"Обновяване",o=(s==null?void 0:s.role_label)||"-",l=(s==null?void 0:s.actor_label)||"-",d=(s==null?void 0:s.target_label)||"-",c=Xf(s==null?void 0:s.occurred_at);return`
        <tr>
          <td>${fe(c)}</td>
          <td>${fe(i)}</td>
          <td>${fe(o)}</td>
      ...
      `}).join("")}}function Dr(e,t){const n=(e==="view_screen_scope"?Jf:e==="create_records_scope"?Qf:gl).map(s=>{const a=s.value===t?"selected":"";return`<option value="${s.value}" ${a}>${fe(s.label)}</option>`}).join("");return`<select class="form-select form-select-sm" data-permission-field="${e}">${n}</select>`}function Or(e){const t=String(e||"").trim();return gl.some(r=>r.value===t)?t:"none"}function Yf(e){const t=String(e||"").trim();if(!t)return"-";const r=U.roleCatalog.find(s=>(s==null?void 0:s.name)===t);return r&&String((r==null?void 0:r.display_name_bg)||"").trim()||cn(t)}function Xf(e){if(!e)return"-";const t=new Date(e);return Number.isNaN(t.getTime())?"-":t.toLocaleString("bg-BG")}const Zf=["admin","head_of_transport","instructor","crew","user"],yo={admin:10,crew_manager:20,head_of_transport:30,crew_instructor:40,instructor:50,crew:60,crew_member:70,user:80};let nr=null;async function ey(e){e.innerHTML=Vf;const t=e.querySelector("#admin-permissions-role");t&&(t.value=U.permissionsRole),es(e),hy(e),ty(e),await Na(e),await yn(e,U.permissionsRole)}function ty(e){const t=e.querySelector("#admin-assign-role-modal"),r=e.querySelector("#admin-assign-role-modal-close"),n=e.querySelector("#admin-assign-role-modal-cancel"),s=e.querySelector("#admin-profile-link-modal"),a=e.querySelector("#admin-profile-link-modal-close"),i=e.querySelector("#admin-profile-link-modal-cancel"),o=e.querySelector("#open-admin-role-modal"),l=e.querySelector("#admin-role-modal"),d=e.querySelector("#admin-role-modal-form"),c=e.querySelector("#admin-role-modal-close"),u=e.querySelector("#admin-role-modal-cancel"),p=e.querySelector("#admin-role-warning-modal"),m=e.querySelector("#admin-role-warning-close"),h=e.querySelector("#admin-role-warning-cancel"),f=e.querySelector("#admin-role-warning-confirm"),w=e.querySelector("#admin-role-form"),y=e.querySelector("#admin-profile-link-form"),v=e.querySelector("#admin-profile-link-clear"),b=e.querySelector("#admin-permissions-form"),_=e.querySelector("#admin-permissions-role"),k=e.querySelector("#admin-roles-body"),x=e.querySelector("#admin-role-catalog-body"),L=e.querySelector("#admin-profiles-body");r==null||r.addEventListener("click",()=>{Pe(t)}),n==null||n.addEventListener("click",()=>{Pe(t)}),a==null||a.addEventListener("click",()=>{Pe(s)}),i==null||i.addEventListener("click",()=>{Pe(s)}),o==null||o.addEventListener("click",()=>{vo(e,{mode:"create"})}),d==null||d.addEventListener("submit",async q=>{q.preventDefault(),await ay(e)}),c==null||c.addEventListener("click",()=>{Pe(l),es(e)}),u==null||u.addEventListener("click",()=>{Pe(l),es(e)}),m==null||m.addEventListener("click",()=>{Pe(p),nr=null}),h==null||h.addEventListener("click",()=>{Pe(p),nr=null}),f==null||f.addEventListener("click",async()=>{if(!nr){Pe(p);return}const q=nr;nr=null,Pe(p),await q()}),w==null||w.addEventListener("submit",async q=>{q.preventDefault(),await iy(e)}),y==null||y.addEventListener("submit",async q=>{q.preventDefault(),await ly(e)}),v==null||v.addEventListener("click",async()=>{var T;const q=((T=e.querySelector("#admin-profile-link-id"))==null?void 0:T.value)||"";if(!q){g("Избери профил за разкачане.","warning");return}mt(e,{message:"Сигурен ли си, че искаш да разкачиш профила от служителя?",confirmLabel:"Разкачи",onConfirm:()=>pa(e,q,null)})}),b==null||b.addEventListener("submit",async q=>{q.preventDefault(),await fy(e)}),_==null||_.addEventListener("change",async q=>{const T=q.target.value||"admin";U.permissionsRole=T,await yn(e,T)}),k==null||k.addEventListener("click",async q=>{const T=q.target.closest('button[data-admin-action="add-role"]');if(T){const A=T.getAttribute("data-user-id")||"";if(!A)return;sy(e,A);return}const E=q.target.closest('button[data-admin-action="remove-role"]');if(!E)return;const $=E.getAttribute("data-role-id")||"";$&&mt(e,{message:"Сигурен ли си, че искаш да разкачиш тази роля от потребителя?",confirmLabel:"Разкачи",onConfirm:()=>oy(e,$)})}),x==null||x.addEventListener("click",async q=>{const T=q.target.closest('button[data-admin-action="edit-catalog-role"]');if(T){const A=T.getAttribute("data-role-name")||"",C=T.getAttribute("data-role-bg")||"";vo(e,{mode:"edit",roleName:A,roleNameBg:C});return}const E=q.target.closest('button[data-admin-action="delete-catalog-role"]');if(!E)return;const $=E.getAttribute("data-role-name")||"";$&&await py(e,$)}),L==null||L.addEventListener("click",async q=>{const T=q.target.closest('button[data-admin-action="link-profile"]');if(T){const R=T.getAttribute("data-profile-id")||"";R&&ny(e,R);return}const E=q.target.closest('button[data-admin-action="unlink-profile"]');if(E){const R=E.getAttribute("data-profile-id")||"";if(!R)return;mt(e,{message:"Сигурен ли си, че искаш да разкачиш профила от служителя?",confirmLabel:"Разкачи",onConfirm:()=>pa(e,R,null)});return}const $=q.target.closest('button[data-admin-action="deactivate-profile"]');if($){const R=$.getAttribute("data-profile-id")||"";if(!R)return;if(R===U.currentUserId){g("Не можеш да деактивираш собствения си профил.","warning");return}mt(e,{message:"Сигурен ли си, че искаш да деактивираш този профил? Потребителят ще загуби достъп до системата.",confirmLabel:"Деактивирай",onConfirm:()=>bo(e,R,!1)});return}const A=q.target.closest('button[data-admin-action="restore-profile"]');if(A){const R=A.getAttribute("data-profile-id")||"";if(!R)return;mt(e,{message:"Сигурен ли си, че искаш да възстановиш този профил?",confirmLabel:"Възстанови",onConfirm:()=>bo(e,R,!0)});return}const C=q.target.closest('button[data-admin-action="hard-delete-user"]');if(!C)return;const I=C.getAttribute("data-profile-id")||"";if(I){if(I===U.currentUserId){g("Не можеш да изтриеш собствения си акаунт.","warning");return}mt(e,{message:"Сигурен ли си? Това е необратимо: ще бъдат изтрити Auth акаунтът, профилът и ролите.",confirmLabel:"Изтрий",onConfirm:()=>ry(e,I)})}})}async function ry(e,t){if(!t)return;const r=async()=>{var h,f;const{data:c,error:u}=await S.auth.refreshSession();if(!u&&((h=c==null?void 0:c.session)!=null&&h.access_token))return c.session.access_token;const{data:p,error:m}=await S.auth.getSession();return m||!((f=p==null?void 0:p.session)!=null&&f.access_token)?"":p.session.access_token};let n=await r();if(!n){g("Липсва активна сесия. Влез отново и опитай пак.","warning");return}const s="https://ujxczpaupfqaiqrcoykl.supabase.co",a="sb_publishable_EJ7wzzBh1hnKE0j_j7E1mQ_9TAJvRoO",i="admin-hard-delete-user-v2",o=async c=>fetch(`${s}/functions/v1/${i}`,{method:"POST",headers:{"Content-Type":"application/json",apikey:a,Authorization:`Bearer ${c}`},body:JSON.stringify({userId:t,reason:"admin_panel"})});let l;try{l=await o(n)}catch{g("Неуспешна връзка към Edge функцията.","error");return}let d=null;try{d=await l.json()}catch{d=null}if(l.status===401&&(n=await r(),n))try{l=await o(n),d=null;try{d=await l.json()}catch{d=null}}catch{g("Неуспешна връзка към Edge функцията.","error");return}if(!l.ok){const c=String((d==null?void 0:d.error)||(d==null?void 0:d.message)||l.statusText||"Изтриването не беше успешно.");if(l.status===401){g("Нямаш валидна сесия за Edge функцията. Опитай logout/login.","warning");return}if(String(c).toLowerCase().includes("last admin")){g("Не може да се изтрие последният администратор.","warning");return}g(c,"error");return}if(!(d!=null&&d.ok)){g("Изтриването не беше успешно.","error");return}g("Потребителят е изтрит.","success"),await Na(e)}function ny(e,t){const r=e.querySelector("#admin-profile-link-modal"),n=e.querySelector("#admin-profile-link-id"),s=e.querySelector("#admin-profile-link-employee-id"),a=U.profiles.find(i=>i.id===t);n&&(n.value=t),s&&(s.value=(a==null?void 0:a.employee_id)||""),us(r)}function sy(e,t){const r=e.querySelector("#admin-assign-role-modal"),n=e.querySelector("#admin-role-profile-id"),s=e.querySelector("#admin-role-value");n&&(n.value=t),s&&(s.value=""),us(r)}async function Na(e){var p;const{data:t}=await S.auth.getUser();U.currentUserId=((p=t==null?void 0:t.user)==null?void 0:p.id)||"";const[{data:r,error:n},{data:s,error:a},{data:i,error:o},{data:l,error:d},{data:c,error:u}]=await Promise.all([S.from("user_profiles").select("id, username, is_active, employee_id, employees(id, first_name, last_name, is_active)").order("username",{ascending:!0}),S.from("employees").select("id, first_name, last_name, is_active").order("last_name",{ascending:!0}).order("first_name",{ascending:!0}),S.from("user_roles").select("id, user_id, role, granted_by_user_id").order("role",{ascending:!0}).order("created_at",{ascending:!1}),S.from("roles").select("name, display_name_bg").order("name",{ascending:!0}),S.from("user_role_audit_logs").select("id, action, role, actor_user_id, target_user_id, occurred_at").order("occurred_at",{ascending:!1}).limit(100)]);if(n||a||o||d||u){g((n==null?void 0:n.message)||(a==null?void 0:a.message)||(o==null?void 0:o.message)||(d==null?void 0:d.message)||(u==null?void 0:u.message)||"Грешка при зареждане на админ панела.","error"),U.profiles=[],U.employees=[],U.roleCatalog=[],U.availableRoles=[],U.roles=[],U.roleAuditLogs=[],U.currentUserProtectedAdminIds=[],ia(e),la(e,"Няма налични роли."),oa(e,"Няма данни за роли."),ua(e,"Няма записани промени по роли."),da(e,"Няма данни за профили.");return}U.profiles=r||[],U.employees=s||[],U.roleCatalog=l||[],U.roles=cy(i||[],U.profiles),U.roleAuditLogs=wl(c||[],U.profiles),U.currentUserProtectedAdminIds=Sl(U.roles),U.availableRoles=kl(l||[],i||[]),xl(e),ia(e),la(e),oa(e),ua(e),da(e)}async function ay(e){var f,w;const t=e.querySelector("#admin-role-modal-original-name"),r=e.querySelector("#admin-role-modal-name"),n=e.querySelector("#admin-role-modal-name-bg"),s=e.querySelector("#admin-role-modal-save"),a=((f=t==null?void 0:t.value)==null?void 0:f.trim())||"",i=(r==null?void 0:r.value)||"",o=(n==null?void 0:n.value)||"",l=i.trim().toLowerCase(),d=o.trim();if(!l){g("Въведи име на роля.","warning");return}if(!d){g("Въведи име на ролята на български.","warning");return}if(!/^[a-z0-9_]+$/.test(l)){g("Ролята може да съдържа само малки латински букви, цифри и _.","warning");return}const c=(s==null?void 0:s.innerHTML)||"";s&&(s.disabled=!0,s.innerHTML='<span class="spinner-border spinner-border-sm me-2" role="status" aria-hidden="true"></span>Създаване...');const{data:u}=await S.auth.getUser();let p=null;if(a){const{error:y}=await S.from("roles").update({name:l,display_name_bg:d||l}).eq("name",a);p=y}else{const{error:y}=await S.from("roles").insert({name:l,display_name_bg:d||l,created_from:((w=u==null?void 0:u.user)==null?void 0:w.email)||"admin_panel"});p=y}if(p){s&&(s.disabled=!1,s.innerHTML=c),g(p.message,"error");return}const m=[...new Map(U.permissions.map(y=>[y.resource,y]).filter(([y])=>!!y)).values()];if(m.length){const y=m.map(b=>({role:l,resource:b.resource,display_name_bg:b.display_name_bg||wa(b.resource),view_screen_scope:"none",view_records_scope:"none",create_records_scope:"none",edit_records_scope:"none",delete_records_scope:"none"})),{error:v}=await S.from("role_permissions").upsert(y,{onConflict:"role,resource"});if(v){s&&(s.disabled=!1,s.innerHTML=c),g(v.message,"error");return}}r&&(r.value=""),n&&(n.value=""),t&&(t.value=""),s&&(s.disabled=!1,s.innerHTML=c),g(a?"Ролята е обновена.":"Ролята е създадена.","success"),await ql(e);const h=e.querySelector("#admin-permissions-role");h&&(h.value=l),U.permissionsRole=l,await yn(e,l),Pe(e.querySelector("#admin-role-modal")),es(e)}async function iy(e){var d,c,u,p;const t=((d=e.querySelector("#admin-role-profile-id"))==null?void 0:d.value)||"",r=((c=e.querySelector("#admin-role-value"))==null?void 0:c.value)||"",n=e.querySelector("#admin-role-add");if(!t||!r){g("Избери профил и роля.","warning");return}const s=(n==null?void 0:n.innerHTML)||"";n&&(n.disabled=!0,n.innerHTML='<span class="spinner-border spinner-border-sm me-2" role="status" aria-hidden="true"></span>Добавяне...');const{data:a}=await S.auth.getUser(),{error:i}=await S.from("user_roles").insert({user_id:t,role:r,granted_by_user_id:((u=a==null?void 0:a.user)==null?void 0:u.id)||null,created_from:((p=a==null?void 0:a.user)==null?void 0:p.email)||"admin_panel"});if(n&&(n.disabled=!1,n.innerHTML=s),i){g(i.message,"error");return}g("Ролята е добавена.","success"),Pe(e.querySelector("#admin-assign-role-modal"));const o=e.querySelector("#admin-role-profile-id"),l=e.querySelector("#admin-role-value");o&&(o.value=""),l&&(l.value=""),await Promise.all([Ha(e),_l(e)])}async function oy(e,t){const{error:r}=await S.from("user_roles").delete().eq("id",t);if(r){if(String(r.message||"").toLowerCase().includes("last admin")){g("Не може да се премахне последната админ роля.","warning");return}if(String(r.message||"").toLowerCase().includes("grantor")){g("Не можеш да отнемеш админ права нагоре по grantor веригата.","warning");return}g(r.message,"error");return}g("Ролята е премахната.","success"),await Promise.all([Ha(e),_l(e)])}async function ly(e){var n,s;const t=((n=e.querySelector("#admin-profile-link-id"))==null?void 0:n.value)||"",r=((s=e.querySelector("#admin-profile-link-employee-id"))==null?void 0:s.value)||"";if(!t||!r){g("Избери профил и служител.","warning");return}await pa(e,t,r)}async function pa(e,t,r){const n=e.querySelector("#admin-profile-link-save"),s=(n==null?void 0:n.innerHTML)||"";if(n&&(n.disabled=!0,n.innerHTML='<span class="spinner-border spinner-border-sm me-2" role="status" aria-hidden="true"></span>Запис...'),r){const{error:l}=await S.from("user_profiles").update({employee_id:null,updated_at:new Date().toISOString()}).eq("employee_id",r).neq("id",t);if(l){n&&(n.disabled=!1,n.innerHTML=s),g(l.message,"error");return}}const{error:a}=await S.from("user_profiles").update({employee_id:r,updated_at:new Date().toISOString()}).eq("id",t);if(n&&(n.disabled=!1,n.innerHTML=s),a){g(a.message,"error");return}g(r?"Профилът е свързан със служителя.":"Профилът е разкачен от служител.","success"),Pe(e.querySelector("#admin-profile-link-modal"));const i=e.querySelector("#admin-profile-link-id"),o=e.querySelector("#admin-profile-link-employee-id");i&&(i.value=""),o&&(o.value=""),await my(e)}async function bo(e,t,r){if(!t)return;const n=U.profiles.find(i=>String((i==null?void 0:i.id)||"")===String(t));if((n==null?void 0:n.is_active)!==!1===r){g(r?"Профилът вече е активен.":"Профилът вече е деактивиран.","warning");return}const{error:a}=await S.from("user_profiles").update({is_active:r,updated_at:new Date().toISOString()}).eq("id",t);if(a){if(String(a.message||"").toLowerCase().includes("last active admin")){g("Не може да деактивираш последния активен администратор.","warning");return}g(a.message,"error");return}g(r?"Профилът е възстановен.":"Профилът е деактивиран.","success"),await Na(e)}async function Ha(e){const{data:t,error:r}=await S.from("user_roles").select("id, user_id, role, granted_by_user_id").order("created_at",{ascending:!1});if(r){g(r.message,"error");return}U.roles=dy(t||[],U.profiles),U.currentUserProtectedAdminIds=Sl(U.roles),oa(e)}async function _l(e){const{data:t,error:r}=await S.from("user_role_audit_logs").select("id, action, role, actor_user_id, target_user_id, occurred_at").order("occurred_at",{ascending:!1}).limit(100);if(r){g(r.message,"error");return}U.roleAuditLogs=wl(t||[],U.profiles),ua(e)}function wl(e,t){const r=new Map((t||[]).map(n=>[n.id,n]));return(e||[]).map(n=>{const s=r.get(n.actor_user_id),a=r.get(n.target_user_id),i=String((n==null?void 0:n.role)||"").trim();return{...n,role_label:i?cn(i):"-",actor_label:(s==null?void 0:s.username)||(n==null?void 0:n.actor_user_id)||"-",target_label:(a==null?void 0:a.username)||(n==null?void 0:n.target_user_id)||"-"}})}function Sl(e){const t=String(U.currentUserId||"").trim();if(!t)return[];const r=(e||[]).filter(d=>String((d==null?void 0:d.role)||"").trim().toLowerCase()==="admin"),n=new Map(r.map(d=>[String((d==null?void 0:d.user_id)||"").trim(),String((d==null?void 0:d.granted_by_user_id)||"").trim()])),s=(e||[]).find(d=>String((d==null?void 0:d.user_id)||"").trim()===t&&String((d==null?void 0:d.role)||"").trim().toLowerCase()==="admin"),a=String((s==null?void 0:s.granted_by_user_id)||"").trim();if(!a)return[];const i=[],o=new Set([t]);let l=a;for(;l&&!o.has(l);){i.push(l),o.add(l);const d=String(n.get(l)||"").trim();if(!d||d===l)break;l=d}return i}function dy(e,t){const r=new Map((t||[]).map(n=>[n.id,n]));return(e||[]).map(n=>{var s,a;return{...n,username:((s=r.get(n.user_id))==null?void 0:s.username)||"",granted_by_username:((a=r.get(n.granted_by_user_id))==null?void 0:a.username)||""}})}function cy(e,t){const r=new Map((t||[]).map(a=>[a.id,a])),n=new Map;(e||[]).forEach(a=>{n.has(a.user_id)||n.set(a.user_id,[]),n.get(a.user_id).push(a)});const s=[];return(t||[]).forEach(a=>{const i=n.get(a.id)||[];i.length>0?i.forEach(o=>{var l;s.push({...o,username:a.username,user_id:a.id,granted_by_username:((l=r.get(o.granted_by_user_id))==null?void 0:l.username)||""})}):s.push({id:null,user_id:a.id,role:null,username:a.username,granted_by_user_id:null,granted_by_username:""})}),s}function kl(e,t){const r=(e||[]).map(a=>String((a==null?void 0:a.name)||"").trim()).filter(Boolean),n=(t||[]).map(a=>String((a==null?void 0:a.role)||"").trim()).filter(Boolean);return[...new Set([...Zf,...r,...n])].sort((a,i)=>{const o=String(a||"").trim().toLowerCase(),l=String(i||"").trim().toLowerCase(),d=yo[o]??999,c=yo[l]??999;return d!==c?d-c:o.localeCompare(l,"en")})}function xl(e){const t=e.querySelector("#admin-role-value"),r=e.querySelector("#admin-permissions-role"),n=(t==null?void 0:t.value)||"",s=(r==null?void 0:r.value)||U.permissionsRole||"",a=U.availableRoles.map(i=>{const o=Ll(i);return`<option value="${i}">${o}</option>`}).join("");if(t&&(t.innerHTML=`<option value="">Избери роля</option>${a}`,t.value=U.availableRoles.includes(n)?n:""),r){r.innerHTML=a;const i=U.availableRoles.includes("admin")?"admin":U.availableRoles[0]||"",o=U.availableRoles.includes(s)?s:i;r.value=o,U.permissionsRole=o}}async function ql(e){const{data:t,error:r}=await S.from("roles").select("name, display_name_bg").order("name",{ascending:!0});if(r){g(r.message,"error");return}U.roleCatalog=t||[],U.availableRoles=kl(t||[],U.roles),xl(e),la(e)}function Ll(e){const t=U.roleCatalog.find(n=>(n==null?void 0:n.name)===e),r=String((t==null?void 0:t.display_name_bg)||"").trim();return r||cn(e)}async function uy(e,t){if(!t)return;if(t==="admin"){g("Ролята admin не може да бъде изтрита.","warning");return}const{error:r}=await S.from("role_permissions").delete().eq("role",t);if(r){g(r.message,"error");return}const{error:n}=await S.from("roles").delete().eq("name",t);if(n){g(n.message,"error");return}if(g("Ролята е изтрита.","success"),await ql(e),await Ha(e),U.permissionsRole===t){const s=U.availableRoles.includes("admin")?"admin":U.availableRoles[0]||"";U.permissionsRole=s,s?await yn(e,s):(U.permissions=[],ca(e,"Няма данни за права."))}}function vo(e,{mode:t,roleName:r="",roleNameBg:n=""}){const s=e.querySelector("#admin-role-modal"),a=e.querySelector("#admin-role-modal-title"),i=e.querySelector("#admin-role-modal-original-name"),o=e.querySelector("#admin-role-modal-name"),l=e.querySelector("#admin-role-modal-name-bg"),d=e.querySelector("#admin-role-modal-save");i&&(i.value=t==="edit"?r:""),o&&(o.value=t==="edit"?r:""),l&&(l.value=t==="edit"?n||r:""),a&&(a.textContent=t==="edit"?"Редакция на роля":"Нова роля"),d&&(d.textContent=t==="edit"?"Запази":"Създай"),us(s)}function es(e){const t=e.querySelector("#admin-role-modal-original-name"),r=e.querySelector("#admin-role-modal-name"),n=e.querySelector("#admin-role-modal-name-bg"),s=e.querySelector("#admin-role-modal-title"),a=e.querySelector("#admin-role-modal-save");t&&(t.value=""),r&&(r.value=""),n&&(n.value=""),s&&(s.textContent="Нова роля"),a&&(a.textContent="Създай")}function mt(e,{message:t,confirmLabel:r,onConfirm:n}){const s=e.querySelector("#admin-role-warning-modal"),a=e.querySelector("#admin-role-warning-message"),i=e.querySelector("#admin-role-warning-confirm");nr=typeof n=="function"?n:null,a&&(a.textContent=t||"Сигурен ли си?"),i&&(i.textContent=r||"Потвърди"),us(s)}async function py(e,t){const{count:r,error:n}=await S.from("user_roles").select("id",{count:"exact",head:!0}).eq("role",t);if(n){g(n.message,"error");return}const s=Ll(t),a=Number(r||0);if(a>0){mt(e,{message:`Ролята "${s}" не може да бъде изтрита, защото е разкачена към ${a} потребител(и). Първо премахни свързаните роли от потребителите.`,confirmLabel:"Затвори",onConfirm:null});return}mt(e,{message:`Сигурен ли си, че искаш да премахнеш ролята "${s}"? Ще бъдат изтрити и всички права за тази роля.`,confirmLabel:"Премахни",onConfirm:()=>uy(e,t)})}function us(e){e&&e.classList.remove("d-none")}function Pe(e){e&&e.classList.add("d-none")}function hy(e){var a,i;const t=[...e.querySelectorAll("[data-admin-tab]")],r=[...e.querySelectorAll("[data-admin-tab-pane]")];if(!t.length||!r.length)return;const n=o=>{t.forEach(l=>{const d=l.getAttribute("data-admin-tab")===o;l.classList.toggle("active",d),l.setAttribute("aria-selected",d?"true":"false")}),r.forEach(l=>{const d=l.getAttribute("data-admin-tab-pane")===o;l.classList.toggle("active",d),l.classList.toggle("d-none",!d)})};t.forEach(o=>{o.addEventListener("click",()=>{const l=o.getAttribute("data-admin-tab")||"";l&&n(l)})});const s=((a=t.find(o=>o.classList.contains("active")))==null?void 0:a.getAttribute("data-admin-tab"))||((i=t[0])==null?void 0:i.getAttribute("data-admin-tab"))||"";s&&n(s)}async function my(e){const{data:t,error:r}=await S.from("user_profiles").select("id, username, is_active, employee_id, employees(id, first_name, last_name, is_active)").order("username",{ascending:!0});if(r){g(r.message,"error");return}U.profiles=t||[],ia(e),da(e)}async function yn(e,t){const r=t||"admin",{data:n,error:s}=await S.from("role_permissions").select("role, resource, display_name_bg, view_screen_scope, view_records_scope, create_records_scope, edit_records_scope, delete_records_scope").eq("role",r).order("resource",{ascending:!0});if(s){g(s.message,"error"),U.permissions=[],ca(e,"Няма данни за права.");return}U.permissions=n||[],ca(e)}async function fy(e){const t=e.querySelector("#admin-permissions-save"),r=e.querySelector("#admin-permissions-role"),n=(r==null?void 0:r.value)||"admin",s=[...e.querySelectorAll("#admin-permissions-body tr[data-resource]")];if(!s.length){g("Няма редове за запис.","warning");return}const a=s.map(l=>{const d=l.getAttribute("data-resource")||"",c=U.permissions.find(p=>p.resource===d),u=p=>{var h;const m=((h=l.querySelector(`[data-permission-field="${p}"]`))==null?void 0:h.value)||"none";return p==="view_screen_scope"||p==="create_records_scope"?["none","all"].includes(m)?m:"none":["none","all","own","role_attached_employees"].includes(m)?m:"none"};return{role:n,resource:d,display_name_bg:(c==null?void 0:c.display_name_bg)||wa(d),view_screen_scope:u("view_screen_scope"),view_records_scope:u("view_records_scope"),create_records_scope:u("create_records_scope"),edit_records_scope:u("edit_records_scope"),delete_records_scope:u("delete_records_scope"),updated_at:new Date().toISOString()}}),i=(t==null?void 0:t.innerHTML)||"";t&&(t.disabled=!0,t.innerHTML='<span class="spinner-border spinner-border-sm me-2" role="status" aria-hidden="true"></span>Запис...');const{error:o}=await S.from("role_permissions").upsert(a,{onConflict:"role,resource"});if(t&&(t.disabled=!1,t.innerHTML=i),o){g(o.message,"error");return}g("Правата са записани.","success"),await yn(e,n)}const yy=`<section class="card border-0 shadow-sm">\r
  <div class="card-body p-4 p-md-5">\r
    <div class="d-flex justify-content-between align-items-center mb-3">\r
...
`;function Et(e){e.classList.remove("d-none"),document.body.classList.add("overflow-hidden")}const go=new Map;function by(e,t){const r=go.get(e);r&&document.removeEventListener("keydown",r);const n=s=>{if(s.key==="Escape"){for(const a of t)if(a&&!a.classList.contains("d-none")){Fe(a);return}}};go.set(e,n),document.addEventListener("keydown",n)}function Fe(e){var t,r,n,s,a;e.classList.add("d-none"),(t=document.querySelector("#document-category-modal"))!=null&&t.classList.contains("d-none")&&((r=document.querySelector("#document-modal"))!=null&&r.classList.contains("d-none"))&&((n=document.querySelector("#document-category-delete-modal"))!=null&&n.classList.contains("d-none"))&&((s=document.querySelector("#document-delete-modal"))!=null&&s.classList.contains("d-none"))&&((a=document.querySelector("#document-preview-modal"))!=null&&a.classList.contains("d-none"))&&document.body.classList.remove("overflow-hidden")}function Ne(e){return String(e).replaceAll("&","&amp;").replaceAll("<","&lt;").replaceAll(">","&gt;").replaceAll('"',"&quot;").replaceAll("'","&#039;")}const Ae={categories:[],documents:[],searchQuery:"",categoryFilter:""};async function vy(e){const{data:t,error:r}=await S.from("document_categories").select("id, name").order("name",{ascending:!0});if(r){g(r.message,"error"),Ae.categories=[],_o(e,"Грешка при зареждане на категориите.");return}Ae.categories=t||[],_o(e),Ua(e)}async function gy(e){const{data:t,error:r}=await S.from("documents").select("id, title, document_url, storage_path, notes, category_id, document_categories(name)").order("title",{ascending:!0});if(r){g(r.message,"error"),Ae.documents=[],ts(e,"Грешка при зареждане на документите.");return}Ae.documents=t||[],ts(e)}function Ua(e){const t=e.querySelector("#documents-category-filter"),r=e.querySelector("#document-category"),n=(Ae.categories||[]).map(s=>`<option value="${s.id}">${Ne(s.name||"-")}</option>`).join("");t&&(t.innerHTML='<option value="">Всички</option>'+n,t.value=Ae.categoryFilter||""),r&&(r.innerHTML='<option value="">Избери категория</option>'+n)}function _o(e,t){const r=e.querySelector("#document-categories-table-body"),n=e.querySelector("#document-categories-empty"),s=Ae.categories||[];if(!s.length){r.innerHTML="",n.classList.remove("d-none"),n.textContent=t||"Няма въведени категории.";return}n.classList.add("d-none"),r.innerHTML=s.map(a=>`
      <tr>
        <td>${Ne(a.name||"-")}</td>
        <td class="text-end">
          <div class="d-inline-flex gap-2">
    ...
    `}).join("")}const un="documents-files";async function _y(e){e.innerHTML=yy,wy(e),await bn(e)}async function bn(e){await vy(e),await gy(e)}function wy(e){var h,f,w,y,v,b,_,k,x;const t=e.querySelector("#open-create-document-category"),r=e.querySelector("#open-create-document"),n=e.querySelector("#document-category-modal"),s=e.querySelector("#document-modal"),a=e.querySelector("#document-category-delete-modal"),i=e.querySelector("#document-delete-modal"),o=e.querySelector("#document-preview-modal"),l=e.querySelector("#document-category-form"),d=e.querySelector("#document-form"),c=e.querySelector("#document-categories-table-body"),u=e.querySelector("#documents-table-body"),p=e.querySelector("#documents-search"),m=e.querySelector("#documents-category-filter");t==null||t.addEventListener("click",()=>{Tl(e),Et(n)}),r==null||r.addEventListener("click",()=>{if(!(Ae.categories||[]).length){g("Добави първо категория.","warning");return}El(e),Ua(e),Et(s)}),l==null||l.addEventListener("submit",async L=>{L.preventDefault(),await Sy(e)}),d==null||d.addEventListener("submit",async L=>{L.preventDefault(),await ky(e)}),(h=e.querySelector("#document-category-modal-close"))==null||h.addEventListener("click",()=>Fe(n)),(f=e.querySelector("#document-category-cancel-btn"))==null||f.addEventListener("click",()=>Fe(n)),(w=e.querySelector("#document-modal-close"))==null||w.addEventListener("click",()=>Fe(s)),(y=e.querySelector("#document-cancel-btn"))==null||y.addEventListener("click",()=>Fe(s)),(v=e.querySelector("#document-category-delete-cancel"))==null||v.addEventListener("click",()=>Fe(a)),(b=e.querySelector("#document-delete-cancel"))==null||b.addEventListener("click",()=>Fe(i)),(_=e.querySelector("#document-preview-close"))==null||_.addEventListener("click",()=>Ay(e)),(k=e.querySelector("#document-category-delete-confirm"))==null||k.addEventListener("click",async()=>{const L=e.querySelector("#document-category-delete-id").value;await xy(e,L)}),(x=e.querySelector("#document-delete-confirm"))==null||x.addEventListener("click",async()=>{const L=e.querySelector("#document-delete-id").value;await qy(e,L)}),c==null||c.addEventListener("click",L=>{const q=L.target.closest("button[data-category-action]");if(!q)return;const T=q.getAttribute("data-category-action");if(T==="edit"){Ly(e,{id:q.getAttribute("data-id"),name:q.getAttribute("data-name")}),Et(n);return}T==="delete"&&(e.querySelector("#document-category-delete-id").value=q.getAttribute("data-id")||"",Et(a))}),u==null||u.addEventListener("click",L=>{const q=L.target.closest("button[data-document-action]");if(!q)return;const T=q.getAttribute("data-document-action");if(T==="edit"){Ty(e,{id:q.getAttribute("data-id"),title:q.getAttribute("data-title"),categoryId:q.getAttribute("data-category-id"),url:q.getAttribute("data-url"),storagePath:q.getAttribute("data-storage-path"),notes:q.getAttribute("data-notes")}),Et(s);return}if(T==="delete"){e.querySelector("#document-delete-id").value=q.getAttribute("data-id")||"",Et(i);return}if(T==="preview"){const E=String(q.getAttribute("data-url")||"").trim(),$=String(q.getAttribute("data-title")||"").trim();$y(e,E,$)}}),p==null||p.addEventListener("input",L=>{Ae.searchQuery=L.target.value.trim().toLowerCase(),ts(e)}),m==null||m.addEventListener("change",L=>{Ae.categoryFilter=L.target.value||"",ts(e)}),by("documents-page",[o,i,a,s,n])}async function Sy(e){var o,l;const t=e.querySelector("#document-category-id").value,r=e.querySelector("#document-category-name"),n=e.querySelector("#document-category-save-btn"),s=r.value.trim();if(!s){g("Попълни име на категория.","warning");return}const a=n.innerHTML;n.disabled=!0,n.innerHTML='<span class="spinner-border spinner-border-sm me-2" role="status" aria-hidden="true"></span>Запис...';let i=null;if(t)({error:i}=await S.from("document_categories").update({name:s,updated_at:new Date().toISOString()}).eq("id",t));else{const{data:d}=await S.auth.getUser(),c=((o=d==null?void 0:d.user)==null?void 0:o.id)??((l=d==null?void 0:d.user)==null?void 0:l.email)??"web_app";({error:i}=await S.from("document_categories").insert({name:s,created_from:c}))}if(n.disabled=!1,n.innerHTML=a,i){if(i.code==="23505"){g("Категория с това име вече съществува.","warning");return}g(i.message,"error");return}Fe(e.querySelector("#document-category-modal")),Tl(e),await bn(e),g(t?"Категорията е обновена.":"Категорията е създадена.","success")}async function ky(e){var h,f,w,y,v,b;const t=e.querySelector("#document-id").value,r=e.querySelector("#document-title").value.trim(),n=e.querySelector("#document-category").value||null,s=((h=e.querySelector("#document-current-file-link"))==null?void 0:h.getAttribute("href"))||"",a=((w=(f=e.querySelector("#document-current-file-link"))==null?void 0:f.dataset)==null?void 0:w.storagePath)||"",i=e.querySelector("#document-file"),o=((y=i==null?void 0:i.files)==null?void 0:y[0])||null,l=e.querySelector("#document-notes").value.trim()||null,d=e.querySelector("#document-save-btn");if(!r||!n){g("Попълни всички задължителни полета.","warning");return}if(!t&&!o){g("Качи файл за документа.","warning");return}const c=d.innerHTML;d.disabled=!0,d.innerHTML='<span class="spinner-border spinner-border-sm me-2" role="status" aria-hidden="true"></span>Запис...';let u=null,p=s,m=a;if(o){const _=await Ey(o);if(_.error){d.disabled=!1,d.innerHTML=c,g(_.error.message||"Файлът не може да се качи.","error");return}p=_.publicUrl,m=_.path}if(t)({error:u}=await S.from("documents").update({title:r,category_id:n,document_url:p,storage_path:m,notes:l,updated_at:new Date().toISOString()}).eq("id",t));else{const{data:_}=await S.auth.getUser(),k=((v=_==null?void 0:_.user)==null?void 0:v.id)??((b=_==null?void 0:_.user)==null?void 0:b.email)??"web_app";({error:u}=await S.from("documents").insert({title:r,category_id:n,document_url:p,storage_path:m,notes:l,created_from:k}))}if(d.disabled=!1,d.innerHTML=c,u){o&&m&&await S.storage.from(un).remove([m]),g(u.message,"error");return}t&&o&&a&&a!==m&&await S.storage.from(un).remove([a]),Fe(e.querySelector("#document-modal")),El(e),await bn(e),g(t?"Документът е обновен.":"Документът е създаден.","success")}async function xy(e,t){const r=e.querySelector("#document-category-delete-confirm"),n=r.innerHTML;r.disabled=!0,r.innerHTML='<span class="spinner-border spinner-border-sm me-2" role="status" aria-hidden="true"></span>Изтриване...';const{count:s,error:a}=await S.from("documents").select("id",{count:"exact",head:!0}).eq("category_id",t);if(a){r.disabled=!1,r.innerHTML=n,g(a.message,"error");return}if((s||0)>0){r.disabled=!1,r.innerHTML=n,g("Категорията не може да се изтрие, защото съдържа документи.","warning");return}const{error:i}=await S.from("document_categories").delete().eq("id",t);if(r.disabled=!1,r.innerHTML=n,i){g(i.message,"error");return}Fe(e.querySelector("#document-category-delete-modal")),await bn(e),g("Категорията е изтрита.","success")}async function qy(e,t){const r=e.querySelector("#document-delete-confirm"),n=r.innerHTML;r.disabled=!0,r.innerHTML='<span class="spinner-border spinner-border-sm me-2" role="status" aria-hidden="true"></span>Изтриване...';const{data:s}=await S.from("documents").select("storage_path").eq("id",t).maybeSingle(),{error:a}=await S.from("documents").delete().eq("id",t);if(r.disabled=!1,r.innerHTML=n,a){g(a.message,"error");return}s!=null&&s.storage_path&&await S.storage.from(un).remove([s.storage_path]),Fe(e.querySelector("#document-delete-modal")),await bn(e),g("Документът е изтрит.","success")}function Ly(e,t){e.querySelector("#document-category-id").value=t.id||"",e.querySelector("#document-category-name").value=t.name||"",e.querySelector("#document-category-form-title").textContent="Редакция на категория",e.querySelector("#document-category-save-btn").textContent="Запази"}function Tl(e){e.querySelector("#document-category-id").value="",e.querySelector("#document-category-name").value="",e.querySelector("#document-category-form-title").textContent="Нова категория",e.querySelector("#document-category-save-btn").textContent="Създай"}function Ty(e,t){Ua(e);const r=e.querySelector("#document-file"),n=e.querySelector("#document-file-help"),s=e.querySelector("#document-current-file-wrap"),a=e.querySelector("#document-current-file-link");e.querySelector("#document-id").value=t.id||"",e.querySelector("#document-title").value=t.title||"",e.querySelector("#document-category").value=t.categoryId||"",e.querySelector("#document-notes").value=t.notes||"",r&&(r.value="",r.required=!1),n&&(n.textContent="По избор: качи нов файл, за да замениш текущия."),s&&a&&t.url&&(s.classList.remove("d-none"),a.setAttribute("href",t.url),a.dataset.storagePath=t.storagePath||""),e.querySelector("#document-form-title").textContent="Редакция на документ",e.querySelector("#document-save-btn").textContent="Запази"}function El(e){const t=e.querySelector("#document-file"),r=e.querySelector("#document-file-help"),n=e.querySelector("#document-current-file-wrap"),s=e.querySelector("#document-current-file-link");e.querySelector("#document-id").value="",e.querySelector("#document-title").value="",e.querySelector("#document-category").value="",e.querySelector("#document-notes").value="",t&&(t.value="",t.required=!0),r&&(r.textContent="Качи файл за документа."),n&&n.classList.add("d-none"),s&&(s.setAttribute("href","#"),s.dataset.storagePath=""),e.querySelector("#document-form-title").textContent="Нов документ",e.querySelector("#document-save-btn").textContent="Създай"}async function Ey(e){var l,d;const{data:t}=await S.auth.getSession(),r=((d=(l=t==null?void 0:t.session)==null?void 0:l.user)==null?void 0:d.id)||"anonymous",n=Date.now(),s=String((e==null?void 0:e.name)||"document").replace(/[^a-zA-Z0-9._-]/g,"_"),a=`${r}/${n}_${s}`,{error:i}=await S.storage.from(un).upload(a,e,{upsert:!1});if(i)return{error:i};const{data:o}=S.storage.from(un).getPublicUrl(a);return{path:a,publicUrl:(o==null?void 0:o.publicUrl)||"",error:null}}function $y(e,t,r){const n=e.querySelector("#document-preview-modal"),s=e.querySelector("#document-preview-frame"),a=e.querySelector("#document-preview-text-wrap"),i=e.querySelector("#document-preview-text"),o=e.querySelector("#document-preview-csv-wrap"),l=e.querySelector("#document-preview-csv-note"),d=e.querySelector("#document-preview-csv-head"),c=e.querySelector("#document-preview-csv-body"),u=e.querySelector("#document-preview-title"),p=e.querySelector("#document-preview-fallback"),m=e.querySelector("#document-preview-open");if(!n||!s||!a||!i||!o||!l||!d||!c||!u||!p||!m)return;const h=String(t||"").trim();if(!h){g("Липсва файл за преглед.","warning");return}const f=Dy(h),w=ha(h),y=w==="csv",v=["txt","csv","json"].includes(w);u.textContent=r?`Преглед: ${r}`:"Преглед на документ",m.setAttribute("href",h),p.classList.add("d-none"),a.classList.add("d-none"),o.classList.add("d-none"),l.textContent="",d.innerHTML="",c.innerHTML="",i.textContent="",s.classList.remove("d-none"),s.src="about:blank",y?(o.classList.remove("d-none"),s.classList.add("d-none"),Cy(h,d,c,l,p)):v?(a.classList.remove("d-none"),s.classList.add("d-none"),i.textContent="Зареждане...",Iy(h,i,p)):(s.src=f,s.onload=()=>{if(f!==h){p.classList.add("d-none");return}const b=ha(h),_=["doc","docx","xls","xlsx","ppt","pptx"].includes(b);p.classList.toggle("d-none",!_)},s.onerror=()=>{p.classList.remove("d-none")}),Et(n)}function Ay(e){const t=e.querySelector("#document-preview-modal"),r=e.querySelector("#document-preview-frame"),n=e.querySelector("#document-preview-text-wrap"),s=e.querySelector("#document-preview-text"),a=e.querySelector("#document-preview-csv-wrap"),i=e.querySelector("#document-preview-csv-note"),o=e.querySelector("#document-preview-csv-head"),l=e.querySelector("#document-preview-csv-body"),d=e.querySelector("#document-preview-fallback"),c=e.querySelector("#document-preview-open");!t||!r||!n||!s||!a||!i||!o||!l||!d||!c||(r.src="about:blank",r.classList.remove("d-none"),n.classList.add("d-none"),a.classList.add("d-none"),s.textContent="",i.textContent="",o.innerHTML="",l.innerHTML="",c.setAttribute("href","#"),d.classList.add("d-none"),Fe(t))}async function Cy(e,t,r,n,s){try{const a=await fetch(e,{cache:"no-store"});if(!a.ok)throw new Error(`HTTP ${a.status}`);const i=await a.text(),o=Ry(i);if(!o.length){t.innerHTML="",r.innerHTML="",n.textContent="Файлът е празен.",s.classList.add("d-none");return}const l=200,d=o.slice(0,l),c=d[0]||[],u=d.slice(1);t.innerHTML=`
      <tr>${c.map(p=>`<th>${wo(p)}</th>`).join("")}</tr>
    `,r.innerHTML=u.map(p=>`<tr>${p.map(m=>`<td>${wo(m)}</td>`).join("")}</tr>`).join(""),o.length>l?n.textContent=`Показани са първите ${l} реда от общо ${o.length}.`:n.textContent=`Редове: ${o.length}.`,s.classList.add("d-none")}catch{t.innerHTML="",r.innerHTML="",n.textContent="",s.classList.remove("d-none")}}function Ry(e){const t=[];let r=[],n="",s=!1;for(let a=0;a<e.length;a+=1){const i=e[a],o=e[a+1];if(i==='"'){s&&o==='"'?(n+='"',a+=1):s=!s;continue}if(!s&&i===","){r.push(n),n="";continue}if(!s&&(i===`
`||i==="\r")){i==="\r"&&o===`
`&&(a+=1),r.push(n),t.push(r),r=[],n="";continue}n+=i}return(n.length||r.length)&&(r.push(n),t.push(r)),t}async function Iy(e,t,r){try{const n=await fetch(e,{cache:"no-store"});if(!n.ok)throw new Error(`HTTP ${n.status}`);const s=await n.text();t.textContent=s||"(Празен файл)",r.classList.add("d-none")}catch{t.textContent="Неуспешно зареждане на текстов преглед.",r.classList.remove("d-none")}}function Dy(e){const t=ha(e);return["doc","docx","xls","xlsx","ppt","pptx"].includes(t)?`https://view.officeapps.live.com/op/embed.aspx?src=${encodeURIComponent(e)}`:e}function ha(e){const t=String(e||"").trim();if(!t)return"";try{const n=new URL(t).pathname.split("/").pop()||"",s=n.includes(".")?n.split(".").pop():"";return String(s||"").toLowerCase()}catch{return""}}function wo(e){return String(e).replaceAll("&","&amp;").replaceAll("<","&lt;").replaceAll(">","&gt;").replaceAll('"',"&quot;").replaceAll("'","&#039;")}const Oy=`<section class="card border-0 shadow-sm">\r
  <div class="card-body p-4 p-md-5">\r
    <div class="d-flex flex-wrap justify-content-between align-items-end gap-3 mb-3">\r
      <div>\r
...
`;function Fa(e){e==null||e.classList.remove("d-none"),document.body.classList.add("overflow-hidden")}const So=new Map;function Py(e,t){const r=So.get(e);r&&document.removeEventListener("keydown",r);const n=s=>{if(s.key==="Escape"){for(const a of t)if(a&&!a.classList.contains("d-none")){_t(a);return}}};So.set(e,n),document.addEventListener("keydown",n)}function _t(e){var t,r,n;e==null||e.classList.add("d-none"),(t=document.querySelector("#user-profile-view-modal"))!=null&&t.classList.contains("d-none")&&((r=document.querySelector("#user-profile-edit-modal"))!=null&&r.classList.contains("d-none"))&&((n=document.querySelector("#user-profile-reset-confirm-modal"))!=null&&n.classList.contains("d-none"))&&document.body.classList.remove("overflow-hidden")}function Ur(e){return String(e).replaceAll("&","&amp;").replaceAll("<","&lt;").replaceAll(">","&gt;").replaceAll('"',"&quot;").replaceAll("'","&#039;")}function ma(e){var s,a;const t=String(((s=e==null?void 0:e.employees)==null?void 0:s.first_name)||"").trim(),r=String(((a=e==null?void 0:e.employees)==null?void 0:a.last_name)||"").trim();return`${t} ${r}`.trim()||"-"}const oe={rows:[],employees:[],searchQuery:"",isAdmin:!1,currentUserId:"",resetConfirmResolver:null};function fa(e,t){const r=e.querySelector("#user-profiles-table-body"),n=e.querySelector("#user-profiles-empty"),s=(oe.rows||[]).filter(a=>{const i=[a==null?void 0:a.username,a==null?void 0:a.email,a==null?void 0:a.first_name,a==null?void 0:a.last_name,ma(a)].map(o=>String(o||"").toLowerCase()).join(" ");return!oe.searchQuery||i.includes(oe.searchQuery)});if(!s.length){r.innerHTML="",n.classList.remove("d-none"),n.textContent=t||"Няма намерени профили.";return}n.classList.add("d-none"),r.innerHTML=s.map(a=>{const i=`${String((a==null?void 0:a.first_name)||"").trim()} ${String((a==null?void 0:a.last_name)||"").trim()}`.trim()||"-",o=ma(a);return`
        <tr>
      ...
      `}).join("")}function My(e){const t=e.querySelector("#user-profile-edit-employee-id");if(!t)return;const r=(oe.employees||[]).map(n=>{const s=`${String((n==null?void 0:n.first_name)||"").trim()} ${String((n==null?void 0:n.last_name)||"").trim()}`.trim()||"-";return`<option value="${n.id}">${Ur(s)}</option>`}).join("");t.innerHTML='<option value="">Без свързан служител</option>'+r}async function jy(e){e.innerHTML=Oy,Ny(e),Fy(e),await ya(e)}function Ny(e){e.querySelectorAll("button[data-toggle-password]").forEach(t=>{t.addEventListener("click",()=>{const r=t.getAttribute("data-toggle-password")||"",n=e.querySelector(`#${r}`);if(!n)return;const s=n.type==="password";n.type=s?"text":"password",t.textContent=s?"🙈":"👁";const a=r.includes("confirm"),i=a?"Покажи потвърждението на новата парола":"Покажи новата парола",o=a?"Скрий потвърждението на новата парола":"Скрий новата парола";t.setAttribute("aria-label",s?o:i)})})}function Hy(){return`${window.location.origin}/reset-password`}function Pn(e,t){const r=oe.resetConfirmResolver;r&&(oe.resetConfirmResolver=null,_t(e.querySelector("#user-profile-reset-confirm-modal")),r(!!t))}function Uy(e,t){const r=e.querySelector("#user-profile-reset-confirm-modal"),n=e.querySelector("#user-profile-reset-confirm-message");if(!r||!n)return Promise.resolve(!1);if(oe.resetConfirmResolver){const s=oe.resetConfirmResolver;oe.resetConfirmResolver=null,s(!1)}return n.textContent=`Да се изпрати ли линк за смяна на парола до ${t}?`,Fa(r),new Promise(s=>{oe.resetConfirmResolver=s})}function Fy(e){var i,o,l,d,c,u,p,m;const t=e.querySelector("#user-profiles-search"),r=e.querySelector("#user-profiles-table-body"),n=e.querySelector("#user-profile-view-modal"),s=e.querySelector("#user-profile-edit-modal"),a=e.querySelector("#user-profile-reset-confirm-modal");t==null||t.addEventListener("input",h=>{var f;oe.searchQuery=String(((f=h.target)==null?void 0:f.value)||"").trim().toLowerCase(),fa(e)}),r==null||r.addEventListener("click",h=>{const f=h.target.closest("button[data-user-profile-action]");if(!f)return;const w=f.getAttribute("data-user-profile-action"),y=f.getAttribute("data-id")||"";if(w==="view"){Ky(e,y);return}if(w==="edit"){zy(e,y);return}w==="reset-password"&&By(e,y,f)}),(i=e.querySelector("#user-profile-view-close"))==null||i.addEventListener("click",()=>_t(n)),(o=e.querySelector("#user-profile-view-close-secondary"))==null||o.addEventListener("click",()=>_t(n)),(l=e.querySelector("#user-profile-edit-close"))==null||l.addEventListener("click",()=>_t(s)),(d=e.querySelector("#user-profile-edit-cancel"))==null||d.addEventListener("click",()=>_t(s)),(c=e.querySelector("#user-profile-reset-confirm-close"))==null||c.addEventListener("click",()=>Pn(e,!1)),(u=e.querySelector("#user-profile-reset-confirm-cancel"))==null||u.addEventListener("click",()=>Pn(e,!1)),(p=e.querySelector("#user-profile-reset-confirm-accept"))==null||p.addEventListener("click",()=>Pn(e,!0)),a==null||a.addEventListener("click",h=>{h.target===a&&Pn(e,!1)}),(m=e.querySelector("#user-profile-edit-form"))==null||m.addEventListener("submit",async h=>{h.preventDefault(),await Wy(e)}),Py("user-profiles",[a,s,n])}async function By(e,t,r){if(!oe.isAdmin){g("Нямаш права за това действие.","warning");return}const n=oe.rows.find(c=>c.id===t);if(!n){g("Профилът не е намерен.","warning");return}const s=String(n.email||"").trim();if(!s){g("Потребителят няма валиден имейл.","warning");return}const i=`${String(n.first_name||"").trim()} ${String(n.last_name||"").trim()}`.trim()||n.username||s;if(!await Uy(e,i))return;const l=(r==null?void 0:r.innerHTML)||"Reset парола";r&&(r.disabled=!0,r.innerHTML="Изпращане...");const{error:d}=await S.auth.resetPasswordForEmail(s,{redirectTo:Hy()});if(r&&(r.disabled=!1,r.innerHTML=l),d){g(d.message||"Линкът за смяна на парола не беше изпратен.","error");return}g("Изпратен е линк за смяна на парола към потребителя.","success")}async function ya(e){var d,c;const t=await fr(),r=((d=t==null?void 0:t.user)==null?void 0:d.id)||"";oe.currentUserId=r,oe.isAdmin=await fu();const n=e.querySelector("#user-profiles-search");n&&(n.classList.toggle("d-none",!oe.isAdmin),(c=n.previousElementSibling)==null||c.classList.toggle("d-none",!oe.isAdmin));const s=S.from("user_profiles").select("id, username, email, first_name, last_name, employee_id, employees(id, first_name, last_name)").order("username",{ascending:!0});!oe.isAdmin&&r&&s.eq("id",r);const[{data:a,error:i},{data:o,error:l}]=await Promise.all([s,oe.isAdmin?S.from("employees").select("id, first_name, last_name").order("last_name",{ascending:!0}).order("first_name",{ascending:!0}):Promise.resolve({data:[],error:null})]);if(i||l){g((i==null?void 0:i.message)||(l==null?void 0:l.message)||"Грешка при зареждане на профили.","error"),oe.rows=[],oe.employees=[],fa(e,"Няма данни за профили.");return}oe.rows=a||[],oe.employees=o||[],My(e),fa(e)}function Ky(e,t){const r=oe.rows.find(n=>n.id===t);if(!r){g("Профилът не е намерен.","warning");return}e.querySelector("#user-profile-view-username").textContent=r.username||"-",e.querySelector("#user-profile-view-email").textContent=r.email||"-",e.querySelector("#user-profile-view-first-name").textContent=r.first_name||"-",e.querySelector("#user-profile-view-last-name").textContent=r.last_name||"-",e.querySelector("#user-profile-view-employee").textContent=ma(r),Fa(e.querySelector("#user-profile-view-modal"))}function zy(e,t){const r=oe.rows.find(l=>l.id===t);if(!r){g("Профилът не е намерен.","warning");return}const n=e.querySelector("#user-profile-edit-employee-wrap"),s=e.querySelector("#user-profile-password-wrap"),a=e.querySelector("#user-profile-edit-new-password"),i=e.querySelector("#user-profile-edit-confirm-password");n&&n.classList.toggle("d-none",!oe.isAdmin);const o=r.id===oe.currentUserId;s&&s.classList.toggle("d-none",!o),a&&(a.value=""),i&&(i.value=""),e.querySelector("#user-profile-edit-id").value=r.id,e.querySelector("#user-profile-edit-username").value=r.username||"",e.querySelector("#user-profile-edit-email").value=r.email||"",e.querySelector("#user-profile-edit-first-name").value=r.first_name||"",e.querySelector("#user-profile-edit-last-name").value=r.last_name||"",e.querySelector("#user-profile-edit-employee-id").value=r.employee_id||"",Fa(e.querySelector("#user-profile-edit-modal"))}async function Wy(e){var f,w,y,v,b,_,k,x;const t=(((f=e.querySelector("#user-profile-edit-id"))==null?void 0:f.value)||"").trim(),r=(((w=e.querySelector("#user-profile-edit-username"))==null?void 0:w.value)||"").trim(),n=(((y=e.querySelector("#user-profile-edit-email"))==null?void 0:y.value)||"").trim(),s=(((v=e.querySelector("#user-profile-edit-first-name"))==null?void 0:v.value)||"").trim(),a=(((b=e.querySelector("#user-profile-edit-last-name"))==null?void 0:b.value)||"").trim(),i=((_=e.querySelector("#user-profile-edit-new-password"))==null?void 0:_.value)||"",o=((k=e.querySelector("#user-profile-edit-confirm-password"))==null?void 0:k.value)||"",l=((x=e.querySelector("#user-profile-edit-employee-id"))==null?void 0:x.value)||null,d=e.querySelector("#user-profile-edit-save"),c=t===oe.currentUserId;if(!t||!r||!n||!s||!a){g("Попълни всички задължителни полета.","warning");return}if(!/^[A-Za-z0-9_]{3,30}$/.test(r)){g("Потребителското име трябва да е 3-30 символа и да съдържа само латински букви, цифри и _.","warning");return}if((i||o)&&!c){g("Можеш да сменяш паролата само на собствения си профил.","warning");return}if(c&&(i||o)){if(i.length<6){g("Новата парола трябва да е поне 6 символа.","warning");return}const L=/[\u0400-\u04FF]/;if(L.test(i)||L.test(o)){g("Паролата не трябва да съдържа кирилица.","warning");return}if(i!==o){g("Новата парола и потвърждението не съвпадат.","warning");return}}const p=(d==null?void 0:d.innerHTML)||"";d&&(d.disabled=!0,d.innerHTML='<span class="spinner-border spinner-border-sm me-2" role="status" aria-hidden="true"></span>Запис...');const m={username:r,email:n,first_name:s,last_name:a,updated_at:new Date().toISOString()};oe.isAdmin&&(m.employee_id=l||null);const{error:h}=await S.from("user_profiles").update(m).eq("id",t);if(d&&(d.disabled=!1,d.innerHTML=p),h){if(h.code==="23505"){g("Потребителско име или имейл вече съществува.","warning");return}g(h.message,"error");return}if(c&&i){const{error:L}=await S.auth.updateUser({password:i});if(L){g(L.message||"Профилът е обновен, но паролата не беше сменена.","warning"),await ya(e),_t(e.querySelector("#user-profile-edit-modal"));return}}_t(e.querySelector("#user-profile-edit-modal")),await ya(e),g(c&&i?"Профилът и паролата са обновени.":"Профилът е обновен.","success")}const ko={"/":{render:Zu,title:"TrainCrewHub"},"/login":{render:rp,title:"TrainCrewHub / Sign In"},"/register":{render:ip,title:"TrainCrewHub / Register"},"/forgot-password":{render:pp,title:"TrainCrewHub / Forgot Password"},"/reset-password":{render:bp,title:"TrainCrewHub / Reset Password"},"/pending-access":{render:_p,title:"TrainCrewHub / Чака одобрение"},"/schedule-keys":{render:kp,title:"TrainCrewHub / Ключ-График",resource:"schedule_keys"},"/duties":{render:jp,title:"TrainCrewHub / Повески",resource:"duties"},"/duty-types":{render:tf,title:"TrainCrewHub / Типове повески",resource:"duty_types"},"/trains":{render:Pf,title:"TrainCrewHub / Влакове",resource:"trains"},"/employees":{render:lh,title:"TrainCrewHub / Служители",resource:"employees"},"/employee-absences":{render:bh,title:"TrainCrewHub / Отсъствия",resource:"employee_absences"},"/planned-duties":{render:Hh,title:"TrainCrewHub / Планирани повески",resource:"planned_duties"},"/actual-duties":{render:Qh,title:"TrainCrewHub / Реални повески",resource:"actual_duties"},"/plan-schedule":{render:Cm,title:"TrainCrewHub / План График",screenResource:"page_plan_schedule",resource:"planned_duties"},"/schedule":{render:Qm,title:"TrainCrewHub / График",screenResource:"page_schedule",resource:"actual_duties"},"/schedule-key-duties":{render:ff,title:"TrainCrewHub / Повески към Ключ-График",resource:"duties"},"/documents":{render:_y,title:"TrainCrewHub / Документи",resource:"documents"},"/user-profiles":{render:jy,title:"TrainCrewHub / Потребителски профили"},"/admin":{render:ey,title:"TrainCrewHub / Админ Панел",requiresAdmin:!0}},Vy="page-content";function xo(e){return ko[e]??ko["/"]}async function Gy(e,t){var a,i,o,l;const r=new Set(["/login","/register","/forgot-password","/reset-password"]);let n=null;if(!r.has(e)){const d=await _i();if(!d.allowed)return d.reason==="inactive-profile"&&g("Профилът е деактивиран. Свържи се с администратор.","warning"),"/login";if(n=await fr(),!((a=n==null?void 0:n.user)!=null&&a.id))return"/login";if(!await Ks(n.user.id))return e==="/pending-access"?e:(g("Акаунтът ти чака одобрение от администратор.","warning"),"/pending-access")}if(e==="/pending-access"){if(!((i=n==null?void 0:n.user)!=null&&i.id)){const c=await _i();if(!c.allowed)return c.reason==="inactive-profile"&&g("Профилът е деактивиран. Свържи се с администратор.","warning"),"/login";n=await fr()}return(o=n==null?void 0:n.user)!=null&&o.id?await Ks(n.user.id)?"/":e:"/login"}if(!(t!=null&&t.requiresAdmin)){const d=(t==null?void 0:t.screenResource)||(t==null?void 0:t.resource)||"";return!d||await Bo(d)?e:(g("Нямаш права за достъп до този екран.","warning"),"/")}return n=n||await fr(),(l=n==null?void 0:n.user)!=null&&l.id?await _a(n.user.id)?e:(g("Нямаш права за достъп до админ панела.","warning"),"/"):"/login"}async function ba(){const e=document.getElementById(Vy);gu();const t=window.location.pathname,r=xo(t),n=await Gy(t,r);n!==t&&window.history.replaceState({},"",n);const s=xo(n);document.title=s.title,await s.render(e),s.resource&&await vu(e,s.resource),window.dispatchEvent(new CustomEvent("route:changed",{detail:{pathname:n}}))}function Jy(e){const t=e.target.closest("a[data-link]");if(!t)return;e.preventDefault();const r=t.getAttribute("href");r!==window.location.pathname&&(window.history.pushState({},"",r),ba())}function Qy(){Ko(),window.addEventListener("popstate",ba),document.addEventListener("click",Jy),ba()}function $l(){return document.getElementById("app")}function Yy(e){const t=$l();t&&(t.innerHTML=`
	...
	`)}async function Xy(){const e=$l();if(!e)throw new Error("App root element (#app) not found.");await ku(e),Qy()}function qo(){Xy().catch(e=>{const t=e instanceof Error?e.message:String(e);console.error("App bootstrap failed:",e),Yy(t)})}document.readyState==="loading"?document.addEventListener("DOMContentLoaded",qo,{once:!0}):qo();
