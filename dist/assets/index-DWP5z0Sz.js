...
`))throw new Error(`@supabase/auth-js: Invalid SIWE message field "statement". Statement must not include '\\n'. Provided value: ${e.statement}`)}const m=Po(e.address),h=c?`${c}://${n}`:n,f=e.statement?`${e.statement}
...
`;function Nt(e){if(!e)return"00:00";const t=String(e).match(/(\d{1,2}):(\d{2})(?::\d{2})?/);if(!t)return"00:00";const[,r,n]=t;return`${r.padStart(2,"0")}:${n}`}function yi(e){if(!e)return 0;const[t,r]=e.split(":"),n=Number(t),s=Number(r);return!Number.isFinite(n)||!Number.isFinite(s)?0:n*60+s}function Ie(e,t){const r=yi(e),n=yi(t);return n>=r?n-r:24*60-r+n}async function qu(e,t){const{setText:r,formatDateTime:n,escapeHtml:s}=t,[a,i,o,l]=await Promise.all([S.from("user_profiles").select("id",{count:"exact",head:!0}),S.from("user_roles").select("user_id"),S.from("roles").select("name",{count:"exact",head:!0}),S.from("user_profiles").select("id",{count:"exact",head:!0}).not("employee_id","is",null)]);[a,i,o,l].some(u=>u.error)&&v("Част от админ данните на индекс страницата не могат да се заредят.","warning");const c=new Set((i.data||[]).map(u=>u.user_id).filter(Boolean));r(e,"#index-kpi-planned",String(a.count??0)),r(e,"#index-kpi-actual",String(c.size)),r(e,"#index-kpi-absences",String(l.count??0)),r(e,"#index-kpi-employees",String(o.count??0)),await Lu(e,{setText:r,formatDateTime:n,escapeHtml:s}),r(e,"#index-last-updated",`Последно обновяване: ${n(new Date)}`)}async function Lu(e,t){const{setText:r,formatDateTime:n,escapeHtml:s}=t,a=e.querySelector("#index-pending-users-panel"),i=e.querySelector("#index-pending-users-body");if(!a||!i)return;const[{data:o,error:l},{data:d,error:c}]=await Promise.all([S.from("user_profiles").select("id, username, created_at").order("created_at",{ascending:!0}),S.from("user_roles").select("user_id")]);if(l||c){v("Списъкът с чакащи потребители не може да се зареди.","warning"),r(e,"#index-pending-users-count","0"),i.innerHTML='<tr><td colspan="3" class="text-secondary">Грешка при зареждане.</td></tr>';return}const u=new Set((d||[]).map(m=>String((m==null?void 0:m.user_id)||"").trim()).filter(Boolean)),p=(o||[]).filter(m=>!u.has(String((m==null?void 0:m.id)||"").trim()));if(r(e,"#index-pending-users-count",String(p.length)),!p.length){i.innerHTML='<tr><td colspan="3" class="text-secondary">Няма чакащи потребители.</td></tr>';return}i.innerHTML=p.map(m=>{const h=String((m==null?void 0:m.username)||"").trim()||String((m==null?void 0:m.id)||"-"),f=(m==null?void 0:m.created_at)||"",_=f?new Date(f):null,y=_&&!Number.isNaN(_.getTime())?n(_):"-",g=Tu(f);return`
      ...
      `}).join("")}function Tu(e){if(!e)return"-";const t=new Date(e);if(Number.isNaN(t.getTime()))return"-";const r=Math.max(Date.now()-t.getTime(),0),n=Math.floor(r/6e4),s=Math.floor(n/(24*60)),a=Math.floor(n%(24*60)/60),i=n%60;return s>0?`${s} д ${a} ч`:a>0?`${a} ч ${i} мин`:i>0?`${i} мин`:"под 1 мин"}function Eu(e){const{loadKpiSnapshot:t,getTodayIsoDate:r,formatDate:n,escapeHtml:s,formatMinutesAsClock:a,formatSignedMinutesAsClock:i,getDeviationClassByThreshold:o,parseIsoDateSafe:l,toMonthKey:d,getMonthBounds:c,countBulgarianWorkdays:u,getDutyTimingSummary:p,getActualDutyTimingSummary:m,getDistinctBadgeClassByReason:h}=e,f={groups:[],selectedKey:""};function _($,C,D,O){const U=$.querySelector(C);if(U){if(!D.length){U.innerHTML=`<tr><td colspan="3" class="text-secondary">${O}</td></tr>`;return}U.innerHTML=D.map(K=>`
        <tr>
          <td>${s(K.employeeName)}</td>
          <td>${s(K.certificateLabel)}</td>
          <td>${s(n(K.date))}</td>
        </tr>
      `).join("")}}function y($){const C=new Date;C.setHours(0,0,0,0);const D=new Date(C);D.setDate(D.getDate()+30);const O=[{key:"psychological_assessment_expiry",label:"Психологическа годност"},{key:"medical_certificate_expiry",label:"Медицинско"},{key:"license_expiry",label:"Лиценз"}],U=[],K=[];($||[]).forEach(j=>{const z=`${(j==null?void 0:j.first_name)||""} ${(j==null?void 0:j.last_name)||""}`.trim()||"-";O.forEach(ee=>{const le=j==null?void 0:j[ee.key];if(!le)return;const P=new Date(`${le}T00:00:00`);if(Number.isNaN(P.getTime()))return;const R={employeeName:z,certificateLabel:ee.label,date:le};if(P<C){K.push(R);return}P<=D&&U.push(R)})});const W=(j,z)=>{const ee=String(j.date).localeCompare(String(z.date));return ee!==0?ee:String(j.employeeName).localeCompare(String(z.employeeName),"bg")};return{soon:U.sort(W),expired:K.sort(W)}}function g($,C){return C?$==="expired"?"text-bg-danger":"text-bg-warning":"text-bg-secondary"}function b($,C){const D=$.querySelector("#index-certificates-soon-details"),O=$.querySelector("#index-certificates-expired-details");if(!(!D||!O)){if(C==="soon"){D.classList.toggle("d-none"),O.classList.add("d-none");return}C==="expired"&&(O.classList.toggle("d-none"),D.classList.add("d-none"))}}async function w($){const C=$.querySelector("#index-certificates-panel"),D=$.querySelector("#index-certificates-soon-toggle"),O=$.querySelector("#index-certificates-expired-toggle");if(!C||!D||!O)return;const{data:U,error:K}=await S.from("employees").select("first_name, last_name, psychological_assessment_expiry, medical_certificate_expiry, license_expiry").eq("is_active",!0).order("last_name",{ascending:!0}).order("first_name",{ascending:!0});if(K){v("Сертификатите не могат да се заредят.","warning"),D.textContent="0",O.textContent="0",D.className="badge text-bg-secondary border-0",O.className="badge text-bg-secondary border-0",_($,"#index-certificates-soon-body",[],"Няма служители."),_($,"#index-certificates-expired-body",[],"Няма служители.");return}const W=y(U||[]);D.textContent=String(W.soon.length),O.textContent=String(W.expired.length),D.className=`badge ${g("soon",W.soon.length)} border-0`,O.className=`badge ${g("expired",W.expired.length)} border-0`,_($,"#index-certificates-soon-body",W.soon,"Няма служители с изтичащи сертификати."),_($,"#index-certificates-expired-body",W.expired,"Няма служители с изтекли сертификати.")}function k($){const C=new Map;($||[]).forEach(O=>{var ee,le,P;const U=String(((ee=O==null?void 0:O.absence_reasons)==null?void 0:ee.name)||"").trim()||"Без посочена причина",K=`${((le=O==null?void 0:O.employees)==null?void 0:le.first_name)||""} ${((P=O==null?void 0:O.employees)==null?void 0:P.last_name)||""}`.trim()||"-",W=(O==null?void 0:O.start_date)||"",j=(O==null?void 0:O.end_date)||"",z=C.get(U)||{reason:U,details:[]};z.details.push({employeeName:K,startDate:W,endDate:j}),C.set(U,z)});const D=Array.from(C.values()).map(O=>({...O,details:O.details.sort((U,K)=>String(U.employeeName).localeCompare(String(K.employeeName),"bg")),count:O.details.length}));return D.sort((O,U)=>U.count!==O.count?U.count-O.count:String(O.reason).localeCompare(String(U.reason),"bg")),D}function T($,C){const D=$.querySelector("#index-absence-reasons-body");if(D){if(!C.length){D.innerHTML='<p class="text-secondary mb-0">Няма активни отсъствия.</p>';return}D.innerHTML=C.map((O,U)=>{const K=h(O.reason),W=String(U),j=W===f.selectedKey,z=O.details.map(ee=>{const le=`${n(ee.startDate)} - ${n(ee.endDate)}`;return`
              <tr>
          ...
          `:""}
        `}).join("")}}function q($,C){if(C===""||C===f.selectedKey){f.selectedKey="",T($,f.groups);return}if(!(f.groups[Number(C)]||null)){f.selectedKey="",T($,f.groups);return}f.selectedKey=String(C),T($,f.groups)}async function E($){const C=r(),{data:D,error:O}=await S.from("employee_absences").select("start_date, end_date, employees(first_name, last_name), absence_reasons(name)").lte("start_date",C).gte("end_date",C).order("start_date",{ascending:!0});if(O){v("Отсъствията не могат да се заредят.","warning"),f.groups=[],f.selectedKey="",T($,[]);return}f.groups=k(D||[]),f.selectedKey="",T($,f.groups)}function x($,C){const D=$.querySelector("#index-workload-body");if(D){if(!C.length){D.innerHTML='<tr><td colspan="5" class="text-secondary">Няма данни.</td></tr>';return}D.innerHTML=C.map(O=>`
        <tr>
...
`).map((N,B)=>x(N,B)).filter(N=>N.url)}function x(P,R){if(P&&typeof P=="object"&&!Array.isArray(P)){const B=String(P.url||"").trim(),Q=String(P.label||"").trim()||`Разписание ${R+1}`;return{url:B,label:Q}}return{url:String(P||"").trim(),label:`Разписание ${R+1}`}}function L(P){const R=String(P||"").trim();if(!R)return"ОТС";const N=R.toLowerCase(),B={бо:"БО",болничен:"БО","болничен отпуск":"БО",до:"ДО","допълнителен отпуск":"ДО",по:"ПО","платен отпуск":"ПО",нп:"НП","неплатен отпуск":"НП",командировка:"К"};if(B[N])return B[N];if(R.length<=4&&/^[\p{L}\p{N}]+$/u.test(R))return R.toUpperCase();const Q=R.split(/\s+/).map(H=>H.trim()).filter(Boolean);return Q.length>=2?Q.slice(0,3).map(H=>H.charAt(0).toUpperCase()).join(""):R.slice(0,2).toUpperCase()}function A(P){const R=L(P);return Iu[R]||b(P)}function $(P,R){const N=new Date(`${P}T00:00:00`),B=new Date(`${R}T00:00:00`);if(Number.isNaN(N.getTime())||Number.isNaN(B.getTime())||N>B)return[];const Q=[],H=new Date(N);for(;H<=B;)Q.push(a(H)),H.setDate(H.getDate()+1);return Q}function C(P){var R;return String(((R=P==null?void 0:P.absence_reasons)==null?void 0:R.name)||"").trim()||"Отсъствие"}function D(P){const R=String(P||"").trim();if(!R)return"";try{const B=new URL(R).pathname.split("/").pop()||"",Q=B.includes(".")?B.split(".").pop():"";return String(Q||"").toLowerCase()}catch{return""}}function O(P){const R=D(P);return["doc","docx","xls","xlsx","ppt","pptx"].includes(R)?`https://view.officeapps.live.com/op/embed.aspx?src=${encodeURIComponent(P)}`:P}function U(P,R,N){const B=P.querySelector("#index-timetable-preview-modal"),Q=P.querySelector("#index-timetable-preview-frame"),H=P.querySelector("#index-timetable-preview-title"),J=P.querySelector("#index-timetable-preview-fallback"),te=P.querySelector("#index-timetable-preview-open");if(!B||!Q||!H||!J||!te)return;const ce=String(R||"").trim();if(!ce){v("Липсва линк за преглед.","warning");return}const ae=O(ce);H.textContent=N?`Преглед: ${N}`:"Преглед на разписание",te.setAttribute("href",ce),J.classList.add("d-none"),Q.src="about:blank",Q.src=ae,Q.onload=()=>{if(ae!==ce){J.classList.add("d-none");return}const re=D(ce),Le=["doc","docx","xls","xlsx","ppt","pptx"].includes(re);J.classList.toggle("d-none",!Le)},Q.onerror=()=>{J.classList.remove("d-none")},B.classList.remove("d-none")}function K(P){const R=P.querySelector("#index-timetable-preview-modal"),N=P.querySelector("#index-timetable-preview-frame"),B=P.querySelector("#index-timetable-preview-fallback"),Q=P.querySelector("#index-timetable-preview-open");!R||!N||!B||!Q||(N.src="about:blank",Q.setAttribute("href","#"),B.classList.add("d-none"),R.classList.add("d-none"))}function W(P){const R=i(P),N=s(R),B=n(),Q=B.startsWith(N),H=new Set([...t.plannedRows.map(re=>w(re==null?void 0:re.date)).filter(Boolean),...t.actualRows.map(re=>w(re==null?void 0:re.date)).filter(Boolean),...t.absenceRows.flatMap(re=>$(re==null?void 0:re.start_date,re==null?void 0:re.end_date)),...Array.from(t.pendingConfirmationDates||[]).map(re=>w(re)).filter(Boolean),...Array.from((t.changeCountByDate||new Map).keys()).map(re=>w(re)).filter(Boolean)]),J=w(t.selectedDate),te=J?H.has(J):!1,ce=H.size>0;if(J&&J.startsWith(N)&&(!ce||te))return;const ae=[...H].sort((re,Le)=>String(re).localeCompare(String(Le),"bg"));if(Q&&H.has(B)){t.selectedDate=B;return}if(ae.length){t.selectedDate=String(ae[0]);return}t.selectedDate=`${N}-01`}function j(){const P=new Map;return t.plannedRows.forEach(R=>{const N=w(R==null?void 0:R.date);if(!N)return;const B=P.get(N)||{planned:0,actual:0};B.planned+=1,P.set(N,B)}),t.actualRows.forEach(R=>{const N=w(R==null?void 0:R.date);if(!N)return;const B=P.get(N)||{planned:0,actual:0};B.actual+=1,P.set(N,B)}),t.pendingConfirmationDates.forEach(R=>{const N=w(R);if(!N)return;const B=P.get(N)||{planned:0,actual:0,absences:[]};B.pendingConfirmation=!0,P.set(N,B)}),t.changeCountByDate.forEach((R,N)=>{const B=w(N);if(!B)return;const Q=P.get(B)||{planned:0,actual:0,absences:[]};Q.changeCount=Number(R||0),P.set(B,Q)}),t.absenceRows.forEach(R=>{const N=C(R),B=A(N);$(R==null?void 0:R.start_date,R==null?void 0:R.end_date).forEach(Q=>{const H=P.get(Q)||{planned:0,actual:0,absences:[]};Array.isArray(H.absences)||(H.absences=[]),H.absences.some(te=>(te==null?void 0:te.reason)===N)||H.absences.push({reason:N,className:B}),P.set(Q,H)})}),P}function z(P){const R=P.querySelector("#index-crew-calendar-days");if(!R)return;const N=t.visibleMonth||s(new Date),B=i(N),Q=new Date(B.getFullYear(),B.getMonth(),1),H=Q.getDay(),J=H===0?6:H-1,te=new Date(Q);te.setDate(Q.getDate()-J);const ce=j(),ae=n();r(P,"#index-crew-month-label",o(N));const re=[];for(let Le=0;Le<42;Le+=1){const I=new Date(te);I.setDate(te.getDate()+Le);const ue=`${s(I)}-${String(I.getDate()).padStart(2,"0")}`,Je=I.getMonth()!==B.getMonth(),Te=ue===t.selectedDate,Ft=ue===ae,Ce=ce.get(ue)||{planned:0,actual:0,absences:[]},wt=Array.isArray(Ce.absences)?Ce.absences.map(Ye=>`<span class="badge ${u(Ye.className||"text-bg-danger")}" title="${u(Ye.reason||"Отсъствие")}">${u(Ye.reason||"Отсъствие")}</span>`).join(""):"";re.push(`
      ...
      `)}R.innerHTML=re.join("")}function ee(P){const R=w(t.selectedDate),N=P.querySelector("#index-crew-planned-body"),B=P.querySelector("#index-crew-actual-body"),Q=P.querySelector("#index-crew-change-body"),H=P.querySelector("#index-crew-absence-body");if(!N||!B||!Q||!H)return;r(P,"#index-crew-selected-date-label",`Детайли за ${d(R)}`);const J=t.plannedRows.filter(I=>w(I==null?void 0:I.date)===R).sort((I,ne)=>{var ue,Je;return String(((ue=I==null?void 0:I.duties)==null?void 0:ue.start_time)||"").localeCompare(String(((Je=ne==null?void 0:ne.duties)==null?void 0:Je.start_time)||""),"bg")});J.length?N.innerHTML=J.map(I=>{var Ft,Ce,wt,Ye;const ne=((Ft=I==null?void 0:I.duties)==null?void 0:Ft.name)||"-",ue=g(I==null?void 0:I.assignment_role),Je=`${((Ce=I==null?void 0:I.duties)==null?void 0:Ce.start_time)||"-"} - ${((wt=I==null?void 0:I.duties)==null?void 0:wt.end_time)||"-"}${(Ye=I==null?void 0:I.duties)!=null&&Ye.second_day?" (+1)":""}`,Te=m(I==null?void 0:I.duties);return`
        ...
        `).join(""):Q.innerHTML='<p class="text-secondary mb-0">Няма регистрирани промени за избрания ден.</p>';const Le=t.absenceRows.filter(I=>{const ne=String((I==null?void 0:I.start_date)||""),ue=String((I==null?void 0:I.end_date)||"");return!!(ne&&ue&&R>=ne&&R<=ue)}).sort((I,ne)=>{const ue=String((I==null?void 0:I.start_date)||"").localeCompare(String((ne==null?void 0:ne.start_date)||""),"bg");return ue!==0?ue:C(I).localeCompare(C(ne),"bg")});if(!Le.length){H.innerHTML='<p class="text-secondary mb-0">Няма отсъствия за избрания ден.</p>';return}H.innerHTML=Le.map(I=>{const ne=C(I),ue=A(ne),Je=`${d(I==null?void 0:I.start_date)} - ${d(I==null?void 0:I.end_date)}`;return`
          <article class="border rounded p-2">
...
`).map(r=>r.trim()).filter(Boolean).length}function Pp(e){const t=e.querySelector("#duties-type-filter");if(!t)return;const r=Z.dutyTypeFilter||"",n=[...new Set(Z.allDuties.map(s=>{var a;return String(((a=s==null?void 0:s.duty_types)==null?void 0:a.name)||"").trim()}).filter(Boolean))].sort((s,a)=>s.localeCompare(a,"bg"));t.innerHTML=`
    <option value="">Всички</option>
    ${n.map(s=>`<option value="${pe(s.toLowerCase())}">${pe(s)}</option>`).join("")}
  `,t.value=r}function Op(e){const t=e.querySelector("#duties-schedule-key-filter");if(!t)return;const r=Z.scheduleKeyFilter||"",n=[...new Set(Z.allDuties.flatMap(s=>Vs(s)).map(s=>String(s||"").trim()).filter(Boolean))].sort((s,a)=>s.localeCompare(a,"bg"));t.innerHTML=`
    <option value="">Всички</option>
    ${n.map(s=>`<option value="${pe(s.toLowerCase())}">${pe(s)}</option>`).join("")}
  ...
  `)}async function Np(e){const t=e.querySelector("#duty-trains"),{data:r,error:n}=await S.from("trains").select("id, number, origin_station, destination_station").order("number",{ascending:!0});if(n){v(Zp(n),"error");return}const s=(r||[]).map(a=>{const i=`${a.origin_station||"-"} - ${a.destination_station||"-"}`;return`<option value="${a.id}">${pe(a.number||"-")} (${pe(i)})</option>`}).join("");t.innerHTML=s}async function Hp(e){const t=e.querySelector("#duty-type"),{data:r,error:n}=await S.from("duty_types").select("id, name").order("name",{ascending:!0});if(n){v(n.message,"error");return}const s=(r||[]).map(a=>`<option value="${a.id}">${pe(a.name)}</option>`).join("");t.innerHTML='<option value="">Избери тип</option>'+s}function Up(e){var E,x;const t=e.querySelector("#open-create-duty"),r=e.querySelector("#duty-form"),n=e.querySelector("#duty-cancel-btn"),s=e.querySelector("#duties-table-body"),a=e.querySelector("#duty-modal"),i=e.querySelector("#duty-delete-modal"),o=e.querySelector("#duty-profile-modal"),l=e.querySelector("#duty-attachment-preview-modal"),d=e.querySelector("#duty-modal-close"),c=e.querySelector("#duty-delete-confirm"),u=e.querySelector("#duty-delete-cancel"),p=e.querySelector("#duty-profile-close"),m=e.querySelector("#duty-profile-close-secondary"),h=e.querySelector("#duty-profile-duplicate"),f=e.querySelector("#duty-profile-edit"),_=e.querySelector("#duties-search"),y=e.querySelector("#duties-schedule-key-filter"),g=e.querySelector("#duties-type-filter"),b=e.querySelector("#duties-filter-reset"),w=e.querySelector("#duties-prev-page"),k=e.querySelector("#duties-next-page"),T=e.querySelector("#duty-attachment-file"),q=e.querySelector("#duty-current-attachments-links");t==null||t.addEventListener("click",()=>{_a(e),Ht(a)}),r==null||r.addEventListener("submit",async L=>{L.preventDefault(),await Bp(e)}),n==null||n.addEventListener("click",()=>{Ve(a)}),d==null||d.addEventListener("click",()=>{Ve(a)}),u==null||u.addEventListener("click",()=>{Ve(i)}),p==null||p.addEventListener("click",()=>{Ve(o)}),m==null||m.addEventListener("click",()=>{Ve(o)}),(E=e.querySelector("#duty-profile-content"))==null||E.addEventListener("click",L=>{const A=L.target.closest('button[data-duty-profile-action="preview-attachment"]');if(!A)return;const $=String(A.getAttribute("data-url")||"").trim(),C=String(A.getAttribute("data-label")||"").trim();Ai(e,$,C)}),(x=e.querySelector("#duty-attachment-preview-close"))==null||x.addEventListener("click",()=>{Gp(e)}),f==null||f.addEventListener("click",()=>{var A;const L=((A=o==null?void 0:o.dataset)==null?void 0:A.dutyId)||"";L&&(Ve(o),Ci(e,L))}),h==null||h.addEventListener("click",()=>{var A;const L=((A=o==null?void 0:o.dataset)==null?void 0:A.dutyId)||"";L&&(Ve(o),Ri(e,L))}),_==null||_.addEventListener("input",L=>{Z.searchQuery=L.target.value.trim().toLowerCase(),Z.currentPage=1,tt(e)}),g==null||g.addEventListener("change",L=>{Z.dutyTypeFilter=L.target.value||"",Z.currentPage=1,tt(e)}),y==null||y.addEventListener("change",L=>{Z.scheduleKeyFilter=L.target.value||"",Z.currentPage=1,tt(e)}),b==null||b.addEventListener("click",()=>{Z.searchQuery="",Z.scheduleKeyFilter="",Z.dutyTypeFilter="",Z.currentPage=1,_&&(_.value=""),y&&(y.value=""),g&&(g.value=""),tt(e)}),w==null||w.addEventListener("click",()=>{Z.currentPage-=1,tt(e)}),k==null||k.addEventListener("click",()=>{Z.currentPage+=1,tt(e)}),T==null||T.addEventListener("change",()=>{var L;(L=T.files)!=null&&L.length&&T.files.length>Xr&&(v(`Може да избереш до ${Xr} файла наведнъж.`,"warning"),T.value="")}),q==null||q.addEventListener("input",L=>{const A=L.target.closest(".duty-existing-attachment-label");if(!A)return;const $=Number(A.getAttribute("data-index"));if(!Number.isInteger($)||$<0)return;const C=e.querySelector("#duty-draft-attachments"),D=Ut((C==null?void 0:C.value)||"");D[$]&&(D[$].label=A.value,C&&(C.value=Zr(D)||""))}),q==null||q.addEventListener("click",L=>{const A=L.target.closest(".duty-existing-attachment-preview");if(A){const U=String(A.getAttribute("data-url")||"").trim(),K=String(A.getAttribute("data-label")||"").trim();Ai(e,U,K);return}const $=L.target.closest(".duty-existing-attachment-remove");if(!$)return;const C=Number($.getAttribute("data-index"));if(!Number.isInteger(C)||C<0)return;const D=e.querySelector("#duty-draft-attachments"),O=Ut((D==null?void 0:D.value)||"");O[C]&&(O.splice(C,1),wa(e,O))}),$p("duties",[l,o,i,a]),c==null||c.addEventListener("click",async()=>{const L=e.querySelector("#duty-delete-id").value;await Kp(L,e)}),s==null||s.addEventListener("click",async L=>{const A=L.target.closest("button[data-action]");if(!A)return;const $=A.getAttribute("data-action");if($==="profile"){const C=A.getAttribute("data-id");eh(e,C);return}if($==="edit"){const C=A.getAttribute("data-id");Ci(e,C);return}if($==="duplicate"){const C=A.getAttribute("data-id");Ri(e,C);return}if($==="delete"){const C=A.getAttribute("data-id");e.querySelector("#duty-delete-id").value=C,Ht(i)}}),s==null||s.addEventListener("dragstart",L=>{const A=L.target.closest("tr[data-duty-id]");A&&(Z.draggedDutyId=A.getAttribute("data-duty-id"),A.classList.add("table-active"))}),s==null||s.addEventListener("dragend",L=>{const A=L.target.closest("tr[data-duty-id]");A&&A.classList.remove("table-active"),Z.draggedDutyId=null}),s==null||s.addEventListener("dragover",L=>{L.preventDefault()}),s==null||s.addEventListener("drop",async L=>{L.preventDefault();const A=L.target.closest("tr[data-duty-id]"),$=Z.draggedDutyId;if(!A||!$)return;const C=A.getAttribute("data-duty-id");if(!C||C===$)return;const D=Z.allDuties.findIndex(W=>W.id===$),O=Z.allDuties.findIndex(W=>W.id===C);if(D<0||O<0)return;const[U]=Z.allDuties.splice(D,1);if(Z.allDuties.splice(O,0,U),tt(e),!await Ap()){await ss(e);return}v("Редът на повеските е запазен.","success")})}async function Fp(e){const t=e.querySelector("#duty-schedule-keys"),{data:r,error:n}=await S.from("schedule_keys").select("id, name").order("name",{ascending:!0});if(n){v(n.message,"error");return}const s=(r||[]).map(a=>`<option value="${a.id}">${pe(a.name)}</option>`).join("");t.innerHTML=s}async function Bp(e){var B,Q;const t=e.querySelector("#duty-id"),r=e.querySelector("#duty-name"),n=e.querySelector("#duty-type"),s=e.querySelector("#duty-schedule-keys"),a=e.querySelector("#duty-trains"),i=Or(e,"#duty-start","#duty-start-time"),o=Or(e,"#duty-end","#duty-end-time"),l=e.querySelector("#duty-second-day"),d=Or(e,"#duty-break-start","#duty-break-start-time"),c=Or(e,"#duty-break-end","#duty-break-end-time"),u=e.querySelector("#duty-notes"),p=e.querySelector("#duty-attachment-file"),m=e.querySelector("#duty-existing-attachments"),h=e.querySelector("#duty-draft-attachments"),f=e.querySelector("#duty-save-btn"),_=r.value.trim(),y=n.value||null,g=Array.from(s.selectedOptions||[]).map(H=>H.value).filter(Boolean),b=Array.from(a.selectedOptions||[]).map(H=>H.value).filter(Boolean),w=g[0]||null,k=(i==null?void 0:i.value)||"",T=(o==null?void 0:o.value)||"",q=l.checked,E=(d==null?void 0:d.value)||"00:00",x=(c==null?void 0:c.value)||"00:00",L=u.value.trim()||null,A=Ut((m==null?void 0:m.value)||""),$=Ut((h==null?void 0:h.value)||""),C=Array.from((p==null?void 0:p.files)||[]),D=t.value;if(!_||!y||!k||!T){v("Моля, попълни всички задължителни полета.","warning");return}if(!g.length){v("Избери поне един ключ-график.","warning");return}const O=Ie(k,T);if(Ie(E,x)>O){v("Прекъсването не може да е по-голямо от продължителността на повеската.","warning");return}const K=f.innerHTML;f.disabled=!0,f.innerHTML='<span class="spinner-border spinner-border-sm me-2" role="status" aria-hidden="true"></span>Запис...';const W=D||crypto.randomUUID(),j=Kn($),z=[];if(j.length+C.length>Xr){f.disabled=!1,f.innerHTML=K,v(`Максимум ${Xr} файла/линка за една повеска.`,"warning");return}if(C.length){const H=await Vp(C,W);if(!H){f.disabled=!1,f.innerHTML=K;return}H.forEach(J=>{J!=null&&J.url&&j.push({url:J.url,label:J.label||en(J.url,j.length)}),J!=null&&J.objectPath&&z.push(J.objectPath)})}const le=Kn(j),P={name:_,duty_type_id:y,schedule_key_id:w,start_time:k,end_time:T,second_day:q,break_start_time:E,break_end_time:x,notes:L,duty_files:Zr(le)};let R,N=D||null;if(D)({error:R}=await S.from("duties").update(P).eq("id",D));else{const{data:H}=await S.auth.getUser(),J=((B=H==null?void 0:H.user)==null?void 0:B.id)??((Q=H==null?void 0:H.user)==null?void 0:Q.email)??"web_app",te=Z.allDuties.reduce((re,Le)=>Math.max(re,Number(Le.display_order)||0),0),{data:ce,error:ae}=await S.from("duties").insert({...P,id:W,created_from:J,display_order:te+1}).select("id").single();R=ae,N=(ce==null?void 0:ce.id)??null}if(!R&&N&&(R=await zp(N,g)),!R&&N&&(R=await Wp(N,b)),f.disabled=!1,f.innerHTML=K,R){z.length&&await tn(z),v(R.message,"error");return}if(D){const H=A.map(ae=>Gs(ae.url)).filter(Boolean),J=le.map(ae=>Gs(ae.url)).filter(Boolean),te=new Set(J),ce=H.filter(ae=>!te.has(ae));ce.length&&await tn(ce)}v(D?"Повеската е обновена.":"Повеската е създадена.","success"),Ve(e.querySelector("#duty-modal")),_a(e),await ss(e)}function Go(e,t){const r=Ut(t.dutyFiles);e.querySelector("#duty-id").value=t.id,e.querySelector("#duty-name").value=t.name??"",e.querySelector("#duty-type").value=t.dutyTypeId??"";const n=e.querySelector("#duty-schedule-keys"),s=t.scheduleKeyIds||[];Array.from(n.options).forEach(o=>{o.selected=s.includes(o.value)});const a=e.querySelector("#duty-trains"),i=t.trainIds||[];Array.from(a.options).forEach(o=>{o.selected=i.includes(o.value)}),yt(e,t.startTime??"","#duty-start","#duty-start-time"),yt(e,t.endTime??"","#duty-end","#duty-end-time"),e.querySelector("#duty-second-day").checked=!!t.secondDay,yt(e,Nt(t.breakStartTime),"#duty-break-start","#duty-break-start-time"),yt(e,Nt(t.breakEndTime),"#duty-break-end","#duty-break-end-time"),e.querySelector("#duty-notes").value=t.notes??"",e.querySelector("#duty-existing-attachments").value=Zr(r)||"",e.querySelector("#duty-draft-attachments").value=Zr(r)||"",e.querySelector("#duty-attachment-file").value="",wa(e,r),e.querySelector("#duty-form-title").textContent="Редакция на Повеска",e.querySelector("#duty-save-btn").textContent="Запази"}function _a(e){e.querySelector("#duty-id").value="",e.querySelector("#duty-name").value="",e.querySelector("#duty-type").value="";const t=e.querySelector("#duty-schedule-keys");Array.from(t.options).forEach(n=>{n.selected=!1});const r=e.querySelector("#duty-trains");Array.from(r.options).forEach(n=>{n.selected=!1}),yt(e,"","#duty-start","#duty-start-time"),yt(e,"","#duty-end","#duty-end-time"),e.querySelector("#duty-second-day").checked=!1,yt(e,"00:00","#duty-break-start","#duty-break-start-time"),yt(e,"00:00","#duty-break-end","#duty-break-end-time"),e.querySelector("#duty-notes").value="",e.querySelector("#duty-existing-attachments").value="",e.querySelector("#duty-draft-attachments").value="",e.querySelector("#duty-attachment-file").value="",wa(e,[]),e.querySelector("#duty-form-title").textContent="Нова Повеска",e.querySelector("#duty-save-btn").textContent="Създай"}async function Kp(e,t){const r=t.querySelector("#duty-delete-confirm"),n=r.innerHTML;r.disabled=!0,r.innerHTML='<span class="spinner-border spinner-border-sm me-2" role="status" aria-hidden="true"></span>Изтриване...';const{data:s,error:a}=await S.from("duties").select("duty_files").eq("id",e).maybeSingle();if(a){r.disabled=!1,r.innerHTML=n,v(a.message,"error");return}const{error:i}=await S.from("duties").delete().eq("id",e);if(r.disabled=!1,r.innerHTML=n,i){v(i.message,"error");return}const o=Ut(s==null?void 0:s.duty_files).map(l=>Gs(l.url)).filter(Boolean);o.length&&await tn(o),v("Повеската е изтрита.","success"),Ve(t.querySelector("#duty-delete-modal")),_a(t),await ss(t)}async function zp(e,t){const{error:r}=await S.from("schedule_key_duties").delete().eq("duty_id",e);if(r)return r;const n=t.map(a=>({duty_id:e,schedule_key_id:a})),{error:s}=await S.from("schedule_key_duties").insert(n);return s}async function Wp(e,t){const{error:r}=await S.from("duty_trains").delete().eq("duty_id",e);if(r)return r;if(!t.length)return null;const n=t.map((a,i)=>({duty_id:e,train_id:a,sequence_order:i+1})),{error:s}=await S.from("duty_trains").insert(n);return s}function wa(e,t){const r=e.querySelector("#duty-current-attachments-wrap"),n=e.querySelector("#duty-current-attachments-links"),s=e.querySelector("#duty-draft-attachments");if(!r||!n||!s)return;const a=Kn(t);if(s.value=Zr(a)||"",!a.length){r.classList.add("d-none"),n.innerHTML="";return}r.classList.remove("d-none"),n.innerHTML=a.map((i,o)=>{const l=i.label||en(i.url,o);return`
...
`,Qi=new Map;function Yi(e){const t=String(e||"").trim();if(!t)return"99:99:99";const r=t.match(/^(\d{1,2}):(\d{2})(?::(\d{2}))?/);if(!r)return"99:99:99";const n=String(Number(r[1])).padStart(2,"0"),s=r[2],a=r[3]||"00";return`${n}:${s}:${a}`}function Am(e){const t=(e==null?void 0:e.first_name)??"",r=(e==null?void 0:e.last_name)??"";return`${t} ${r}`.trim()}function Cm(e){var r;const t=e==null?void 0:e.positions;return Array.isArray(t)?((r=t[0])==null?void 0:r.title)??"":t&&typeof t=="object"?t.title??"":""}function Ys(e){const t=String((e==null?void 0:e.assignment_role)||"").trim().toLowerCase();if(t==="chief"||t==="conductor")return t;const r=Cm(e==null?void 0:e.employees).toLowerCase();return r.includes("началник")&&r.includes("влак")?"chief":"conductor"}function Mt(e){var r;const t=e==null?void 0:e.duty_types;return Array.isArray(t)?((r=t[0])==null?void 0:r.name)??"":t&&typeof t=="object"?t.name??"":""}function Wn(e){const t=e==null?void 0:e.duties;return Array.isArray(t)?t[0]||null:t&&typeof t=="object"?t:null}function Rm(){const t=new URLSearchParams(window.location.search).get("date")||"";return/^\d{4}-\d{2}-\d{2}$/.test(t)?t:""}function _e(e){return String(e).replaceAll("&","&amp;").replaceAll("<","&lt;").replaceAll(">","&gt;").replaceAll('"',"&quot;").replaceAll("'","&#039;")}function Im(e){e==null||e.classList.remove("d-none"),document.body.classList.add("overflow-hidden")}function Mn(e){e==null||e.classList.add("d-none"),!!document.querySelector("#schedule-actual-edit-modal:not(.d-none)")||document.body.classList.remove("overflow-hidden")}function Dm(e,t){const r=Qi.get(e);r&&document.removeEventListener("keydown",r);const n=s=>{if(s.key==="Escape"){for(const a of t)if(a&&!a.classList.contains("d-none")){Mn(a);return}}};Qi.set(e,n),document.addEventListener("keydown",n)}const Xi=96/25.4;function Pm(e,{orientation:t,compact:r,fitOnePage:n}){const s=document.documentElement,a=e.querySelector(".plan-schedule-sheet");if(s.classList.add("print-preparing"),s.classList.add("print-hide-second-day"),s.classList.toggle("print-compact",r),s.classList.toggle("print-fit-one-page",n),a&&(a.classList.toggle("print-landscape-page",t==="landscape"),a.classList.toggle("print-portrait-page",t==="portrait")),!n||!a){s.style.setProperty("--plan-print-scale","1");return}s.style.setProperty("--plan-print-scale","1");const i=a.getBoundingClientRect(),o=t==="portrait"?210:297,l=t==="portrait"?297:210,d=(o-20)*Xi,c=(l-20)*Xi,u=d/Math.max(i.width,1),p=c/Math.max(i.height,1),m=Math.min(u,p,1);s.style.setProperty("--plan-print-scale",String(Math.max(.6,m)))}function Om(){const e=document.documentElement;e.classList.remove("print-preparing","print-compact","print-fit-one-page","print-hide-second-day"),e.style.setProperty("--plan-print-scale","1"),document.querySelectorAll(".plan-schedule-sheet").forEach(t=>{t.classList.remove("print-landscape-page","print-portrait-page")})}function Mm(e,t){const r=new Map;return e.forEach(n=>{var l;if(!(n!=null&&n.duty_id)||!(n!=null&&n.employees)||!(n!=null&&n.id)||n!=null&&n.employee_id&&(t!=null&&t.has(n.employee_id)))return;const s=r.get(n.duty_id)||{chiefs:[],conductors:[]},a=Am(n.employees),i=Ys(n),o={id:n.id,employeeId:n.employee_id,role:i,name:a,dutyName:((l=Wn(n))==null?void 0:l.name)||"",date:n.date||""};i==="chief"?a&&!s.chiefs.some(d=>d.id===o.id)&&s.chiefs.push(o):i==="conductor"&&a&&!s.conductors.some(d=>d.id===o.id)&&s.conductors.push(o),r.set(n.duty_id,s)}),r}function Er(e,t,r,n){Ts(e.querySelector("#schedule-train"),t.train,r,n,{allowAdd:!0,allowEdit:!0,conductorRows:2,printConductorRows:3,printExtraCardRows:1,showHours:!0,separateSecondDay:!0,minPanels:2,printAsCards:!0,printHideSecondDay:!0}),Ts(e.querySelector("#schedule-business-trip"),t.businessTrip,r,n,{allowAdd:!0,allowEdit:!0,conductorRows:3,showHours:!1,minPanels:1,hideEmptyConductorRows:!0}),Ts(e.querySelector("#schedule-day-off"),t.dayOff,r,n,{allowAdd:!0,allowEdit:!0,conductorRows:3,showHours:!1,minPanels:1,hideEmptyConductorRows:!0})}function $r(e,{hint:t,error:r,empty:n}){const s=e.querySelector("#schedule-hint"),a=e.querySelector("#schedule-error"),i=e.querySelector("#schedule-empty");s&&(s.textContent=t||"",s.classList.toggle("d-none",!t)),a&&(a.textContent=r||"",a.classList.toggle("d-none",!r)),i&&(i.textContent=n||"",i.classList.toggle("d-none",!n))}function jm(e){const t=new Date(`${e}T00:00:00`);return Number.isNaN(t.getTime())?e:new Intl.DateTimeFormat("bg-BG",{day:"2-digit",month:"long",year:"numeric"}).format(t)}function Xs(e,t){const r=(e==null?void 0:e.schedule_key_id)||"",n=(t==null?void 0:t.schedule_key_id)||"";if(r!==n)return String(r).localeCompare(String(n),"bg");const s=Number.isFinite(Number(e==null?void 0:e.display_order))?Number(e.display_order):Number.MAX_SAFE_INTEGER,a=Number.isFinite(Number(t==null?void 0:t.display_order))?Number(t.display_order):Number.MAX_SAFE_INTEGER;return s!==a?s-a:String((e==null?void 0:e.name)||"").localeCompare(String((t==null?void 0:t.name)||""),"bg")}function Nm(e,t){const r=!!(e!=null&&e.second_day),n=!!(t!=null&&t.second_day);if(r!==n)return r?1:-1;const s=Yi(e==null?void 0:e.start_time),a=Yi(t==null?void 0:t.start_time);return s!==a?s.localeCompare(a,"bg"):Xs(e,t)}function Ts(e,t,r,n,s={}){if(!e)return;if(!t.length){e.innerHTML='<p class="text-secondary mb-0">Няма повески от този тип.</p>';return}const a=s.separateSecondDay?Um(t):t,i=Number.isInteger(s.conductorRows)&&s.conductorRows>=0?s.conductorRows:3,o=5,l=Fm(a,o),d=Number.isInteger(s.minPanels)&&s.minPanels>0?s.minPanels:1,c=Number.isInteger(s.printConductorRows)&&s.printConductorRows>0?s.printConductorRows:i;for(;l.length<d;)l.push([]);const u=l.map(h=>{const f=[...h];for(;f.length<o;)f.push(null);const _=f.map(E=>{const x=Rn(E,"text-center"),L=ut(E)?"":(E==null?void 0:E.name)??"";if(!L)return`<th scope="col"${x}></th>`;const A=String((E==null?void 0:E.notes)||"").trim(),$=A?`<div class="schedule-duty-note" title="${_e(A)}">${_e(A)}</div>`:"";return`<th scope="col"${x}><span class="schedule-duty-name-wrap">${Cn("Влак","train")}<span class="schedule-duty-name-ellipsis" title="${_e(L)}">${_e(L)}</span></span>${$}</th>`}).join(""),y=f.map(E=>{const x=Rn(E),L=E&&!ut(E)?dl(E):"";return!E||ut(E)?`<td${x}></td>`:`<td${x}>${Cn("Час","hours")}${_e(L)}</td>`}).join(""),g=f.map(E=>{if(!E)return"<td></td>";const x=Rn(E);if(ut(E))return`<td${x}></td>`;const L=r.get(E.id)||{chiefs:[]},A=Mt(E).toLowerCase();return`<td${x} data-drop-duty-id="${E.id}" data-drop-duty-name="${_e((E==null?void 0:E.name)||"")}" data-drop-date="${n}" data-drop-role="chief" data-drop-duty-type="${_e(A)}">${Cn("НВ","chief")}${Hm(L.chiefs,E,n,s)}</td>`}).join("");let b=i;if(s.hideEmptyConductorRows){const E=f.reduce((x,L)=>{if(!L||ut(L))return x;const A=r.get(L.id)||{conductors:[]},$=Array.isArray(A.conductors)?A.conductors.length:0;return Math.max(x,$)},0);b=Math.min(i,E)}const w=b>0?Array.from({length:b},(E,x)=>`
      ...
      `;if(!s.printAsCards)return T;const q=Zi(f,r,c,s);return`
        <div class="print-as-cards">
          ${T}
