alter table public.planned_duties
add column if not exists assignment_role text;

update public.planned_duties pd
...
where pd.employee_id = e.id
  and pd.assignment_role is null;

update public.planned_duties
...
begin
  if not exists (
    select 1
    from pg_constraint
    where conname = 'planned_duties_assignment_role_check'
  ) then
    alter table public.planned_duties
      add constraint planned_duties_assignment_role_check
